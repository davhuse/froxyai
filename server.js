...
function repairTurkishMojibake(value) {
  ...
  let out = value;
  const badPattern = /[\u00c2\u00c3\u00c4\u00c5\u00e2\uFFFD]|\?[a-zA-ZğüşıöçÄ?ÜÅ?İÖÇ]/;
  ...
  const phrases = [
    ['Geçersiz veya kullan?lm? token', 'Geçersiz veya kullanılmış token'],
    ['Token süresi dolmuş', 'Token süresi dolmuş'],
    ['Görsel şu an', 'Görsel şu an'],
    ['Lütfen başka', 'Lütfen başka'],
    ['başka bir', 'başka bir'],
    ['yanıt vermedi', 'yanıt vermedi'],
    ['döndü', 'döndü']
  ];
...
if (!JWT_SECRET) {
  console.warn('[SECURITY] JWT_SECRET env degiskeni bulunamadi. Gecici fallback secret kullaniliyor; Railway Variables içinde kalici bir deger ekleyin.');
}
...
const SHOPIER_PACKAGE_CATALOG = {
  starter: { productId: '47408136', name: 'Başlangic', label: 'Başlangıç', credits: 5000, price: 129.99 },
...
function checkDailyLimit(userId, type) {
  ...
  if (type === 'image' && user.daily_image_count >= limits.image) {
    return { allowed: false, reason: 'Gunluk görsel limitinize ulastiniz (' + limits.image + '/' + limits.image + '). Paketinizi yukseltebilirsiniz.' };
  }
  return { allowed: true };
}
...
if (ADMIN_EMAIL && ADMIN_PASSWORD) {
  try {
    ...
    if (!existing) {
      ...
      console.log(`[ADMIN] OK Admin hesabi oluşturuldu: ${ADMIN_EMAIL}`);
...
function getShopierPlanFromPayload(payload) {
  ...
  if (/başlangic|ba\u015flang\u0131\u00e7|starter/.test(text)) return 'starter';
  return null;
}
...
function sendOtpMail({ to, code, purpose }) {
  ...
  const action = isRegister ? 'hesabınızi oluşturmak' : 'hesabınıza giris yapmak';
  const subject = title;
  const text = `${title}: ${code}\n\nBu kod ${LOGIN_OTP_TTL_MINUTES} dakika gecerlidir. Bu islemi siz başlatmadiysaniz bu e-postayi yok sayabilirsiniz.`;
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;background:#070914;color:#f8fafc;padding:28px">
      <div style="max-width:520px;margin:auto;background:#0f172a;border:1px solid #25324a;border-radius:16px;padding:24px">
        ...
        <p style="color:#cbd5e1">Froxy AI ${action} için asagidaki kodu kullanin.</p>
...
app.post('/api/register', authLimiter, async (req, res) => {
  ...
  if(!termsAccepted || !privacyAccepted) {
    ...
    return res.status(400).json({error: 'Kayit için KVKK/Gizlilik ve Kullanim Sartlari onaylarini kabul etmelisiniz.'});
  }
  if(!isAllowedRegistrationEmail(email)) {
    recordSecurityAudit(req, 'register_email_domain', 'blocked', 'untrusted_email_domain', null, { email });
    return res.status(400).json({error: 'Kayit için Gmail, Outlook, Hotmail veya Yandex gibi guvenilir bir e-posta adresi kullanin.'});
  }
...
app.post('/api/dodo/start', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const fallbackUrl = SHOPIER_STATIC_URLS[plan] || 'https://www.shopier.com/froxyai';
    res.status(e.status || 500).json({ error: 'Dodo odeme başlatilamadi: ' + e.message, fallback_url: fallbackUrl });
  }
});
...
app.post('/api/forgot-password', authLimiter, (req, res) => {
  ...
  try {
    ...
    if(user) {
      ...
      // Reset token console'a yazilmaz; sadece dev response içinde doner.
      const payload = { success: true, message: 'Sifre sifirlama baglantisi e-postaniza gonderildi.' };
      if (process.env.NODE_ENV !== 'production') payload.dev_token = token;
      res.json(payload);
    } else {
      res.json({ success: true, message: 'Sifre sifirlama baglantisi e-postaniza gonderildi.' });
    }
  } catch(e) {
    res.status(500).json({error: e.message});
  }
...
app.post('/api/reset-password', authLimiter, (req, res) => {
  ...
  try {
    ...
    if(!rt) return res.status(400).json({error: 'Gecersiz veya kullanilmis token'});
    ...
    res.json({ success: true, message: 'Sifreniz basariyla sifirlandi.' });
  } catch(e) {
    res.status(500).json({error: e.message});
  }
});
...
const SEO_PAGES = {
  '/': {
    title: 'Froxy AI - Türkçe AI İş Asistanı',
    description: 'KOBİ, ajans ve içerik üreticiler için Türkçe AI iş asistanı. 5 dakikada satış metni, sosyal medya postu, müşteri cevabi ve görsel prompt üret.'
  },
  '/sohbet': {
    title: 'AI Sohbet Paneli - ChatGPT, Claude ve Gemini | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve farklı AI modelleriyle tek panelden Türkçe sohbet et. Froxy AI ile 100 ücretsiz krediyle başla.'
  },
  '/panel': {
    title: 'AI Dashboard ve Tek Panel Yapay Zeka Platformu | Froxy AI',
    description: 'Sohbet, görsel üretim, kod, belge ve AI ajanlarini tek dashboard uzerinden yonet.'
  },
  '/dashboard': {
    title: 'Froxy AI Dashboard - Tum AI Araçları Tek Ekranda',
    description: 'Kredi bakiyeni, is içerik üretimlerini, görsel promptlarini ve hazir AI araçlarıni tek dashboard uzerinden takip et.'
  },
  '/görsel-üret': {
    title: 'AI Görsel Üretme Aracı - Yapay Zeka ile Görsel Üret | Froxy AI',
    description: 'Prompt yazarak AI görsel üret. Farkli görsel modellerini tek panelde kullan ve 100 ücretsiz krediyle dene.'
  },
  '/ai-araclar': {
    title: 'AI Araçları - ChatGPT, Claude, Gemini ve Daha Fazlasi | Froxy AI',
    description: 'Yapay zeka sohbet, görsel üretim, kod, belge, ses ve video araçlarıni tek panelde kullan.'
  },
  '/ai-ajanlar': {
    title: 'AI Ajanlar - Gorev Odakli Yapay Zeka Asistanlari | Froxy AI',
    description: 'İçerik, analiz, kod, pazarlama ve otomasyon için AI ajanlariyla daha hizli calis.'
  },
  '/fiyatlandirma': {
    title: 'Froxy AI Fiyatlandirma - 100 Ücretsiz Krediyle Dene',
    description: 'Froxy AI kredi paketlerini incele. Yeni uyeler 100 ücretsiz krediyle is içerik akışını deneyebilir.'
  },
  '/kayit': {
    title: 'Froxy AI Kayit Ol - 100 Ücretsiz Kredi',
    description: 'Froxy AI hesabını oluştur, 100 ücretsiz krediyle Türkçe AI iş asistanıni dene.'
  },
  ...
  '/en-iyi-ai-araçları': {
    title: 'KOBİ İçin Türkçe AI İş Asistanı | Froxy AI',
    description: 'Sohbet, görsel, kod, belge ve pazarlama için en iyi AI araçlarıni tek panelde toplayan Froxy AI platformunu kesfet.'
  },
  '/chatgpt-alternatifi': {
    title: 'ChatGPT Alternatifi AI Platformu | Froxy AI',
    description: 'ChatGPT alternatifi arayan KOBİ, ajans ve içerik ekipleri için Türkçe AI iş asistanı; metin, müşteri cevabi ve görsel promptu tek akışta üret.'
  },
  '/ai-görsel-üretme': {
    title: 'AI Görsel Üretme Rehberi ve Aracı | Froxy AI',
    description: 'Yapay zeka ile görsel üretmek için prompt yaz, farklı görsel modellerini dene ve tek panelden sonuc al.'
  },
  '/yapay-zeka-araçları': {
    title: 'Yapay Zeka Araçları - Tek Panelde AI Platformu | Froxy AI',
    description: 'Yapay zeka araçları, AI sohbet modelleri, görsel üretim ve ajanlari tek panelde kullan.'
  },
  '/ücretsiz-ai-araçları': {
    title: 'Ücretsiz AI Araçları - 100 Krediyle Dene | Froxy AI',
    description: 'Ücretsiz AI araçları arayanlar için Froxy AI yeni uyelere 100 kredi verir. ChatGPT, Claude, Gemini ve daha fazlasini dene.'
  },
...
const SEO_CONTENT = {
  '/chatgpt-claude-gemini-tek-panel': {
    ...
    lead: 'Froxy AI, populer yapay zeka modellerini tek hesapta kullanmak isteyenler için gelistirilmis bir AI calisma alanidir. ChatGPT, Claude, Gemini ve diger modeller arasinda sekme degistirmeden ilerleyebilir, ayni promptu farklı modellerde deneyebilir ve sonucu tek panelde karsilastirabilirsiniz.',
    sections: [
      ['Neden tek panel kullanilmali?', 'ChatGPT hizli fikir üretimi ve genel yardimda gucluyken, Claude uzun metinlerde ve duzenli cevaplarda one cikabilir. Gemini ise Google ekosistemi ve cok modlu is akislari için kullanislidir. Bu modellerin hepsine ayri ayri girmek zaman kaybettirir. Froxy AI, bu daginikligi azaltarak modelleri tek kredi sistemi ve tek arayüzde toplar.'],
      ['Kimler için uygun?', 'İçerik üretenler, ogrenciler, yazilim gelistiriciler, sosyal medya yoneticileri, e-ticaret ekipleri ve kucuk isletmeler ayni panelden farklı AI modellerini deneyebilir. Boylece her is için en uygun modeli elle aramak yerine tek ekrandan secim yapilir.'],
      ['Froxy AI ile nasil başlanir?', 'Ücretsiz hesap oluşturduktan sonra 100 krediyle başlayabilir, sohbet ekraninda model secip ilk promptunuzu yazabilirsiniz. Isterseniz ayni fikri ChatGPT, Claude ve Gemini benzeri farklı modellerle test ederek hangi cevabin daha iyi oldugunu gorebilirsiniz.']
    ],
    faq: [
      ['ChatGPT, Claude ve Gemini ayni anda kullanilabilir mi?', 'Froxy AI panelinde farklı modeller arasinda gecis yapabilir ve ayni is akışını tek hesapta yonetebilirsiniz.'],
      ['Başlamak için kart gerekir mi?', 'Hayir. Yeni uyeler 100 ücretsiz krediyle paneli deneyebilir.'],
      ['Hangi model daha iyi?', 'Tek bir en iyi model yoktur. Metin, kod, analiz, görsel fikir veya uzun belge gibi goreve gore model tercihi degisir.']
    ]
  },
  '/en-iyi-ai-araçları': {
    h1: 'En İyi AI Araçları Tek Panelde',
    lead: 'En iyi AI aracı, tek bir modelden ibaret degildir. Gunluk islerde sohbet modeli, görsel üretim aracı, kod yardimcisi, belge analiz aracı ve ajan tabanli is akislari gerekebilir. Froxy AI bu araçları tek panelde bir araya getirerek arama ve deneme surecini kisaltir.',
    sections: [
      ['AI araçları hangi islerde kullanilir?', 'Yapay zeka araçları reklam metni yazma, blog fikri bulma, kod duzenleme, belge ozetleme, sosyal medya basligi hazirlama, görsel üretme ve müşteri destek taslaklari oluşturma gibi cok farklı islerde kullanilir.'],
      ['Tek panel avantajlari', 'Farkli sitelerde hesap acmak, kredi takip etmek ve model secmek yerine tek panel kullanmak daha pratiktir. Froxy AI, sohbet ve görsel üretim gibi araçları ayni deneyimde topladigi için deneme maliyetini ve zaman kaybini azaltir.'],
      ['AI araçlarıni secmenin dogru yolu', 'Sadece populer olan modeli kullanmak yerine, ayni gorevi birden fazla modelle deneyip kalite, hiz ve maliyet acisindan karsilastirma yapmak daha dogru bir yoldur. Froxy AI bu karsilastirmayi kolaylastirir.']
    ],
    faq: [
      ['Froxy AI ücretsiz denenebilir mi?', 'Evet. Yeni hesaplar 100 ücretsiz krediyle başlar.'],
      ['Hangi AI araçları var?', 'Sohbet, görsel üretim, belge analizi, kod yardimi, prompt araçları ve AI ajanlari gibi farklı araclar tek panelde sunulur.'],
      ['Tek panel performansi etkiler mi?', 'Tek panel, farklı modellere erisimi daha kolay hale getirir; kalite ise secilen modele ve prompta baglidir.']
    ]
  },
  '/chatgpt-alternatifi': {
    h1: 'ChatGPT Alternatifi AI Platformu',
    lead: 'ChatGPT cok guclu bir modeldir, ancak her is için tek secenek degildir. Claude, Gemini, Mistral, DeepSeek ve diger modeller bazi gorevlerde daha uygun olabilir. Froxy AI, ChatGPT alternatifi arayan kullanicilara tek panelden cok model deneme imkani verir.',
    sections: [
      ['Neden ChatGPT alternatifi aranir?', 'Kimi kullanicilar daha uzun metinlerde daha duzenli cevap, kimi kullanicilar kodda daha farklı yaklasim, kimi kullanicilar da maliyet ve hiz avantajlari arar. Alternatif modelleri denemek, tek modele bagli kalmadan daha iyi sonuca ulasmayi saglar.'],
      ['Froxy AI nasil fark yaratir?', 'Froxy AI sadece bir ChatGPT kopyasi degildir. Farkli model ailelerini, görsel araçları ve AI ajanlarini tek arayüzde sunar. Bu sayede kullanici isine gore model degistirebilir.'],
      ['Kimler kullanmali?', 'İçerik ekipleri, yazilimcilar, ogrenciler ve isletmeler ChatGPT alternatifi modelleri test ederek daha esnek bir yapay zeka calisma duzeni kurabilir.']
    ],
    faq: [
      ['Froxy AI ChatGPT yerine kullanilir mi?', 'Evet, ChatGPT benzeri modellerle birlikte farklı alternatifleri de tek panelde denemek için kullanilabilir.'],
      ['Claude ve Gemini de var mi?', 'Froxy AI, farklı saglayicilardan cok sayida modeli tek panelde sunmayi hedefler.'],
      ['Ücretsiz kredi ne ise yarar?', '100 krediyle paneli test edebilir, ilk sohbet ve araclardan bazilarini deneyebilirsiniz.']
    ]
  },
  '/ai-görsel-üretme': {
    h1: 'AI Görsel Üretme Aracı',
    lead: 'AI görsel üretme, yazdiginiz promptu görsele donusturen yapay zeka modelleriyle calisir. Froxy AI, görsel üretim araçlarıni sohbet ve diger AI modelleriyle ayni panelde birlestirerek fikirden görsele daha hizli gecmenizi saglar.',
    sections: [
      ['AI görsel nasil üretilir?', 'Once ne istediginizi anlatan bir prompt yazarsiniz. Stil, renk, oran, konu ve detay seviyesi gibi unsurlar sonucu belirler. Froxy AI panelinde promptunuzu girerek farklı görsel üretim akislariyla deneme yapabilirsiniz.'],
      ['Kimler için faydali?', 'Sosyal medya içerik üreticileri, reklam hazirlayanlar, e-ticaret sahipleri, sunum hazirlayanlar ve tasarim fikri arayan herkes AI görsel üretimden faydalanabilir.'],
      ['Daha iyi sonuc için ipuclari', 'Promptta konu, ortam, stil, renk paleti ve kullanim amacini net belirtmek gerekir. Ornegin sadece “teknoloji görseli? yerine “karanlik premium SaaS paneli, cyan ve mor neon vurgular, dikey reklam posteri? gibi acik bir prompt daha iyi sonuc verir.']
    ],
    faq: [
      ['AI görsel üretmek için tasarim bilmek gerekir mi?', 'Hayir. İyi bir prompt yazmak genelde yeterlidir, denemelerle sonuc iyilesir.'],
      ['Görsel üretim kredi harcar mi?', 'Evet, secilen model ve isleme gore kredi harcanir.'],
      ['Froxy AI ile reklam görseli hazirlanabilir mi?', 'Evet. Urun, hedef kitle ve stil bilgisiyle reklam odakli görseller üretilebilir.']
    ]
  },
  '/yapay-zeka-araçları': {
    h1: 'Yapay Zeka Araçları ve AI Platformu',
    lead: 'Yapay zeka araçları artik sadece sohbet botlarindan olusmuyor. Metin üretimi, kod yardimi, görsel üretme, belge okuma, ozetleme, pazarlama ve otomasyon gibi isler için farklı araclar gerekiyor. Froxy AI bu araçları tek panelde toplamaya odaklanir.',
    sections: [
      ['Hangi yapay zeka araçları onemli?', 'Sohbet modelleri fikir üretme ve yazma islerinde kullanilir. Kod araçları gelistirme surecini hizlandirir. Görsel araclar reklam ve sosyal medya üretimini destekler. Belge araçları uzun metinleri ozetler ve analiz eder.'],
      ...
      ['Froxy AI ile başlama', 'Yeni kullanicilar 100 ücretsiz krediyle hesap oluşturup sohbet, model secimi ve görsel üretim gibi temel akislari deneyebilir.']
    ],
    faq: [
      ['Yapay zeka araçları isletmeler için uygun mu?', 'Evet. İçerik, destek, analiz ve pazarlama gibi alanlarda isletmelere zaman kazandirabilir.'],
      ['Tek bir AI modeli yeterli mi?', 'Cogu zaman hayir. Farkli isler için farklı modeller daha iyi sonuc verebilir.'],
      ['Froxy AI hangi vaadi sunuyor?', 'KOBİ, ajans ve içerik üreticileri için is odakli AI akislari sunar: satış metni, sosyal medya postu, müşteri cevabi ve görsel promptu.']
    ]
  }
};
...
Object.assign(SEO_PAGES, {
  ...
  '/sohbet': {
    title: 'AI Sohbet Paneli - Türkçe İş Asistanı | Froxy AI',
  ...
  '/chatgpt-claude-gemini-tek-panel': {
    title: 'ChatGPT, Claude ve Gemini Tek Panelde | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve Türkçe AI iş asistanıni tek hesapta kullan. Sekme değiştirmeden sohbet et, görsel üret ve araçları çalıştır.'
  },
  '/400-ai-model': {
    title: 'Froxy AI Model Paneli - Türkçe AI İş Asistanı',
    description: 'Bu eski model sayfası uyumluluk için korunur; güncel Froxy AI konumlandırması KOBİ, ajans ve içerik üreticileri için Türkçe AI iş asistanıdir.'
  },
...
Object.assign(SEO_CONTENT, {
  ...
  '/400-ai-model': {
    h1: 'Türkçe AI iş asistanı',
    lead: 'Bu sayfa eski model arama ve paylaşım linkleri için uyumluluk sağlar; güncel Froxy AI mesajı Türkçe AI iş asistanı, 100 başlangic kredisi ve kart bilgisi gerekmez teklifidir.',
    sections: [
      ['Güncel model vaadi', 'Froxy AI, farklı model ailelerini is odakli akislara baglayarak satış metni, sosyal medya icerigi, müşteri cevabi ve görsel prompt üretimini hizlandirir.'],
      ['Eski link neden duruyor?', 'Daha once paylasilan veya indekslenen model linkleri bozulmasin diye sayfa korunur ve yeni iş asistanı konumlandırmasına yonlendirilir.'],
      ['Başlangic teklifi', 'Yeni uyeler 100 ücretsiz krediyle kart bilgisi girmeden paneli deneyebilir.']
    ],
    faq: [
      ['Froxy AI ne sunuyor?', 'Güncel tanıtım mesajı KOBİ, ajans ve içerik üreticileri için Türkçe AI iş asistanıdir.'],
      ['400-ai-model linki neden var?', 'Eski SEO ve paylaşım linklerinin bozulmamasi için uyumluluk sayfası olarak korunur.'],
      ['Denemek için kart gerekir mi?', 'Hayir. Yeni uyeler 100 ücretsiz krediyle başlayabilir.']
    ]
  },
  '/chatgpt-claude-gemini': {
  ...
  '/600-ai-model': {
    ...
    sections: [
      ...
      ['Model karmaşası nasıl azaltılır?', 'Önerilen, en ucuz, en hızlı, en kaliteli, görsel okur ve kod gibi filtreler kullanıcıyı doğru modele yönlendirir. Böylece is odakli model seçeneği kalabalık değil, avantaj haline gelir.']
    ],
    faq: [
...
Object.assign(SEO_PAGES, {
  ...
  '/kayit': {
    ...
    description: 'Froxy AI hesabını oluştur, kart bilgisi girmeden 100 ücretsiz krediyle Türkçe AI iş asistanıni tek panelde dene.'
  },
  '/chatgpt-claude-gemini-tek-panel': {
    title: 'ChatGPT, Claude ve Gemini Tek Panelde | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve Türkçe AI iş asistanıni tek hesapta kullan. Sekme değiştirmeden sohbet et, görsel üret ve araçları çalıştır.'
  ...
  '/en-iyi-ai-araçları': {
    title: 'En İyi AI Araçları - Türkçe AI İş Asistanı | Froxy AI',
  ...
  '/chatgpt-alternatifi': {
    title: 'ChatGPT Alternatifi AI Platformu | Froxy AI',
    description: 'ChatGPT alternatifi arayanlar için Claude, Gemini, Mistral ve Türkçe AI iş asistanıni tek hesapta kullanma çözümü.'
  },
  '/ai-görsel-üretme': {
    title: 'AI Görsel Üretme Rehberi ve Aracı | Froxy AI',
...
Object.assign(SEO_CONTENT, {
  ...
  '/yapay-zeka-araçları': {
    ...
    faq: [
      ...
      ['Froxy AI hangi vaadi sunuyor?', 'Türkçe AI iş asistanıni ve farklı yapay zeka araçlarını tek panelde kullanma deneyimi sunar.']
    ]
  },
  '/ai-model-karsilastirma': {
...
// v483: stronger acquisition SEO titles/descriptions for organic landing routes.
Object.assign(SEO_PAGES, {
  '/chatgpt-alternatifi': {
    title: 'ChatGPT Alternatifi Türkçe AI Platformu | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve Türkçe AI iş asistanıni tek panelde dene. Türkçe arayüz, kredi takibi, görsel üretim ve prompt kütüphanesiyle ücretsiz başla.'
  },
  '/ai-görsel-üretme': {
    title: 'AI Görsel Üretme Aracı - Ücretsiz Krediyle Dene | Froxy AI',
    description: 'Prompt yazarak AI görsel üret, farklı görsel modelleri karsilastir ve sonucu tek panelden yonet. Yeni uyeler 100 krediyle başlar.'
  },
  '/ücretsiz-ai-araçları': {
    title: 'Ücretsiz AI Araçları ve Türkçe İş Asistanı | Froxy AI',
    description: 'Sohbet, görsel üretim, promptlar, dosya analizi ve populer AI modellerini tek hesapla dene. Ücretsiz başlangic kredisiyle hemen başla.'
  },
  '/türkçe-ai-platformu': {
    title: 'Türkçe AI Platformu - ChatGPT, Claude, Gemini | Froxy AI',
    description: 'Türkçe arayüzle ChatGPT, Claude, Gemini, Llama ve görsel AI modellerini tek panelde kullan. Kredi sistemiyle maliyeti kontrol et.'
  },
  '/600-ai-model': {
    title: 'Türkçe AI İş Asistanı - Froxy AI',
    description: 'OpenAI, Claude, Gemini, Llama, Mistral, Qwen ve görsel modelleri tek panelden sec. Model degistir, kredi maliyetini gor, hizli dene.'
  }
});

const SEO_RELATED_LINKS = [
  ['ChatGPT Claude Gemini tek panel', '/chatgpt-claude-gemini-tek-panel'],
  ['Türkçe AI iş asistanı', '/600-ai-model'],
  ['AI kredi sistemi', '/ai-kredi-sistemi'],
  ['Türkçe AI platformu', '/türkçe-ai-platformu'],
  ['AI dosya analizi', '/ai-dosya-analizi'],
  ['AI fotoğraf düzenleme', '/ai-fotograf-duzenleme'],
  ['ChatGPT alternatifi', '/chatgpt-alternatifi'],
  ['AI görsel üretme', '/ai-görsel-üretme'],
...
function buildSeoTrialCtaV477() {
  return '<section><h2>Froxy AI ile dene</h2><p>ChatGPT, Claude, Gemini, görsel AI ve Türkçe AI iş asistanıni tek panelde denemek için ücretsiz hesap oluşturabilir, 100 başlangic kredisiyle ilk AI is akışını test edebilirsin.</p><p><a href="/kayit" data-seo-cta="signup" style="color:#67e8f9;font-weight:800">100 ücretsiz krediyle kayit ol</a></p></section>';
}
...
function sendSeoIndex(req, res) {
  ...
  fs.readFile(path.join(staticRoot, 'index.html'), 'utf8', (err, html) => {
    ...
    const isAppRoute = /^\/(?:sohbet|chat|görsel|görsel-üret|promptlar|ai-araclar|araclar|galeri|destek|admin)$/i.test(key);
...
const appRoutes = new Set([
  ...
  '/kontrol-paneli', '/gorsel', '/görsel', '/görsel-üret', '/araclar', '/ai-araclar', '/ai-araçları',
  '/ajanlar', '/ai-ajanlar', '/magaza', '/fiyatlandirma', '/destek',
  '/galeri', '/analitik', '/promptlar', '/bilgi-bankasi', '/giris',
  '/kayit', '/chatgpt-claude-gemini-tek-panel', '/en-iyi-ai-araçları',
  '/chatgpt-alternatifi', '/ai-görsel-üretme', '/yapay-zeka-araçları',
  '/ücretsiz-ai-araçları', '/ai-model-karsilastirma', '/admin'
]);
...
function buildAppRouteShellHtml(html, key) {
  try {
    ...
    const routeLabel = {
      ...
      '/görsel': 'Görsel üretim',
      '/görsel-üret': 'Görsel üretim',
...
// === FREEMODEL.DEV KEY ROTATION (gpt-5.5/5.4 için) ===
...
function validateGeneratedImageBuffer(buffer, label = 'Görsel', minBytes = 1000) {
  if (!Buffer.isBuffer(buffer) || buffer.length < minBytes) {
    throw new Error(label + ' bos/kucuk görsel dondurdu');
  }
  const magic = imageBufferMagic(buffer);
  if (!magic) {
    const head = buffer.toString('utf8', 0, Math.min(buffer.length, 120)).trim();
    throw new Error(label + ' görsel formatinda degil' + (head ? ': ' + head.slice(0, 80) : ''));
  }
  const stats = pngVisualStats(buffer);
  if (stats) {
    if (stats.alphaAvg !== null && stats.alphaAvg < 5) throw new Error(label + ' tamamen saydam görsel dondurdu');
    if (stats.avgLum < 4 && stats.brightRatio < 0.003) throw new Error(label + ' bos/siyah görsel dondurdu');
    if (stats.avgLum < 8 && stats.variance < 0.25) throw new Error(label + ' tek renk veya bos görsel dondurdu');
  } else {
    ...
    if ((counts.size <= 8 && maxCount / Math.max(1, taken) > 0.72) || zeroes / Math.max(1, taken) > 0.92) {
      throw new Error(label + ' bos veya bozuk görsel dondurdu');
    }
  }
  return magic;
}
...
async function persistImageResult(value, prefix) {
  if (!value) throw new Error('Görsel sonucu bos dondu');
  if (typeof value === 'string' && /^https?:\/\//i.test(value)) {
    const dlRes = await fetch(value, { signal: AbortSignal.timeout(90000) });
    if (!dlRes.ok) throw new Error('Görsel indirilemedi (' + dlRes.status + ')');
    const ct = dlRes.headers.get('content-type') || '';
    const buf = Buffer.from(await dlRes.arrayBuffer());
    if (buf.length < 1000) throw new Error('Görsel sonucu cok kucuk');
  ...
  if (!buf || buf.length < 1000) throw new Error('Görsel sonucu beklenen formatta degil');
...
async function callModalImage({ prompt, imageSize, qualityMode = 'quality', model = 'modal-sdxl', negativePrompt = '' }) {
  if (!MODAL_IMAGE_ENDPOINT) {
    throw new Error('MODAL_IMAGE_ENDPOINT tanimli degil. Modal endpoint deploy edip .env.keys içine ekle.');
  }
  const size = resolveComfyImageSize(imageSize);
...
async function callPerchanceExperimentalImage({ prompt, imageSize }) {
  ...
  if (!base) throw new Error('Perchance için resmi stabil API yok. PERCHANCE_API_URL ile kendi proxy/DIY endpointini tanimla.');
  ...
  for (let i = 0; i < 90; i++) {
    ...
    for (const out of Object.values(outputs)) {
      ...
      if (img?.filename) {
        ...
        if (!viewRes.ok) throw new Error('ComfyUI görsel indirilemedi (' + viewRes.status + ')');
...
async function callComfyLocalVideo({ prompt }) {
  ...
  if (!raw) throw new Error('COMFYUI_VIDEO_WORKFLOW_JSON tanimli degil. Video için ComfyUI workflow JSON gerekli.');
  const base = localBaseUrl('COMFYUI_API_URL', 'http://127.0.0.1:8188');
...
async function callOpenAIImage({ prompt, model, imageSize, apiKey: apiKeyOverride }) {
  ...
  if (item?.b64_json) buffer = Buffer.from(item.b64_json, 'base64');
    ...
    if (!dl.ok) throw new Error(`OpenAI görsel indirilemedi (${dl.status})`);
    buffer = Buffer.from(await dl.arrayBuffer());
  }
  if (!buffer || buffer.length < 1000) throw new Error('OpenAI bos görsel dondurdu');
...
async function callOpenAIImageEdit({ prompt, image, model, imageSize, apiKey: apiKeyOverride }) {
  ...
  if (item?.b64_json) buffer = Buffer.from(item.b64_json, 'base64');
    ...
    const dl = await fetch(item.url, { signal: AbortSignal.timeout(60000) });
    if (!dl.ok) throw new Error(`OpenAI edit görseli indirilemedi (${dl.status})`);
    buffer = Buffer.from(await dl.arrayBuffer());
  }
  if (!buffer || buffer.length < 1000) throw new Error('OpenAI edit bos görsel dondurdu');
...
async function callGoogleDirectImageEdit({ prompt, image, model, apiKey: apiKeyOverride }) {
  ...
  const parts = json.candidates?.[0]?.content?.parts || [];
  const imagePart = parts.find(p => p.inlineData?.data);
  if (!imagePart) throw new Error('Google edit görsel yaniti alinamadi');
  const mime = imagePart.inlineData.mimeType || 'image/png';
  const ext = mime.includes('jpeg') || mime.includes('jpg') ? 'jpg' : mime.includes('webp') ? 'webp' : 'png';
...
app.post(['/api/chat', '/v1/chat/completions', '/v1/responses'], chatLimiter, optionalAuthMiddleware, async (req, res) => {
  ...
  // === SYSTEM PROMPT INJECTION: Türkçe, kaliteli, emoji ile bozuk yanit verme ===
...
app.post('/api/search', chatLimiter, async (req, res) => {
  ...
  try {
    ...
    while ((match = resultRegex.exec(html)) !== null && results.length < 5) {
      const url = match[1].replace('//duckduckgo.com/l/Şuddg=', '').split('&')[0];
  ...
  if (POLLINATIONS_IMAGE_MODEL_ALIASES[imgModel] || ['turbo', 'sana', 'klein', 'wan-image', 'qwen-image', 'flux-realism', 'flux-anime', 'flux-3d'].includes(imgModel) || imgModel.startsWith('style-')) {
    ...
    if (canRecoverPollinationsWithCloudflare && CLOUDFLARE_ACCOUNT_ID && CLOUDFLARE_API_TOKEN) {
      try {
        ...
        if (cfRes.ok) {
          const buf = Buffer.from(await cfRes.arrayBuffer());
          if (buf.length < 10000) throw new Error('Cloudflare fallback cok kucuk/eksik görsel dondurdu');
          const genDir = GENERATED_DIR;
          if (!fs.existsSync(genDir)) fs.mkdirSync(genDir, { recursive: true });
          const fileName = `cf_${Date.now()}.png`;
          fs.writeFileSync(path.join(genDir, fileName), buf);
...
// Browser src='/api/img-proxy?prompt=...' -> server pollinations'a gider, görseli getirir, cache'ler.
...
app.post('/api/video', chatLimiter, async (req, res) => {
  ...
  try {
    if (model === 'vidu-cheap' || model === 'vidu-720') {
      if (!VIDU_API_KEY) {
        return res.status(503).json({ error: 'Vidu video için VIDU_API_KEY gerekli.' });
      }
      ...
      if (!startRes.ok || !startData.task_id) {
        if ((startData.message || startData.error || '').includes('CreditInsufficient')) {
          return res.status(402).json({ error: 'Vidu kredisi yetersiz. API key dogru calisiyor ama hesapta video üretimi için yeterli kredi/free quota yok.' });
        }
        throw new Error(startData.message || startData.error || 'Vidu video başlatilamadi');
      }
      ...
      for (let i = 0; i < 48; i++) {
        ...
        if (pollData.state === 'failed') {
          throw new Error(pollData.err_msg || pollData.err_code || 'Vidu video üretimi basarisiz');
        }
      }

      throw new Error('Vidu video üretim zaman asimi');
    }
    ...
    if (model === 'seedance-2' || model === 'seedance-2-fast') {
      if (FAL_API_KEY) {
        ...
        const startData = await startRes.json();
        if (!startRes.ok) throw new Error(startData.detail || startData.error || 'Seedance başlatilamadi');
        ...
        for (let i = 0; i < 36; i++) {
          ...
          throw new Error('Seedance üretimi basarisiz: ' + checkData.status);
        }
        throw new Error('Seedance video üretim zaman asimi');
      }
      ...
      if (REPLICATE_API_TOKEN) {
        ...
        if (!startRes.ok) throw new Error(pred.detail || pred.error || 'Replicate Seedance başlatilamadi');
      ...
      return res.status(503).json({ error: 'Seedance 2.0 için FAL_API_KEY veya REPLICATE_API_TOKEN gerekli.' });
    }
...
function getModelCreditCost(model, provider) {
  ...
  // ? FREE TIER (3 kredi) ? tamamen ücretsiz saglay\u0131c\u0131lar ?
...
app.get('/api/credit-costs', (req, res) => {
  res.json({
    ...
    info: {
      free: 'Ücretsiz saglay\u0131c\u0131lar \u2014 Groq, Pollinations, Cerebras, OpenRouter :free (3 kredi)',
...
function buildSupportBotFallback(message) {
  ...
  if (/görsel|görsel|foto|resim|image|galeri|düzenle|duzenle|model/.test(s)) {
...
function buildSupportBotFallbackClean(message) {
  ...
  if (/görsel|g\u00f6rsel|foto|resim|image|galeri|duzenle|d\u00fczenle|model/.test(s)) {
    return 'G\u00f6rsel \u00fcretim veya foto\u011fraf d\u00fczenleme i\u00e7in model, kalite modu ve oran se\u00e7ebilirsin. D\u00fczenleme modunda yaln\u0131zca edit destekli modeller \u00e7al\u0131\u015f\u0131r; hata al\u0131rsan se\u00e7ili modeli ve promptu destek talebine ekle.';
  }
