(function(){
  ...
  function isStableMenuOpen(){return !!document.querySelector('.chat-model-stable-menu-v474,.chat-model-stable-menu-v463') || document.body.classList.contains('chat-model-stable-open-v474') || document.body.classList.contains('chat-model-stable-open-v463')}
  ...
  var suppressTriggerClickUntilV474=0;
  window.openModelPicker=function(ev){return openModelMenuV474(ev)};
  window.toggleModelPicker=function(ev){
    if(isStableMenuOpen()){
      if(ev){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation()}
      closeMenus();
      suppressTriggerClickUntilV474=Date.now()+360;
      return false;
    }
    return openModelMenuV474(ev);
  };
  ...
  function handleChatPickerPress(e){
    ...
    if(t&&!t.closest('.chat-model-stable-menu-v474,.chat-model-stable-menu-v463')){
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();
      if(e.type==='click'&&Date.now()<suppressTriggerClickUntilV474)return false;
      if(isStableMenuOpen()){
        closeMenus();
        suppressTriggerClickUntilV474=Date.now()+360;
        return false;
      ...
      if(e.type!=='click')suppressTriggerClickUntilV474=Date.now()+360;
      return false;
    }
