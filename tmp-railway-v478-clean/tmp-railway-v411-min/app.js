...
function init(){
    ...
    function imagePickerLogoHtml(model, klass){
      ...
      return `<span class="${klass} img-provider-logo-v354" data-provider="${provider}" title="${label}"><span>${label}</span></span>`;
    }
  ...
  async function refreshLocalImageProvidersV392(){
    ...
    localProviderRefreshPromiseV416 = (async function(){
      try {
        ...
        localImageProviderStatusV392 = res.ok && data ? data : { comfyui:{ready:false}, fooocus:{ready:false}, errors:{status:'okunamad?'} };
      } catch (err) {
...
/* v416: provider logo images in the image picker can keep the browser load spinner alive.
   Use lightweight text marks for picker logos; showcase preview images stay unchanged. */
(function(){
  if(window.__froxyPickerLogoNoImgV416)return;
  window.__froxyPickerLogoNoImgV416=true;
  function markText(id){
    id=String(id||'').toLowerCase();
    if(id.startsWith('pollinations-'))return 'PO';
    if(id.startsWith('cf-'))return 'CF';
    if(id.startsWith('modal-'))return 'MO';
    if(id.includes('gemini')||id.includes('nano'))return 'GE';
    if(id.includes('openai')||id.includes('gpt'))return 'OA';
    if(id.includes('qwen'))return 'QW';
    if(id.includes('together')||id.includes('flux'))return 'FL';
    return 'AI';
  }
  function patch(){
    document.querySelectorAll('.img-model-picker-option-flag,.img-model-picker-flag').forEach(function(flag){
      ...
      flag.classList.add('img-provider-logo-v354','img-provider-logo-text-v416');
      flag.innerHTML='<span aria-hidden="true">'+markText(id)+'</span>';
    });
  }
...
(function(){
  ...
  async function refreshStatus410(){
    ...
    try{
      const ctrl=new AbortController();
      const timer=setTimeout(function(){try{ctrl.abort()}catch(e){}},2200);
      const res=await fetch('/api/health',{cache:'no-store',headers:{Accept:'application/json'},signal:ctrl.signal});
      clearTimeout(timer);
      const data=await res.json().catch(function(){return null});
  ...
  function patchPickerLabels410(){
    document.querySelectorAll('.img-model-picker-option').forEach(function(btn){
      ...
      if(flag)flag.innerHTML='<span>MO</span>';flag.classList.add('img-provider-logo-text-v416');
...
(function(){
  ...
  function logosFix(){
    document.querySelectorAll('.img-model-picker-option').forEach(btn=>{const f=fam(btn.getAttribute('data-value')||'');const flag=btn.querySelector('.img-model-picker-option-flag');if(flag){flag.classList.add('img-provider-logo-v354');flag.removeAttribute('style');flag.innerHTML='<span>'+esc((data[f]||data.generic)[0].split('/')[0].trim())+'</span>';flag.classList.add('img-provider-logo-text-v416');}});
    const flag=document.querySelector('.img-model-picker-trigger .img-model-picker-flag'); if(flag){const f=fam(model());flag.classList.add('img-provider-logo-v354');flag.removeAttribute('style');flag.innerHTML='<span>'+esc((data[f]||data.generic)[0].split('/')[0].trim())+'</span>';flag.classList.add('img-provider-logo-text-v416');}
  }
...
(function(){
  ...
  function fixModalOption(){
    ...
    document.querySelectorAll('.img-model-picker-option[data-value="modal-sdxl"],.img-model-picker-option[data-value="modal-local-sd"],.img-model-picker-option[data-value="modal-cloud-gpu"]').forEach(function(btn){
      const flag=btn.querySelector('.img-model-picker-option-flag');
      if(flag){flag.dataset.provider='modal';flag.innerHTML='<span>MO</span>';flag.classList.add('img-provider-logo-text-v416');}
      const strong=btn.querySelector('.img-model-picker-option-body strong');
...
/* v425: panel performance and image UI stability.
...
(function(){
  ...
  try{
    ...
    document.documentElement.classList.add('perf-v425');
  }catch(e){}
  ...
  function syncProviderStatus(status){
    ...
    if(sel){
      Array.from(sel.options||[]).forEach(function(opt){
        ...
        opt.disabled=!ready;
        opt.textContent=modelLabel(id,st)+(LOCAL_LABELS[id]?(ready?' - Ready':' - Kurulum gerekli'):(ready?' - Cloud GPU Ready':' - Cloud GPU gerekli'));
      });
      ['__froxyAllOptions','__froxyMasterOptions'].forEach(function(key){
        if(!Array.isArray(sel[key]))return;
        sel[key].forEach(function(group){
          (group.options||[]).forEach(function(opt){
            ...
            const ready=providerReady(id,st);
            opt.disabled=!ready;
            opt.text=modelLabel(id,st)+(LOCAL_LABELS[id]?(ready?' - Ready':' - Kurulum gerekli'):(ready?' - Cloud GPU Ready':' - Cloud GPU gerekli'));
          });
        });
      });
    }
    document.querySelectorAll('.img-model-picker-option').forEach(function(btn){
      ...
      const strong=btn.querySelector('.img-model-picker-option-body strong');
      if(strong)strong.textContent=modelLabel(id,st)+(LOCAL_LABELS[id]?(ready?' - Ready':' - Kurulum gerekli'):(ready?' - Cloud GPU Ready':' - Cloud GPU gerekli'));
      const flag=btn.querySelector('.img-model-picker-option-flag');
      if(flag){
        flag.classList.add('img-provider-logo-text-v416');
        flag.innerHTML='<span>'+(LOCAL_LABELS[id]?'LO':'MO')+'</span>';
      }
    });
    const selected=String(document.getElementById('img-model')?.value||'').toLowerCase();
    if(LOCAL_LABELS[selected]||MODAL_LABELS[selected]){
      const title=document.querySelector('.img-model-picker-trigger .img-model-picker-info strong');
      if(title)title.textContent=modelLabel(selected,st);
    }
  }
