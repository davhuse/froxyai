...
app.post('/api/forgot-password', authLimiter, (req, res) => {
  ...
  try {
    ...
    if(user) {
      ...
      const payload = { success: true, message: 'Åifre sıfırlama bağlantısı e-postanıza gönderildi.' };
      if (process.env.NODE_ENV !== 'production') payload.dev_token = token;
      res.json(payload);
    } else {
      res.json({ success: true, message: 'Åifre sıfırlama bağlantısı e-postanıza gönderildi.' });
    }
  } catch(e) {
    res.status(500).json({error: e.message});
  }
...
app.post('/api/reset-password', authLimiter, (req, res) => {
  ...
  if(password.length < 6) return res.status(400).json({error: 'Åifre en az 6 karakter olmalı'});
  try {
    ...
    res.json({ success: true, message: 'Åifreniz başarıyla sıfırlandı.' });
  } catch(e) {
    res.status(500).json({error: e.message});
  }
});
