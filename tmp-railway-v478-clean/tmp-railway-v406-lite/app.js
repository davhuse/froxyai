...
function selectWorkingImageModel(model){
  const failed=String(model||'');
  const sel=document.getElementById('img-model');
  const candidates=[
    'pollinations-flux',
    'pollinations-zimage',
  ...
  if(sel){
    ...
    const usable=options
      .filter(o=>!o.disabled&&imageModelCanGenerate(o.value)&&o.value!==failed)
      .map(o=>o.value);
...
document.addEventListener('click', function(e){
  ...
  retryWithWorkingImageModel(failedModel);
  return false;
}, true);
...
(function initImgModelPicker(){
  ...
  const NAME_MAP = {
    ...
    'pollinations-nanobanana': { brand: 'NB', desc: 'NanoBanana - Pollinations paid', cost: 10, costType: 'free' },
    'pollinations-nanobanana-2': { brand: 'N2', desc: 'NanoBanana 2 - Pollinations paid', cost: 10, costType: 'free' },
    'pollinations-seedream': { brand: 'SD', desc: 'Seedream 4.0 - Pollinations paid', cost: 10, costType: 'free' },
    'pollinations-seedream5': { brand: 'S5', desc: 'Seedream 5 Lite - Pollinations paid', cost: 10, costType: 'free' },
...
function init(){
    ...
    function imagePickerLogoHtml(model, klass){
      ...
      const assets = {
        ...
        modal:'provider-logos/generic.svg',
        pollinations:'provider-logos/pollinations.svg',
        local:'provider-logos/generic.svg',
        generic:'provider-logos/generic.svg'
      };
      const label = ({openai:'OpenAI',gemini:'Gemini',together:'Together',imagegpt:'ImageGPT',qwen:'Qwen',cloudflare:'Cloudflare',modal:'Modal GPU',pollinations:'Pollinations',local:'Local',generic:'AI'})[provider] || 'AI';
...
(function initChatToneAndImageProviderRepairV387(){
  ...
  const CLOUD_GPU_IMAGE_MODELS_V400 = new Set(['modal-sdxl']);
  ...
  function localImageModelReadyV392(id){
    ...
    if (id === 'modal-sdxl') return !!localImageProviderStatusV392.modal?.ready;
    return false;
  }
      ...
      sel.__froxyMasterOptions = sel.__froxyMasterOptions.map(function(grp){
        return {
          ...
          options: (grp.options || []).filter(function(opt){
            return opt.value !== 'comfyui-local' && opt.value !== 'fooocus-local' && opt.value !== 'a1111-local' && opt.value !== 'forge-local' && opt.value !== 'swarmui-local' && opt.value !== 'modal-sdxl';
          })
        };
      }).filter(function(grp){
  ...
  async function refreshLocalImageProvidersV392(){
    try {
      const res = await fetch('/api/local-providers/status', { headers: { 'Accept': 'application/json' } });
      const data = await res.json().catch(function(){ return null; });
      localImageProviderStatusV392 = res.ok && data ? data : { comfyui:{ready:false}, fooocus:{ready:false}, errors:{status:'okunamadı'} };
    } catch (err) {
  ...
  async function ensureCloudflareImageOptionsV400(sel){
    ...
    try {
      ...
      const data = await res.json().catch(function(){ return null; });
      if (!res.ok || !data?.imageProviders?.cloudflare) return;
    } catch (_) {
      return;
    }
    const items = [
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    setTimeout(refreshLocalImageProvidersV392, 40);
  ...
  [120, 700, 1600, 3200, 6200, 11000].forEach(function(ms){
    setTimeout(function(){
      const sel = document.getElementById('img-model');
      if (!sel) return;
      refreshLocalImageProvidersV392();
  ...
  document.addEventListener('click', function(e){
    ...
    setTimeout(refreshLocalImageProvidersV392, 20);
  }, true);
})();
...
(function(){
  ...
  async function refreshStatus410(){
    try{
      const res=await fetch('/api/local-providers/status',{cache:'no-store',headers:{Accept:'application/json'}});
      const data=await res.json().catch(function(){return null});
      if(res.ok&&data)window.__froxyLocalProviderStatusV410=data;
    }catch(e){}
    try{
      const res=await fetch('/api/health',{cache:'no-store',headers:{Accept:'application/json'}});
      const data=await res.json().catch(function(){return null});
      if(res.ok&&data)window.__froxyHealthV410=data;
    }catch(e){}
  ...
  function ensureImageModelOptions410(){
    ...
    upsertGroup410(sel,'Free / Cloudflare',CLOUDFLARE_ITEMS_V410,{suffix:function(id){
      ...
      if(!st)return '';
      return st.ready?' - Ready':' - gecici sorunlu';
    }});
    ...
    upsertGroup410(sel,'Local Free',LOCAL_ITEMS_V410.concat([['modal-sdxl','Modal GPU SDXL']]),{
      suffix:function(id){return isModal410(id)?modal.suffix:localStatus410(id).suffix},
      disabled:function(id){return isModal410(id)?!modal.ready:!localStatus410(id).ready}
    });
  ...
  function patchPickerLabels410(){
    document.querySelectorAll('.img-model-picker-option').forEach(function(btn){
      ...
      if(flag)flag.innerHTML='<img src="/provider-logos/generic.svg" alt="" loading="lazy" decoding="async"><span>Modal GPU</span>';
  ...
  window.selectWorkingImageModel=function selectWorkingImageModelV410(failedModel){
    ensureImageModelOptions410();
    ...
    const usable=Array.from(sel.options||[]).filter(function(o){
      ...
      return !o.disabled&&id!==failed&&imageModelCanGenerate(id)&&isFreeAuto410(id);
    }).map(function(o){return o.value});
    const picked=FREE_WORKING_ORDER_V410.find(function(id){return usable.includes(id)})||usable[0]||'pollinations-flux';
    setSelected410(picked,'working-v410');
    if(typeof msg==='function')msg('Calisan ucretsiz/local model secildi: '+(getImageModelLabel(picked)||picked),'ok');
    return picked;
  };
  try{selectWorkingImageModel=window.selectWorkingImageModel}catch(e){}
  window.retryWithWorkingImageModel=function retryWithWorkingImageModelV410(failedModel){
    const prompt=document.getElementById('img-prompt');
    if(lastImgPrompt&&prompt)prompt.value=lastImgPrompt;
    const picked=window.selectWorkingImageModel(failedModel);
    lastImgModel=picked;
  ...
  window.renderImageErrorCard=renderImageErrorCard=function(resEl,prompt,model,provider,reason){
    ...
    resEl.innerHTML='<div class="image-result-card"><div class="image-error-card-pro"><div class="image-error-top"><div class="image-error-icon">!</div><div class="image-error-meta"><strong>'+esc410(getImageModelLabel(model)||model||'Gorsel modeli')+' <span class="image-error-provider">('+esc410(provider||imageProviderForModel(model)||'AI')+')</span></strong><span>'+esc410(String(prompt||'').slice(0,80))+'</span></div></div><p class="image-error-reason">'+esc410(reason||'Secili model su an yanit vermedi.')+'</p><p>Bu model yanit vermedi. Calisan ucretsiz/local modele gecebilir, modeli elle degistirebilir veya ayni modeli yeniden deneyebilirsin.</p><div class="image-error-actions"><button type="button" class="btn btn-small btn-primary" data-image-action="working-model" data-failed-model="'+failed+'">'+figIcon('sparkles','inline')+' Calisan Modele Gec</button><button type="button" class="btn btn-small" data-image-action="change-model">'+figIcon('settings','inline')+' Model Degistir</button><button type="button" class="btn btn-small" onclick="regenImage()">'+figIcon('refresh','inline')+' Yeniden Dene</button><button type="button" class="btn btn-small" onclick="editImagePrompt()">'+figIcon('edit','inline')+' Promptu Duzenle</button></div></div></div>';
  ...
  document.addEventListener('click',function(e){
    ...
    window.retryWithWorkingImageModel(actionBtn.getAttribute('data-failed-model')||lastImgModel||'');
    return false;
  },true);
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    [80,1600,5200,11000].forEach(function(ms){setTimeout(refreshStatus410,ms)});
  });
  if(document.readyState!=='loading'){
    ...
    [80,1500,5000].forEach(function(ms){setTimeout(refreshStatus410,ms)});
  }
  setTimeout(refreshStatus410,0);
  refreshStatus410().catch(function(){});
})();
...
(function(){
  ...
  function setModalReadyInSelect(ready){
    ...
    Array.from(sel.options||[]).forEach(function(opt){
      if(opt.value!=='modal-sdxl')return;
      opt.disabled=!ready;
      opt.textContent=text;
    });
  ...
  function fixModalOption(){
    const sel=document.getElementById('img-model');
    if(!sel)return;
    const modalOption=Array.from(sel.options||[]).find(function(opt){return opt.value==='modal-sdxl'});
    const modalReady=modalOption ? !modalOption.disabled : false;
    Array.from(sel.options||[]).forEach(function(opt){
      if(opt.value!=='modal-sdxl')return;
      opt.textContent=modalReady?'Modal GPU SDXL - Cloud GPU Ready':'Modal GPU SDXL - Cloud GPU gerekli';
    ...
    document.querySelectorAll('.img-model-picker-option[data-value="modal-sdxl"]').forEach(function(btn){
      const flag=btn.querySelector('.img-model-picker-option-flag');
      if(flag){flag.dataset.provider='modal';flag.innerHTML='<img src="/provider-logos/generic.svg" alt="" loading="lazy" decoding="async"><span>Modal GPU</span>';}
      const strong=btn.querySelector('.img-model-picker-option-body strong');
      if(strong)strong.textContent=modalReady?'SDXL - Cloud GPU Ready':'SDXL - Cloud GPU gerekli';
      if(modalReady){
        btn.disabled=false;
    ...
    if(current()==='modal-sdxl'){
      ...
      if(flag){flag.dataset.provider='modal';flag.innerHTML='<img src="/provider-logos/generic.svg" alt="" decoding="async"><span>Modal GPU</span>';}
