...
function sendSeoIndex(req, res) {
  ...
  fs.readFile(path.join(staticRoot, 'index.html'), 'utf8', (err, html) => {
    ...
    } else if (isPerformanceAuditRequest(req) && /^\/(?:sohbet|chat|gorsel|gorsel-uret|promptlar|ai-araclar|araclar|galeri|destek|admin)$/i.test(key)) {
...
const MODAL_IMAGE_MODEL_ALIASES = new Set(['modal-sdxl', 'modal-local-sd', 'modal-cloud-gpu']);
...
const POLLINATIONS_IMAGE_GAP_MS = Number(fromEnv('POLLINATIONS_IMAGE_GAP_MS', '1800')) || 1800;
...
function isPerformanceAuditRequest(req) {
  ...
  return q === '1' || /Chrome-Lighthouse|Google Page Speed Insights|PageSpeed|Lighthouse|HeadlessChrome/i.test(ua) || /prefetch/i.test(purpose);
}
...
async function getCloudflareImageGenerationStatus(timeoutMs = 30000) {
  ...
  try {
    ...
    if (fs.existsSync(localPath)) {
      ...
      return { ready: true, detail: `generation ok (${ext}, ${buffer.length} bytes)`, url: out.url };
    }
...
async function getLocalProviderStatus() {
  ...
  return {
    ...
    modal: { ready: modalReady, baseUrl: MODAL_IMAGE_ENDPOINT || '', checkedPath: modalReady ? 'configured' : null, models: ['modal-sdxl', 'modal-local-sd'] },
    errors
  };
}
          ...
          for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
              ...
              if (!response.ok && ct.includes('application/json')) {
                ...
                throw new Error(`Pollinations HTTP ${response.status}: ${errMessage}`);
              }
              if (!response.ok) throw new Error(`Pollinations HTTP ${response.status}`);
...
function getModelCreditCost(model, provider) {
  ...
  if (m === 'modal-sdxl' || m === 'modal-local-sd' || m === 'modal-cloud-gpu') return 5;
...
app.post('/api/deduct-image-credit', authMiddleware, (req, res) => {
  ...
  if (m === 'imagegpt-free') {
    ...
    cost = 1;
  } else if (m === 'modal-sdxl' || m === 'modal-local-sd' || m === 'modal-cloud-gpu') {
    cost = 5;
