...
/* v435: final authority patch for backend admin auth, picker lock, safe provider logos and lighter image UI. */
(function(){
  if(window.__froxyV435FinalAuthority)return;
  window.__froxyV435FinalAuthority=true;

  function safe(fn){try{return fn&&fn()}catch(e){}}
  function norm(v){return String(v||'').trim().toLowerCase()}
  function fixText(v){
    if(typeof fixCommonTurkishMojibake==='function')return fixCommonTurkishMojibake(String(v||''));
    return String(v||'');
  }
  function routeIs(path){
    return String(location.pathname||'').toLowerCase()===String(path||'').toLowerCase();
  }
  function getTokenV435(){
    return (typeof authToken!=='undefined'&&authToken) || localStorage.getItem('saas_token') || '';
  }
  function getAuthUserV435(){
    return (typeof authUser!=='undefined'&&authUser) || window.authUser || null;
  }
  function getLocalUserV435(){
    return (typeof user!=='undefined'&&user) || window.user || null;
  }
  function isBackendAdminV435(){
    const a=getAuthUserV435();
    return !!(getTokenV435() && a && (a.is_admin || a.isAdmin));
  }
  function providerForImageModelV435(id){
    const m=norm(id);
    if(m.startsWith('pollinations-'))return 'pollinations';
    if(m.startsWith('cf-'))return 'cloudflare';
    if(m.startsWith('modal-'))return 'modal';
    if(/a1111|swarmui|fooocus|forge|comfyui/.test(m))return 'generic';
    ...
    if(m.includes('qwen'))return 'qwen';
    return safe(()=>imageProviderForModel(id)) || 'generic';
  }
  function providerIconHtmlV435(provider){
    const icons={
      openai:'<svg viewBox="0 0 24 24" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3.7c1.75-1.02 3.95.17 4.06 2.18 1.95-.08 3.48 1.77 2.84 3.63 1.62 1.02 1.54 3.44-.15 4.32.44 1.88-1.25 3.52-3.08 3.18-.72 1.82-3.1 2.28-4.4.82-1.58 1.22-3.9.28-4.28-1.7-1.95-.1-3.25-2.12-2.4-3.87-1.42-1.25-1.05-3.62.7-4.36-.2-1.98 1.7-3.48 3.55-2.8C9.54 3.45 11.2 3.06 12 3.7Z"/><path d="M8.85 5.1 15.2 8.8v7.3"/><path d="m16.05 5.9-6.38 3.7-3.28 5.68"/><path d="m18.9 9.5-6.36 3.68-6.28-.04"/><path d="m15.68 17.02-3.2-5.72-6.3-3.62"/><path d="m7.02 16.1 6.34-3.68 3.18-5.52"/></g></svg>',
      gemini:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 2.8c1.08 4.62 3.02 7.1 7.2 8.96-4.18 1.86-6.12 4.34-7.2 8.96-1.08-4.62-3.02-7.1-7.2-8.96 4.18-1.86 6.12-4.34 7.2-8.96Z"/></svg>',
      cloudflare:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M8.2 17.3h9.9a3.1 3.1 0 0 0 .5-6.16A5.7 5.7 0 0 0 7.7 9.35a4 4 0 0 0 .5 7.95Z"/><path fill="currentColor" opacity=".55" d="M17.15 10.15c1.2.32 2.12 1.28 2.38 2.48l1.62-.05a4.42 4.42 0 0 0-4.5-3.72c-.84 0-1.6.22-2.25.6.98.02 1.9.24 2.75.69Z"/></svg>',
      pollinations:'<svg viewBox="0 0 24 24" aria-hidden="true"><g fill="currentColor"><circle cx="12" cy="12" r="2.2"/><ellipse cx="12" cy="5.5" rx="2" ry="3.2"/><ellipse cx="12" cy="18.5" rx="2" ry="3.2"/><ellipse cx="5.5" cy="12" rx="3.2" ry="2"/><ellipse cx="18.5" cy="12" rx="3.2" ry="2"/></g></svg>',
      together:'<svg viewBox="0 0 24 24" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="7.5" cy="8" r="3.2"/><circle cx="16.5" cy="16" r="3.2"/><path d="M10 10.2 14 13.8"/><path d="M16.5 5v5.2M7.5 13.8V19"/></g></svg>',
      qwen:'<svg viewBox="0 0 24 24" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="1.85" stroke-linecap="round" stroke-linejoin="round"><circle cx="11.2" cy="11.1" r="6.2"/><path d="m15.7 15.5 3.6 3.6"/><path d="M8.4 9.8 12 7.6l3.6 2.2v4.3L12 16.3l-3.6-2.2V9.8Z"/></g></svg>',
      modal:'<svg viewBox="0 0 24 24" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 18V7l7-3 7 3v11l-7 3z"/><path d="M12 4v17"/><path d="M5 7l7 4 7-4"/></g></svg>',
      generic:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 3.2c.82 3.5 2.28 5.38 5.45 6.78-3.17 1.42-4.63 3.3-5.45 6.82-.82-3.52-2.28-5.4-5.45-6.82C9.72 8.58 11.18 6.7 12 3.2Z"/></svg>'
    };
    return icons[provider]||icons.generic;
  }
  const prevProviderIconSvgV435=window.providerBrandIconSvg||safe(()=>providerBrandIconSvg);
  if(typeof prevProviderIconSvgV435==='function'&&!prevProviderIconSvgV435.__froxyV435Wrapped){
    const next=function(key){
      return providerIconHtmlV435(norm(key)||'generic');
    };
    next.__froxyV435Wrapped=true;
    window.providerBrandIconSvg=providerBrandIconSvg=next;
  }
  function syncSelectedModelV435(reason){
    const sel=document.getElementById('img-model');
    if(!sel)return '';
    const current=sel.value||window.__froxyImageModelLock||window.__froxyLastManualImageModel||localStorage.getItem('ap_img_last_manual_model')||'';
    if(current&&Array.from(sel.options||[]).some(o=>o.value===current&&!o.disabled&&!o.hidden)){
      sel.value=current;
      window.__froxyImageModelLock=current;
      window.__froxyLastManualImageModel=current;
      try{localStorage.setItem('ap_img_last_manual_model',current)}catch(e){}
      safe(()=>typeof rememberImageModelChoice==='function'&&rememberImageModelChoice(current,reason||'v435-lock'));
      return current;
    }
    const first=Array.from(sel.options||[]).find(o=>!o.disabled&&!o.hidden);
    if(first){
      sel.value=first.value;
      window.__froxyImageModelLock=first.value;
      window.__froxyLastManualImageModel=first.value;
      try{localStorage.setItem('ap_img_last_manual_model',first.value)}catch(e){}
      return first.value;
    }
    return '';
  }
  function paintTriggerV435(){
    const sel=document.getElementById('img-model');
    const trigger=document.querySelector('.img-model-picker-trigger');
    if(!sel||!trigger)return;
    const value=syncSelectedModelV435('paint-v435');
    const opt=Array.from(sel.options||[]).find(o=>o.value===value);
    const title=trigger.querySelector('.img-model-picker-info strong');
    const sub=trigger.querySelector('.img-model-picker-info span');
    const flag=trigger.querySelector('.img-model-picker-flag,.img-provider-logo-v354,.img-model-stable-logo');
    if(title)title.textContent=fixText(opt?.textContent||getImageModelLabel?.(value)||value||'Model');
    if(sub)sub.textContent='Seçili model: '+value;
    if(flag){
      flag.innerHTML=providerIconHtmlV435(providerForImageModelV435(value));
      flag.setAttribute('data-provider',providerForImageModelV435(value));
    }
  }
  function patchStablePickerLogosV435(root){
    (root||document).querySelectorAll('.img-model-stable-option[data-value], .img-model-picker-option[data-value]').forEach(btn=>{
      const value=btn.getAttribute('data-value')||'';
      const logo=btn.querySelector('.img-model-stable-logo,.img-model-picker-option-flag,.img-provider-logo-v354');
      if(logo){
        logo.innerHTML=providerIconHtmlV435(providerForImageModelV435(value));
        logo.setAttribute('data-provider',providerForImageModelV435(value));
      }
      const body=btn.querySelector('.img-model-stable-option-body strong,.img-model-picker-option-body strong');
      if(body)body.textContent=fixText(body.textContent);
    });
  }
  function openStablePickerV435(){
    const fn=window.openImageModelPickerTrigger;
    if(typeof fn==='function')return fn();
    const trigger=document.querySelector('.img-model-picker-trigger');
    if(trigger)trigger.click();
  }
  function selectWorkingModelV435(failedModel, failedReason){
    if(typeof window.selectWorkingImageModel==='function'){
      const before=document.getElementById('img-model')?.value||'';
      const next=window.selectWorkingImageModel(failedModel, failedReason);
      const after=document.getElementById('img-model')?.value||'';
      if(next && norm(next)!==norm(failedModel))return next;
      if(after && norm(after)!==norm(failedModel))return after;
      if(before && norm(before)!==norm(failedModel))return before;
    }
    return '';
  }
  function interceptImageErrorButtonsV435(){
    if(window.__froxyImageErrorButtonsV435)return;
    window.__froxyImageErrorButtonsV435=true;
    document.addEventListener('click',function(e){
      const btn=e.target&&e.target.closest&&e.target.closest('.image-error-actions [data-image-action], .image-error-actions button');
      if(!btn)return;
      const action=btn.getAttribute('data-image-action')||'';
      const txt=norm(btn.textContent);
      if(action==='change-model'||txt.includes('model değiştir')||txt.includes('model degistir')){
        e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();
        openStablePickerV435();
        return false;
      }
      if(action==='working-model'||txt.includes('çalışan modele geç')||txt.includes('calisan modele gec')){
        e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();
        const failedModel=btn.getAttribute('data-failed-model')||window.lastImgModel||document.getElementById('img-model')?.value||'';
        const failedReason=btn.getAttribute('data-failed-reason')||btn.closest('.image-error-card-pro,.image-error-card')?.querySelector('.image-error-reason')?.textContent||'';
        const next=selectWorkingModelV435(failedModel, failedReason);
        if(next && norm(next)!==norm(failedModel)){
          paintTriggerV435();
          if(typeof msg==='function')msg('Çalışan model seçildi: '+(getImageModelLabel?.(next)||next),'ok');
        }
        return false;
      }
    },true);
  }
  function patchAdminLoginFlowV435(){
    const prevDoLogin=window.doLogin||safe(()=>doLogin);
    if(typeof prevDoLogin==='function'&&!prevDoLogin.__froxyV435Wrapped){
      const next=function(){
        if(!allowLocalFallback() && typeof doAuth==='function'){
          return doAuth('login');
        }
        return prevDoLogin.apply(this,arguments);
      };
      next.__froxyV435Wrapped=true;
      window.doLogin=doLogin=next;
    }

    const prevComplete=window.completeAuthTransition||safe(()=>completeAuthTransition);
    if(typeof prevComplete==='function'&&!prevComplete.__froxyV435Wrapped){
      const next=function(target){
        if(String(target||'').toLowerCase()==='admin' && !isBackendAdminV435()){
          target='chat';
        }
        return prevComplete.call(this,target);
      };
      next.__froxyV435Wrapped=true;
      window.completeAuthTransition=completeAuthTransition=next;
    }
  }
  async function repairAdminV435(){
    const token=getTokenV435();
    if(!token){
    ...
    try{
      ...
      if(typeof authUser!=='undefined')authUser=data.user||data;
      if(typeof window.authUser!=='undefined')window.authUser=data.user||data;
      if(data.user){
        localStorage.setItem('saas_user',JSON.stringify(data.user));
        if(typeof user!=='undefined'){
          user={...(user||{}),...data.user,isAdmin:!!data.user.is_admin};
          safe(()=>LS.set('ap_user',user));
        }
      }
      if(typeof admin!=='undefined')admin=!!data.user?.is_admin;
      return {ok:!!data.user?.is_admin,status:res.status,data};
    }catch(e){
      return {ok:false,status:0,error:e};
    }
  }
  function cleanAdminUiV435(){
    if(!routeIs('/admin'))return;
    const root=document.getElementById('v-admin');
    if(!root)return;
    const stateEl=root.querySelector('#admin-api-state');
    if(stateEl && !isBackendAdminV435()){
      safe(()=>typeof adminSetApiState==='function'&&adminSetApiState('fallback','Admin backend oturumu gerekli'));
    }
    const apiStateText=norm(stateEl?.textContent||'');
    if(isBackendAdminV435()){
      const ghost=root.querySelector('#admin-auth-clean-v434');
      if(ghost)ghost.remove();
      return;
    }
    if(apiStateText.includes('backend bağlı')||apiStateText.includes('backend bagli')){
      safe(()=>typeof adminSetApiState==='function'&&adminSetApiState('fallback','Admin backend oturumu gerekli'));
    }
  }
  function lightenImageRouteV435(){
    const onImage=/\/gorsel/i.test(location.pathname||'')||document.querySelector('#ptab-img.on,#v-img.on');
    if(!onImage)return;
    document.querySelectorAll('.gallery-item,.growth-gallery-card,.image-result-card,.img-gen-card,.img-model-picker-trigger').forEach(el=>{
      el.style.willChange='auto';
      el.style.backdropFilter='none';
      el.style.webkitBackdropFilter='none';
    });
    document.querySelectorAll('.gallery-item img,.growth-gallery-card img,.pro-image-strip img,.image-result-card img').forEach(img=>{
      img.loading='lazy';
      img.decoding='async';
    });
  }
  function runV435(){
    patchAdminLoginFlowV435();
    syncSelectedModelV435('run-v435');
    paintTriggerV435();
    patchStablePickerLogosV435(document);
    cleanAdminUiV435();
    lightenImageRouteV435();
  }

  interceptImageErrorButtonsV435();

  const prevRenderPicker=window.__renderImgModelPicker;
  if(typeof prevRenderPicker==='function'&&!prevRenderPicker.__froxyV435Wrapped){
    const next=function(){
      const out=prevRenderPicker.apply(this,arguments);
      setTimeout(function(){
        syncSelectedModelV435('picker-render-v435');
        patchStablePickerLogosV435(document);
        paintTriggerV435();
      },40);
      return out;
    ...
    next.__froxyV435Wrapped=true;
    window.__renderImgModelPicker=next;
  }

  const prevGenImage=window.genImage||safe(()=>genImage);
  if(typeof prevGenImage==='function'&&!prevGenImage.__froxyV435Wrapped){
    const next=function(){
      syncSelectedModelV435('before-generate-v435');
      return prevGenImage.apply(this,arguments);
    };
    next.__froxyV435Wrapped=true;
    window.genImage=genImage=next;
  }

  document.addEventListener('change',function(e){
    if(e.target&&e.target.id==='img-model'){
      window.__froxyImageModelLock=e.target.value;
      window.__froxyLastManualImageModel=e.target.value;
      try{localStorage.setItem('ap_img_last_manual_model',e.target.value)}catch(err){}
      setTimeout(runV435,30);
    }
  },true);

  document.addEventListener('click',function(e){
    const stableBtn=e.target&&e.target.closest&&e.target.closest('.img-model-stable-option[data-value],.img-model-picker-option[data-value]');
    if(stableBtn){
      const value=stableBtn.getAttribute('data-value')||'';
      if(value){
        const sel=document.getElementById('img-model');
        if(sel && Array.from(sel.options||[]).some(o=>o.value===value&&!o.disabled&&!o.hidden)){
          e.preventDefault();
          e.stopPropagation();
          if(e.stopImmediatePropagation)e.stopImmediatePropagation();
          sel.value=value;
          window.__froxyImageModelLock=value;
          window.__froxyLastManualImageModel=value;
          try{localStorage.setItem('ap_img_last_manual_model',value)}catch(err){}
          safe(()=>typeof rememberImageModelChoice==='function'&&rememberImageModelChoice(value,'stable-click-v435'));
          sel.dispatchEvent(new Event('change',{bubbles:true}));
          paintTriggerV435();
          safe(()=>document.querySelectorAll('.img-model-stable-menu-v434').forEach(el=>el.remove()));
          document.body.classList.remove('img-model-stable-open-v434');
          return false;
        }
      }
  ...
  const prevEnsureAdmin=window.ensureAdminShell||safe(()=>ensureAdminShell);
  if(typeof prevEnsureAdmin==='function'&&!prevEnsureAdmin.__froxyV435Wrapped){
    const next=function(){
      const out=prevEnsureAdmin.apply(this,arguments);
      setTimeout(async function(){
        if(routeIs('/admin')){
          await repairAdminV435();
          cleanAdminUiV435();
        }
      },20);
      return out;
    ...
    next.__froxyV435Wrapped=true;
    window.ensureAdminShell=ensureAdminShell=next;
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',runV435,{once:true});
  else runV435();
  [80,260,700,1600,3200].forEach(ms=>setTimeout(runV435,ms));
})();
