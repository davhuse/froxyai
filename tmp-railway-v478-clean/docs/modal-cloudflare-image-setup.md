...
You can also map extra Modal aliases through env:

```env
MODAL_DEFAULT_MODEL_ID=stabilityai/sdxl-turbo
MODAL_DREAMSHAPER_MODEL_ID=Lykon/dreamshaper-xl-lightning
MODAL_REALISTICVISION_MODEL_ID=SG161222/RealVisXL_V5.0_Lightning
MODAL_A1111_MODEL_ID=stabilityai/sdxl-turbo
```

## Pollinations image pool

Use one primary key or a small backup pool:

```env
POLLINATIONS_API_KEY=sk_...
POLLINATIONS_API_KEYS=sk_1,sk_2
POLLINATIONS_IMAGE_MAX_CONCURRENT=1
POLLINATIONS_IMAGE_GAP_MS=4500
```

If a Pollinations model starts failing after a few generations, the app records a cooldown and the status panel should show the reason instead of silently retrying the same model.
