...
function publicBaseUrl(req) {
  ...
  if (configured) {
    ...
    if (/froxyai-production\.up\.railway\.app/i.test(normalized)) return 'https://froxyai.com';
    return normalized;
  }
  const proto = (req.get('x-forwarded-proto') || req.protocol || 'https').split(',')[0].trim();
  const host = req.get('host');
  if (/froxyai-production\.up\.railway\.app/i.test(host || '')) return 'https://froxyai.com';
  return `${proto}://${host}`;
}
