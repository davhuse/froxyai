...
function selectWorkingImageModel(model){
  ...
  if(sel){
    ...
    const usable=options
      ...
      .map(o=>o.value);
    const freeUsable=usable.filter(id=>/^(pollinations-|comfyui|fooocus|imagegpt)/.test(id));
...
function imageQualityRecommendedModel(mode){
  ...
  const map=imageWorkflowMode==='edit'?{
    ...
    cheap:['pollinations-flux','pollinations-zimage','fooocus-local','comfyui-local','together-flux-schnell'],
    fast:['pollinations-flux','fooocus-local','comfyui-local','together-flux-schnell'],
    quality:['pollinations-flux','pollinations-zimage','together-flux-kontext-pro','together-flux2-pro','together-flux-schnell'],
    premium:['together-flux2-pro','together-flux-kontext-pro','pollinations-flux','together-flux-schnell']
  };
...
async function genImage(){
  ...
  try {
    ...
    const allowPaidFallback = model === 'auto-quality';
...
(function initChatToneAndImageProviderRepairV387(){
  ...
  function ensurePollinationsImageOptionsV389(sel){
    ...
    const items = [
      ...
      ['pollinations-zimage', 'Z-Image Turbo - Pollinations (yava\u015f)']
    ];
    const group = document.createElement('optgroup');
    group.label = 'FREE / POLLINATIONS';
    items.forEach(function(item){
      if (sel.querySelector('option[value="' + item[0] + '"]')) return;
      const opt = document.createElement('option');
      opt.value = item[0];
    ...
    if (typeof syncImageModelOptionsForMode === 'function') syncImageModelOptionsForMode(true);
    if (!imageModelCanGenerate(sel.value)) {
      const fallback = Array.from(sel.options).find(function(o){ return o.value === 'pollinations-flux' && !o.disabled; }) || Array.from(sel.options).find(function(o){ return imageModelCanGenerate(o.value) && !o.disabled; });
      if (fallback) {
  ...
  document.addEventListener('click', function(e){
    ...
    if(e.target && e.target.closest && e.target.closest('.img-model-picker-trigger,.img-model-picker,.image-error-actions,#img-model'))setTimeout(repairImageModelSelect, 160);
  }, true);
})();
...
(function(){
  ...
  function fixModalShowcase(){
    ...
    host.innerHTML='<div class="img-showcase-head"><div><span>Model Yetene&#287;i</span><strong>Modal GPU SDXL</strong><p>Modal uzerindeki cloud GPU hatti. PC kapali olsa da Froxy API uzerinden uretim yapar ve Together fallback kullanmaz.</p></div><em>Yeni g&#246;rsel &#252;retimi</em></div><div class="img-showcase-tags"><b>Cloud GPU</b><b>PC kapal&#305; &#231;al&#305;&#351;&#305;r</b><b>SDXL Turbo</b><b>D&#252;&#351;&#252;k maliyet</b></div><div class="img-showcase-preview-grid"><button type="button" class="img-showcase-preview" data-showcase-index="0"><img src="/assets/model-showcase/showcase-cloudflare-1.png" alt="Modal GPU ornek kullanim 1" loading="lazy" decoding="async"><i>&#214;rnek kullan&#305;m</i><span>Sinematik urun fotografi, temiz isik, gercekci kompozisyon</span></button><button type="button" class="img-showcase-preview" data-showcase-index="1"><img src="/assets/model-showcase/showcase-flux-1.jpg" alt="Modal GPU ornek kullanim 2" loading="lazy" decoding="async"><i>&#214;rnek kullan&#305;m</i><span>Futuristik sehir manzarasi, gun batimi, net detay</span></button></div>';
