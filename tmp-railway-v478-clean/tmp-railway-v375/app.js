...
async function sendMsg(){
  ...
  try{
    ...
    try{
      ...
      saveChats();renderMsgs({stickToBottom:true});
      await chargeSuccessfulUse(requestedModel,requestedProvider,'chat',requestedCost);
      if(typeof msg==='function')msg('?stek i?lendi.','ok');
    }catch(fallbackErr){
      botMsg.content='Bu model ?u an cevap veremedi. Farkl? bir model se?ip tekrar deneyebilir veya biraz sonra yeniden g?nderebilirsin.';
...
function imageModelCanGenerate(model){
  return VERIFIED_IMAGE_GENERATE_MODELS.has(String(model||''));
}
...
function imageQualityRecommendedModel(mode){
  ...
  const map=imageWorkflowMode==='edit'?{
    ...
    cheap:['imagegpt-free','flux','together-flux-schnell'],
    fast:['imagegpt-free','flux','together-flux-schnell'],
    quality:['together-qwen-image','together-imagen4-fast','together-flux2-dev','imagegpt-free','flux','together-flux-schnell'],
    premium:['together-imagen4-fast','together-flux2-dev','together-qwen-image','imagegpt-free','flux','together-flux-schnell']
  };
  return pick(map[mode]||map.cheap)||available[0]||'together-flux-schnell';
}
