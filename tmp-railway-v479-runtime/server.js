...
const SENTRY_RELEASE = process.env.SENTRY_RELEASE || process.env.RAILWAY_GIT_COMMIT_SHA || 'v479';
...
function scrubSentryObject(value, depth = 0, seen = new WeakSet()) {
  ...
  seen.add(value);
  if (Array.isArray(value)) return value.slice(0, 30).map(item => scrubSentryObject(item, depth + 1, seen));
  const out = {};
  for (const key of Object.keys(value).slice(0, 100)) {
    out[key] = SENTRY_SENSITIVE_KEY_RE.test(key) ? '[redacted]' : scrubSentryObject(value[key], depth + 1, seen);
  }
  return out;
}

function scrubSentryUrl(url) {
  try {
    const u = new URL(String(url || ''), 'https://www.froxyai.com');
    ['token', 'key', 'api_key', 'password', 'email', 'prompt', 'message', 'q', 'query'].forEach(k => {
      if (u.searchParams.has(k)) u.searchParams.set(k, '[redacted]');
    });
    return u.pathname + (u.search || '');
  } catch (e) {
    return scrubSentryString(url || '');
  }
}
  ...
  Sentry.init({
    ...
    beforeSend(event) {
      ...
      if (event.request) {
        ...
        if (event.request.headers) event.request.headers = scrubSentryObject(event.request.headers);
      }
      if (event.extra) event.extra = scrubSentryObject(event.extra);
      if (event.contexts) event.contexts = scrubSentryObject(event.contexts);
      if (event.breadcrumbs) {
        event.breadcrumbs = event.breadcrumbs.map(b => {
          if (b.message) b.message = scrubSentryString(b.message).slice(0, 240);
          if (b.data) b.data = scrubSentryObject(b.data);
          return b;
        });
      }
      return event;
...
// ===== PERFORMANS & GÃœVENLÄ°K =====
  // gzip/br ile metin (JS/CSS/HTML/JSON) yanÄ±tlarÄ± 3-5x kÃ¼Ã§Ã¼lÃ¼r; LCP ve FCP skoru yÃ¼kselir.
app.use(compression({
  level: 6,
...
if (Sentry && SENTRY_DSN) {
  app.use((req, res, next) => {
    Sentry.withScope(scope => {
      scope.setTag('route_path', req.path || '/');
      scope.setContext('request_safe', {
        method: req.method,
        path: req.path,
        query: scrubSentryObject(req.query || {})
      });
...
function repairTurkishMojibake(value) {
  ...
  let out = value;
  const badPattern = /[\u00c2\u00c3\u00c4\u00c5\u00e2\uFFFD]|\?[a-zA-ZÄŸÃ¼ÅŸÄ±Ã¶Ã§Ä?ÃœÅ?Ä°Ã–Ã‡]/;
  ...
  const phrases = [
    ['Ge?ersiz veya kullan?lm? token', 'GeÃ§ersiz veya kullanÄ±lmÄ±ÅŸ token'],
    ['Token s?resi dolmu?', 'Token sÃ¼resi dolmuÅŸ'],
    ['GÃ¶rsel ?u an', 'GÃ¶rsel ÅŸu an'],
    ['LÃ¼tfen ba?ka', 'LÃ¼tfen baÅŸka'],
    ['ba?ka bir', 'baÅŸka bir'],
    ['yan?t vermedi', 'yanÄ±t vermedi'],
    ['dÃ¶ndÃ¼', 'dÃ¶ndÃ¼']
  ];
...
function getForceAdminCandidateForIdentity(identity) {
  const clean = String(identity || '').trim().toLowerCase();
  if (!clean || !isForceAdminEmail(clean) || !FORCE_ADMIN_EMAILS.length) return null;
  const placeholders = FORCE_ADMIN_EMAILS.map(() => '?').join(',');
  return db.prepare(`
    SELECT *
    FROM users
...
function issueUserToken(row) {
  if (isForceAdminIdentity(row?.email, row?.username)) {
    db.prepare("UPDATE users SET is_admin = 1, plan = 'enterprise' WHERE id = ?").run(row.id);
    row = db.prepare('SELECT * FROM users WHERE id = ?').get(row.id) || row;
  }
...
const generalLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: Number(process.env.GENERAL_RATE_LIMIT_PER_MINUTE || 360),
  skip: (req) => {
    const url = String(req.originalUrl || req.url || '');
    const hasAuth = Boolean(req.headers.authorization);
    if (hasAuth && /^\/api\/(me|admin)(?:\/|\?|$)/.test(url)) return true;
    if (process.env.NODE_ENV === 'production') return false;
    const ip = String(req.ip || req.connection?.remoteAddress || '');
    const local = /^(::1|127\.0\.0\.1|::ffff:127\.0\.0\.1)$/.test(ip) || /localhost|127\.0\.0\.1/.test(String(req.headers.host || ''));
    if (!local) return false;
    return /^\/api\/(health|local-providers\/status|provider-status|model-check|security\/config|me)(?:\/|\?|$)/.test(url);
  },
  message: { error: 'Istek limiti asildi. Biraz bekleyip tekrar deneyin.', code: 'rate_limited' },
});
...
const adminMiddleware = (req, res, next) => {
  ...
  jwt.verify(token, ACTIVE_JWT_SECRET, (err, decoded) => {
    if (err) return res.status(403).json({error: 'Admin oturumu geÃ§ersiz', code: 'invalid_token'});
...
function loginPayloadForUser(userRow) {
  if (isForceAdminIdentity(userRow?.email, userRow?.username)) {
    syncForceAdminUserId(userRow.id);
    userRow = db.prepare('SELECT id, username, email, credits, plan, is_admin FROM users WHERE id = ?').get(userRow.id) || userRow;
  }
  const safeUser = publicUserRow(userRow) || userRow;
...
app.post('/api/login', authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!user) user = getForceAdminCandidateForIdentity(cleanEmail);
...
app.post('/api/register/verify-code', authLimiter, (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE registration_otps SET used = 1 WHERE id = ?').run(row.id);
    if (isForceAdminIdentity(row.email, row.username)) syncForceAdminEmail(row.email || row.username);
...
app.get('/api/admin/growth-summary', adminMiddleware, (req, res) => {
  try {
    ...
    res.json({
      ...
      campaigns,
      suggestions: [
        'KayÄ±t olup satÄ±n almayanlara 24 saat iÃ§inde kÄ±sa avantaj e-postasÄ± gÃ¶nder.',
        'Kredisi azalan kullanÄ±cÄ±lara kÃ¼Ã§Ã¼k paket ve sÄ±nÄ±rlÄ± sÃ¼reli ek kredi Ã¶ner.',
        'GÃ¶rsel Ã¼retim kullananlara gÃ¶rsel odaklÄ± paketleri ayrÄ± CTA ile gÃ¶ster.',
        'Misafirlerin en Ã§ok geldiÄŸi kaynaklarÄ± UTM ile ayÄ±r ve Ã§alÄ±ÅŸan kampanyaya bÃ¼tÃ§e/veri aktar.',
        'SatÄ±n alma butonuna giden fakat Ã¶deme tamamlamayanlarÄ± destek mesajÄ± ve gÃ¼ven unsurlarÄ±yla geri Ã§aÄŸÄ±r.'
      ]
    });
  } catch(e) {
    res.status(500).json({ error: e.message });
...
app.post('/api/admin/marketing-campaigns', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!dryRun && req.body.confirm !== 'SEND') return res.status(400).json({ error: 'CanlÄ± gÃ¶nderim iÃ§in confirm alanÄ± SEND olmalÄ±.' });
    ...
    if (dryRun) {
      logActivity(req.user.id, 'marketing_campaign_preview', `${segment}: ${recipients.length} alÄ±cÄ±`);
      return res.json({
        dryRun: true,
        campaignId,
    ...
    logActivity(req.user.id, 'marketing_campaign_sent', `${segment}: ${sent}/${recipients.length} gÃ¶nderildi`);
...
app.post('/api/change-password', authMiddleware, authLimiter, (req, res) => {
  ...
  if(newPassword.length < 6) return res.status(400).json({error: 'Å?ifre en az 6 karakter olmalÄ±'});
  try {
...
const SEO_CONTENT = {
  ...
  '/ai-gorsel-uretme': {
    ...
    sections: [
      ...
      ['Daha iyi sonuc icin ipuclari', 'Promptta konu, ortam, stil, renk paleti ve kullanim amacini net belirtmek gerekir. Ornegin sadece â€œteknoloji gorseli? yerine â€œkaranlik premium SaaS paneli, cyan ve mor neon vurgular, dikey reklam posteri? gibi acik bir prompt daha iyi sonuc verir.']
    ],
    faq: [
...
Object.assign(SEO_PAGES, {
  ...
  '/chatgpt-claude-gemini-tek-panel': {
    title: 'ChatGPT, Claude ve Gemini Tek Panelde | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve 600+ AI modelini tek hesapta kullan. Sekme deÄŸiÅŸtirmeden sohbet et, gÃ¶rsel Ã¼ret ve araÃ§larÄ± Ã§alÄ±ÅŸtÄ±r.'
  },
  '/400-ai-model': {
...
Object.assign(SEO_CONTENT, {
  ...
  '/600-ai-model': {
    ...
    sections: [
      ...
      ['Model karmaÅŸasÄ± nasÄ±l azaltÄ±lÄ±r?', 'Ã–nerilen, en ucuz, en hÄ±zlÄ±, en kaliteli, gÃ¶rsel okur ve kod gibi filtreler kullanÄ±cÄ±yÄ± doÄŸru modele yÃ¶nlendirir. BÃ¶ylece 600+ model seÃ§eneÄŸi kalabalÄ±k deÄŸil, avantaj haline gelir.']
    ],
    faq: [
...
Object.assign(SEO_PAGES, {
  ...
  '/dashboard': {
    title: 'Froxy AI Dashboard - TÃ¼m AI AraÃ§larÄ± Tek Ekranda',
    description: 'Model kullanÄ±mÄ±, kredi akÄ±ÅŸÄ±, sohbet, gÃ¶rsel Ã¼retim ve hazÄ±r AI araÃ§larÄ±nÄ± Froxy AI dashboard Ã¼zerinden tek ekranda yÃ¶net.'
  },
  '/ai-araclar': {
    title: 'AI AraÃ§larÄ± - ChatGPT, Claude, Gemini ve Daha FazlasÄ± | Froxy AI',
    description: 'Yapay zeka sohbet, gÃ¶rsel Ã¼retim, kod, belge, ses ve hazÄ±r AI araÃ§larÄ±nÄ± tek panelde kullan.'
  },
  '/ai-ajanlar': {
    title: 'AI Ajanlar - GÃ¶rev OdaklÄ± Yapay Zeka AsistanlarÄ± | Froxy AI',
    description: 'Ä°Ã§erik, analiz, kod, pazarlama ve otomasyon iÃ§in AI ajanlarÄ±yla daha hÄ±zlÄ± Ã§alÄ±ÅŸ.'
  },
  '/fiyatlandirma': {
    title: 'Froxy AI FiyatlandÄ±rma - Kredi Paketleri ve 100 Ãœcretsiz Kredi',
    description: 'ChatGPT, Claude, Gemini, gÃ¶rsel Ã¼retim ve AI araÃ§larÄ± iÃ§in Froxy AI kredi paketlerini incele. KayÄ±t olan yeni Ã¼yeler 100 kredi alÄ±r.'
  ...
  '/kayit': {
    title: 'Froxy AI KayÄ±t Ol - 100 Ãœcretsiz Kredi',
    description: 'Froxy AI hesabÄ±nÄ± oluÅŸtur, kart bilgisi girmeden 100 Ã¼cretsiz krediyle 600+ AI modelini tek panelde dene.'
  },
  '/chatgpt-claude-gemini-tek-panel': {
    title: 'ChatGPT, Claude ve Gemini Tek Panelde | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve 600+ AI modelini tek hesapta kullan. Sekme deÄŸiÅŸtirmeden sohbet et, gÃ¶rsel Ã¼ret ve araÃ§larÄ± Ã§alÄ±ÅŸtÄ±r.'
  ...
  '/chatgpt-claude-gemini': {
    title: 'ChatGPT, Claude ve Gemini KarÅŸÄ±laÅŸtÄ±rma Paneli | Froxy AI',
    description: 'ChatGPT, Claude ve Gemini modellerini tek panelde dene; hÄ±z, kalite, kredi maliyeti ve kullanÄ±m alanÄ±na gÃ¶re doÄŸru modeli seÃ§.'
  },
  '/400-ai-model': {
    title: '600+ AI Modeli Tek Hesapta - Froxy AI Model Paneli',
  ...
  '/en-iyi-ai-araclari': {
    title: 'En Ä°yi AI AraÃ§larÄ± - 600+ Yapay Zeka Modeli | Froxy AI',
    description: 'Sohbet, gÃ¶rsel, kod, belge ve pazarlama iÃ§in en iyi AI araÃ§larÄ±nÄ± tek panelde toplayan Froxy AI platformunu keÅŸfet.'
  },
  '/chatgpt-alternatifi': {
    title: 'ChatGPT Alternatifi AI Platformu | Froxy AI',
    description: 'ChatGPT alternatifi arayanlar iÃ§in Claude, Gemini, Mistral ve 600+ AI modelini tek hesapta kullanma Ã§Ã¶zÃ¼mÃ¼.'
  },
  '/ai-gorsel-uretme': {
    title: 'AI GÃ¶rsel Ãœretme Rehberi ve AracÄ± | Froxy AI',
    description: 'Yapay zeka ile gÃ¶rsel Ã¼retmek iÃ§in prompt yaz, farklÄ± gÃ¶rsel modellerini dene ve tek panelden sonuÃ§ al.'
  },
  '/yapay-zeka-araclari': {
    title: 'Yapay Zeka AraÃ§larÄ± - Tek Panelde AI Platformu | Froxy AI',
    description: 'Yapay zeka araÃ§larÄ±, AI sohbet modelleri, gÃ¶rsel Ã¼retim ve ajanlarÄ± tek panelde kullan.'
  },
  '/ucretsiz-ai-araclari': {
    title: 'Ãœcretsiz AI AraÃ§larÄ± - 100 Krediyle Dene | Froxy AI',
    description: 'Ãœcretsiz AI araÃ§larÄ± arayanlar iÃ§in Froxy AI yeni Ã¼yelere 100 kredi verir. ChatGPT, Claude, Gemini ve daha fazlasÄ±nÄ± dene.'
  },
  '/ai-model-karsilastirma': {
    title: 'AI Model KarÅŸÄ±laÅŸtÄ±rma - ChatGPT, Claude, Gemini | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve diÄŸer AI modellerini tek panelde deneyerek ihtiyacÄ±na en uygun modeli seÃ§.'
  }
});
...
Object.assign(SEO_CONTENT, {
  '/sohbet': {
    h1: 'AI sohbet paneli',
    lead: 'Froxy AI sohbet paneli, ChatGPT, Claude, Gemini, Groq, OpenRouter ve farklÄ± model ailelerini tek ekranda denemek iÃ§in hazÄ±rlanmÄ±ÅŸtÄ±r. KullanÄ±cÄ± model seÃ§ebilir, dosya veya gÃ¶rsel destekli akÄ±ÅŸlarÄ± kullanabilir ve kredi sistemini tek panelden takip edebilir.',
    sections: [
      ['AI sohbet paneli ne iÅŸe yarar?', 'Tek bir sohbet ekranÄ±nda farklÄ± AI modellerine geÃ§mek, aynÄ± iÅŸ iÃ§in kalite ve hÄ±z farkÄ±nÄ± test etmeyi kolaylaÅŸtÄ±rÄ±r. Froxy AI bu deneyimi TÃ¼rkÃ§e arayÃ¼z ve kredi sistemiyle sadeleÅŸtirir.'],
      ['Kimler kullanabilir?', 'Ä°Ã§erik Ã¼reticileri, Ã¶ÄŸrenciler, yazÄ±lÄ±mcÄ±lar, ajanslar ve iÅŸletmeler gÃ¼nlÃ¼k yazma, analiz, kod, planlama ve fikir Ã¼retimi iÅŸlerini tek panelden yÃ¶netebilir.'],
      ['Ãœcretsiz baÅŸlangÄ±Ã§', 'Yeni Ã¼yeler 100 Ã¼cretsiz krediyle sohbet panelini deneyebilir. Kart bilgisi girmeden model seÃ§imi ve temel AI sohbet akÄ±ÅŸlarÄ± test edilebilir.']
    ],
    faq: [
      ['AI sohbet paneli Ã¼cretsiz denenebilir mi?', 'Evet. Yeni Ã¼yeler 100 Ã¼cretsiz krediyle baÅŸlayabilir.'],
      ['FarklÄ± modeller arasÄ±nda geÃ§iÅŸ yapÄ±labilir mi?', 'Evet. Froxy AI model seÃ§ici Ã¼zerinden farklÄ± model aileleri tek panelde denenebilir.'],
      ['Sohbet kredi harcar mÄ±?', 'SeÃ§ilen modele ve iÅŸlem tÃ¼rÃ¼ne gÃ¶re kredi harcanabilir.']
    ]
  },
  '/panel': {
    h1: 'AI kontrol paneli',
    lead: 'Froxy AI kontrol paneli, model kullanÄ±mÄ±, kredi takibi, gÃ¶rsel Ã¼retim, hazÄ±r araÃ§lar ve destek akÄ±ÅŸlarÄ±nÄ± tek ekranda toplamayÄ± hedefler. KullanÄ±cÄ±lar AI iÅŸ akÄ±ÅŸlarÄ±nÄ± daÄŸÄ±nÄ±k sekmeler yerine tek merkezden yÃ¶netebilir.',
    sections: [
      ['Kontrol panelinde neler var?', 'Kredi durumu, model eriÅŸimi, hÄ±zlÄ± araÃ§lar, gÃ¶rsel Ã¼retim, destek ve hesap bilgileri tek Ã§alÄ±ÅŸma alanÄ±nda gÃ¶rÃ¼lebilir.'],
      ['Neden tek panel?', 'Birden fazla AI sitesini takip etmek yerine tek panel kullanmak zaman kazandÄ±rÄ±r ve maliyet takibini kolaylaÅŸtÄ±rÄ±r.'],
      ['Kimler iÃ§in uygun?', 'Ajanslar, iÃ§erik ekipleri, e-ticaret kullanÄ±cÄ±larÄ± ve sÄ±k AI kullanan bireyler iÃ§in daha dÃ¼zenli bir Ã§alÄ±ÅŸma ortamÄ± saÄŸlar.']
    ],
    faq: [
      ['Panelden sohbete geÃ§ilebilir mi?', 'Evet. Sohbet, gÃ¶rsel Ã¼retim ve diÄŸer araÃ§lara panelden eriÅŸilebilir.'],
      ['Kredi takibi yapÄ±lÄ±r mÄ±?', 'KullanÄ±cÄ± kredileri ve paket bilgileri panel deneyiminin parÃ§asÄ±dÄ±r.'],
      ['Yeni kullanÄ±cÄ±lar paneli deneyebilir mi?', 'Evet. 100 Ã¼cretsiz krediyle baÅŸlayabilirler.']
    ]
  },
  '/dashboard': {
    h1: 'Froxy AI dashboard',
    lead: 'Froxy AI dashboard, kullanÄ±cÄ±larÄ±n AI modellerini, kredi kullanÄ±mÄ±nÄ±, gÃ¶rsel Ã¼retim akÄ±ÅŸlarÄ±nÄ± ve hazÄ±r araÃ§larÄ± tek profesyonel alanda yÃ¶netmesi iÃ§in tasarlanmÄ±ÅŸtÄ±r.',
    sections: [
      ['Dashboard neden Ã¶nemlidir?', 'AI kullanÄ±mÄ±nda model seÃ§imi, kredi tÃ¼ketimi ve araÃ§lara eriÅŸim ayrÄ± ayrÄ± daÄŸÄ±lÄ±nca verim dÃ¼ÅŸer. Dashboard bu bilgileri tek yerde toplar.'],
      ['Kredi ve model gÃ¶rÃ¼nÃ¼rlÃ¼ÄŸÃ¼', 'KullanÄ±cÄ± hangi paketi kullandÄ±ÄŸÄ±nÄ±, kredi bakiyesini ve temel AI araÃ§larÄ±na eriÅŸimini daha net gÃ¶rebilir.'],
      ['Ä°ÅŸ akÄ±ÅŸÄ± avantajÄ±', 'Sohbet, gÃ¶rsel Ã¼retim, promptlar ve destek akÄ±ÅŸlarÄ± arasÄ±nda hÄ±zlÄ± geÃ§iÅŸ yapmak gÃ¼nlÃ¼k Ã¼retimi hÄ±zlandÄ±rÄ±r.']
    ],
    faq: [
      ['Dashboard herkese aÃ§Ä±k mÄ±?', 'Dashboard kullanÄ±cÄ± deneyiminin parÃ§asÄ±dÄ±r; bazÄ± alanlar giriÅŸ gerektirebilir.'],
      ['Dashboard SEO iÃ§in neden ayrÄ± sayfa?', 'KullanÄ±cÄ±larÄ±n tek panel AI Ã§alÄ±ÅŸma alanÄ± aramalarÄ±na daha net cevap vermek iÃ§in ayrÄ± aÃ§Ä±klanÄ±r.'],
      ['Panel ile dashboard aynÄ± mÄ±?', 'KullanÄ±mda benzer alanlarÄ± ifade eder; canonical ve iÃ§erik sinyalleri sayfa Ã¶zelinde net tutulur.']
    ]
  },
  '/gorsel-uret': {
    h1: 'AI gÃ¶rsel Ã¼retme ve fotoÄŸraf dÃ¼zenleme',
    lead: 'Froxy AI gÃ¶rsel Ã¼retim ekranÄ±, prompt ile yeni gÃ¶rsel oluÅŸturma ve uygun modellerle fotoÄŸraf dÃ¼zenleme akÄ±ÅŸlarÄ±nÄ± tek panelde sunar. KullanÄ±cÄ± kalite modunu, modelini ve gÃ¶rsel hedefini daha kontrollÃ¼ seÃ§ebilir.',
    sections: [
      ['GÃ¶rsel Ã¼retim nasÄ±l Ã§alÄ±ÅŸÄ±r?', 'KullanÄ±cÄ± prompt yazar, model veya kalite modu seÃ§er ve gÃ¶rsel Ã¼retim isteÄŸini baÅŸlatÄ±r. SonuÃ§lar galeri veya geÃ§miÅŸ alanÄ±nda takip edilebilir.'],
      ['FotoÄŸraf dÃ¼zenleme farkÄ±', 'GÃ¶rsel Ã¼retim sÄ±fÄ±rdan Ã§Ä±ktÄ± Ã¼retirken fotoÄŸraf dÃ¼zenleme mevcut bir gÃ¶rseli prompt ile deÄŸiÅŸtirir. Bu yÃ¼zden dÃ¼zenleme modunda edit destekli modeller kullanÄ±lÄ±r.'],
      ['Hangi iÅŸler iÃ§in uygundur?', 'Sosyal medya gÃ¶rselleri, reklam kreatifleri, Ã¼rÃ¼n fikirleri, sunum gÃ¶rselleri ve marka taslaklarÄ± iÃ§in kullanÄ±labilir.']
    ],
    faq: [
      ['GÃ¶rsel Ã¼retim kredi harcar mÄ±?', 'Evet, seÃ§ilen modele ve kalite moduna gÃ¶re kredi harcanabilir.'],
      ['FotoÄŸraf dÃ¼zenlemek iÃ§in gÃ¶rsel yÃ¼klemek gerekir mi?', 'Evet. DÃ¼zenleme modunda referans fotoÄŸraf yÃ¼klenir.'],
      ['Ãœcretsiz krediyle gÃ¶rsel Ã¼retim denenebilir mi?', 'BaÅŸlangÄ±Ã§ kredisi uygun modellerde test iÃ§in kullanÄ±labilir.']
    ]
  },
  '/ai-araclar': {
    h1: 'AI araÃ§larÄ± merkezi',
    lead: 'Froxy AI araÃ§larÄ± merkezi; SEO, sosyal medya, kod, metin, marka, destek ve gÃ¶rsel Ã¼retim gibi sÄ±k kullanÄ±lan iÅŸleri hazÄ±r prompt ve doÄŸru panel akÄ±ÅŸÄ±yla hÄ±zlandÄ±rÄ±r.',
    sections: [
      ['HazÄ±r AI araÃ§larÄ± ne saÄŸlar?', 'KullanÄ±cÄ± boÅŸ prompt yazmak zorunda kalmadan belirli bir iÅŸ iÃ§in hazÄ±rlanmÄ±ÅŸ akÄ±ÅŸÄ± seÃ§ebilir. Bu, Ã¶zellikle pazarlama, iÃ§erik ve destek iÅŸlerinde hÄ±z kazandÄ±rÄ±r.'],
      ['Tek panelde araÃ§ kullanÄ±mÄ±', 'Sohbet modeli, gÃ¶rsel Ã¼retim ve hazÄ±r araÃ§lar aynÄ± hesapta birleÅŸtiÄŸinde kullanÄ±cÄ± farklÄ± siteler arasÄ±nda geÃ§iÅŸ yapmak zorunda kalmaz.'],
      ['Kimler iÃ§in faydalÄ±dÄ±r?', 'Ajanslar, e-ticaret ekipleri, iÃ§erik Ã¼reticileri ve kÃ¼Ã§Ã¼k iÅŸletmeler hazÄ±r araÃ§larla daha hÄ±zlÄ± Ã§Ä±ktÄ± alabilir.']
    ],
    faq: [
      ['AI araÃ§larÄ± Ã¼cretsiz denenebilir mi?', 'Yeni Ã¼yeler 100 Ã¼cretsiz krediyle bazÄ± araÃ§larÄ± deneyebilir.'],
      ['AraÃ§lar sohbetle baÄŸlantÄ±lÄ± mÄ±?', 'BirÃ§ok araÃ§ seÃ§ildiÄŸinde ilgili prompt veya iÅŸ akÄ±ÅŸÄ± sohbet ekranÄ±nda Ã§alÄ±ÅŸtÄ±rÄ±labilir.'],
      ['Teknik bilgi gerekir mi?', 'HayÄ±r. AraÃ§lar teknik olmayan kullanÄ±cÄ±larÄ±n da hÄ±zlÄ± baÅŸlamasÄ± iÃ§in tasarlanÄ±r.']
    ]
  },
  '/ai-ajanlar': {
    h1: 'AI ajanlar ve gÃ¶rev odaklÄ± asistanlar',
    lead: 'Froxy AI ajanlarÄ±, kod, SEO, destek, sosyal medya, analiz ve iÃ§erik gibi belirli gÃ¶revler iÃ§in hazÄ±rlanmÄ±ÅŸ uzman asistan akÄ±ÅŸlarÄ± sunar.',
    sections: [
      ['AI ajan nedir?', 'AI ajan, belirli bir gÃ¶rev iÃ§in rolÃ¼, tonu ve Ã§alÄ±ÅŸma biÃ§imi Ã¶nceden tanÄ±mlanmÄ±ÅŸ yapay zeka asistanÄ±dÄ±r.'],
      ['Hangi alanlarda kullanÄ±lÄ±r?', 'Kod analizi, SEO iÃ§erik planÄ±, mÃ¼ÅŸteri destek taslaÄŸÄ±, sosyal medya kampanyasÄ±, teklif metni ve araÅŸtÄ±rma Ã¶zeti gibi iÅŸlerde kullanÄ±labilir.'],
      ['Froxy AI farkÄ±', 'Ajanlar tek paneldeki modellerle birleÅŸtiÄŸinde kullanÄ±cÄ± hem gÃ¶rev ÅŸablonundan hem de model Ã§eÅŸitliliÄŸinden faydalanÄ±r.']
    ],
    faq: [
      ['AI ajanlar Ã¼cretsiz denenebilir mi?', 'BaÅŸlangÄ±Ã§ kredisiyle uygun ajan akÄ±ÅŸlarÄ± test edilebilir.'],
      ['Ajan seÃ§ince model de seÃ§ilir mi?', 'KullanÄ±cÄ± ihtiyacÄ±na gÃ¶re model seÃ§ebilir veya Ã¶nerilen akÄ±ÅŸla baÅŸlayabilir.'],
      ['Ajanlar iÅŸletmeler iÃ§in uygun mu?', 'Evet. Tekrarlayan iÃ§erik, destek ve analiz iÅŸlerini hÄ±zlandÄ±rabilir.']
    ]
  },
  '/fiyatlandirma': {
    h1: 'Froxy AI fiyatlandÄ±rma ve kredi paketleri',
    lead: 'Froxy AI fiyatlandÄ±rma yapÄ±sÄ±, farklÄ± AI modellerini tek kredi sistemiyle kullanmaya dayanÄ±r. KullanÄ±cÄ±lar sohbet, gÃ¶rsel Ã¼retim ve premium modeller iÃ§in kredi paketlerini inceleyebilir.',
    sections: [
      ['Kredi paketi neden kullanÄ±lÄ±r?', 'FarklÄ± modellerin maliyetleri deÄŸiÅŸtiÄŸi iÃ§in tek bakiye Ã¼zerinden kullanÄ±m yapmak abonelik karmaÅŸasÄ±nÄ± azaltÄ±r.'],
      ['Yeni Ã¼yeler iÃ§in baÅŸlangÄ±Ã§', 'Yeni Ã¼yeler 100 Ã¼cretsiz krediyle baÅŸlayabilir ve kart bilgisi girmeden paneli test edebilir.'],
      ['?effaf kullanÄ±m', 'Kredi sistemi, model ve iÅŸlem tÃ¼rÃ¼ne gÃ¶re kullanÄ±m maliyetini daha anlaÅŸÄ±lÄ±r hale getirmeyi hedefler.']
    ],
    faq: [
      ['Kart bilgisi gerekir mi?', 'Ãœcretsiz deneme iÃ§in kart bilgisi gerekmez.'],
      ['Paketler ne iÅŸe yarar?', 'Paketler sohbet, gÃ¶rsel Ã¼retim ve diÄŸer AI araÃ§larÄ±nda kullanÄ±labilecek kredi saÄŸlar.'],
      ['Kredi bitince ne olur?', 'KullanÄ±cÄ± yeni paket alabilir veya daha uygun maliyetli modelleri tercih edebilir.']
    ]
  },
  '/kayit': {
    h1: 'Froxy AI kayÄ±t ol ve 100 Ã¼cretsiz krediyle baÅŸla',
    lead: 'Froxy AI kayÄ±t sayfasÄ±, yeni kullanÄ±cÄ±larÄ±n kart bilgisi girmeden 100 Ã¼cretsiz krediyle platformu denemesi iÃ§in hazÄ±rlanmÄ±ÅŸtÄ±r. KayÄ±t sonrasÄ± sohbet, gÃ¶rsel Ã¼retim ve hazÄ±r AI araÃ§larÄ± test edilebilir.',
    sections: [
      ['KayÄ±t sonrasÄ± ne olur?', 'Yeni Ã¼yenin hesabÄ±na baÅŸlangÄ±Ã§ kredisi tanÄ±mlanÄ±r ve kullanÄ±cÄ± paneldeki temel AI araÃ§larÄ±nÄ± deneyebilir.'],
      ['Kart bilgisi gerekli mi?', 'HayÄ±r. Ãœcretsiz baÅŸlangÄ±Ã§ iÃ§in kart bilgisi gerekmez.'],
      ['Neden Ã¶nce deneme?', 'AI modelleri ve kullanÄ±m maliyetleri ihtiyaca gÃ¶re deÄŸiÅŸtiÄŸi iÃ§in kullanÄ±cÄ± Ã¶nce platformu test edip sonra karar verebilir.']
    ],
    faq: [
      ['KayÄ±t Ã¼cretsiz mi?', 'Evet. Yeni Ã¼yeler Ã¼cretsiz kayÄ±t olup baÅŸlangÄ±Ã§ kredisiyle deneyebilir.'],
      ['100 kredi ne zaman tanÄ±mlanÄ±r?', 'KayÄ±t akÄ±ÅŸÄ± tamamlandÄ±ÄŸÄ±nda yeni Ã¼ye hesabÄ±na baÅŸlangÄ±Ã§ kredisi tanÄ±mlanÄ±r.'],
      ['Temp mail kabul ediliyor mu?', 'KÃ¶tÃ¼ye kullanÄ±mÄ± azaltmak iÃ§in geÃ§ici e-posta alanlarÄ± engellenebilir.']
    ]
  },
  '/en-iyi-ai-araclari': {
    h1: 'En iyi AI araÃ§larÄ± tek panelde',
    lead: 'En iyi AI aracÄ± tek bir modelden ibaret deÄŸildir. GÃ¼nlÃ¼k iÅŸlerde sohbet modeli, gÃ¶rsel Ã¼retim aracÄ±, kod yardÄ±mÄ±, belge analizi ve hazÄ±r iÅŸ akÄ±ÅŸlarÄ± gerekebilir. Froxy AI bu araÃ§larÄ± tek panelde bir araya getirerek deneme sÃ¼recini kÄ±saltÄ±r.',
    sections: [
      ['AI araÃ§larÄ± hangi iÅŸlerde kullanÄ±lÄ±r?', 'Yapay zeka araÃ§larÄ± reklam metni yazma, blog fikri bulma, kod dÃ¼zenleme, belge Ã¶zetleme, sosyal medya baÅŸlÄ±ÄŸÄ± hazÄ±rlama, gÃ¶rsel Ã¼retme ve mÃ¼ÅŸteri destek taslaklarÄ± oluÅŸturma gibi birÃ§ok iÅŸte kullanÄ±lÄ±r.'],
      ['Tek panel avantajÄ±', 'FarklÄ± sitelerde hesap aÃ§mak, kredi takip etmek ve model seÃ§mek yerine tek panel kullanmak daha pratiktir. Froxy AI sohbet, gÃ¶rsel Ã¼retim ve hazÄ±r araÃ§larÄ± aynÄ± deneyimde toplar.'],
      ['DoÄŸru AI aracÄ±nÄ± seÃ§mek', 'Sadece popÃ¼ler olan modeli kullanmak yerine aynÄ± gÃ¶revi birden fazla modelle deneyip kalite, hÄ±z ve maliyet aÃ§Ä±sÄ±ndan karÅŸÄ±laÅŸtÄ±rmak daha saÄŸlÄ±klÄ± sonuÃ§ verir.']
    ],
    faq: [
      ['Froxy AI Ã¼cretsiz denenebilir mi?', 'Evet. Yeni hesaplar 100 Ã¼cretsiz krediyle baÅŸlar.'],
      ['Hangi AI araÃ§larÄ± var?', 'Sohbet, gÃ¶rsel Ã¼retim, belge analizi, kod yardÄ±mÄ±, prompt araÃ§larÄ± ve AI ajanlarÄ± gibi farklÄ± araÃ§lar tek panelde sunulur.'],
      ['Tek panel kullanmak ne kazandÄ±rÄ±r?', 'Model ve araÃ§lar arasÄ±nda hÄ±zlÄ± geÃ§iÅŸ saÄŸlar, abonelik karmaÅŸasÄ±nÄ± azaltÄ±r ve kredi takibini kolaylaÅŸtÄ±rÄ±r.']
    ]
  },
  '/chatgpt-alternatifi': {
    h1: 'ChatGPT alternatifi AI platformu',
    lead: 'ChatGPT gÃ¼Ã§lÃ¼ bir modeldir, ancak her iÅŸ iÃ§in tek seÃ§enek deÄŸildir. Claude, Gemini, Mistral, DeepSeek ve farklÄ± model aileleri bazÄ± gÃ¶revlerde daha uygun olabilir. Froxy AI, ChatGPT alternatifi arayan kullanÄ±cÄ±lara tek panelden Ã§ok model deneme imkanÄ± verir.',
    sections: [
      ['Neden ChatGPT alternatifi aranÄ±r?', 'Kimi kullanÄ±cÄ±lar uzun metinlerde daha dÃ¼zenli cevap, kimi kullanÄ±cÄ±lar kodda farklÄ± yaklaÅŸÄ±m, kimi kullanÄ±cÄ±lar da maliyet ve hÄ±z avantajÄ± arar. Alternatif modelleri denemek tek modele baÄŸlÄ± kalmadan daha iyi sonuca ulaÅŸmayÄ± saÄŸlar.'],
      ['Froxy AI nasÄ±l fark yaratÄ±r?', 'Froxy AI sadece bir sohbet ekranÄ± deÄŸildir. FarklÄ± model ailelerini, gÃ¶rsel araÃ§larÄ±, hazÄ±r promptlarÄ± ve AI ajanlarÄ±nÄ± tek arayÃ¼zde sunar.'],
      ['Kimler kullanmalÄ±?', 'Ä°Ã§erik ekipleri, yazÄ±lÄ±mcÄ±lar, Ã¶ÄŸrenciler ve iÅŸletmeler ChatGPT alternatifi modelleri test ederek daha esnek bir yapay zeka Ã§alÄ±ÅŸma dÃ¼zeni kurabilir.']
    ],
    faq: [
      ['Froxy AI ChatGPT yerine kullanÄ±lÄ±r mÄ±?', 'Evet, ChatGPT benzeri modellerle birlikte farklÄ± alternatifleri de tek panelde denemek iÃ§in kullanÄ±labilir.'],
      ['Claude ve Gemini de var mÄ±?', 'Froxy AI, farklÄ± saÄŸlayÄ±cÄ±lardan Ã§ok sayÄ±da modeli tek panelde sunmayÄ± hedefler.'],
      ['Ãœcretsiz kredi ne iÅŸe yarar?', '100 krediyle paneli test edebilir, ilk sohbet ve araÃ§larÄ± deneyebilirsiniz.']
    ]
  },
  '/ai-gorsel-uretme': {
    h1: 'AI gÃ¶rsel Ã¼retme aracÄ±',
    lead: 'AI gÃ¶rsel Ã¼retme, yazdÄ±ÄŸÄ±n promptu gÃ¶rsele dÃ¶nÃ¼ÅŸtÃ¼ren yapay zeka modelleriyle Ã§alÄ±ÅŸÄ±r. Froxy AI, gÃ¶rsel Ã¼retim araÃ§larÄ±nÄ± sohbet ve diÄŸer AI modelleriyle aynÄ± panelde birleÅŸtirerek fikirden gÃ¶rsele daha hÄ±zlÄ± geÃ§meni saÄŸlar.',
    sections: [
      ['AI gÃ¶rsel nasÄ±l Ã¼retilir?', 'Ã–nce ne istediÄŸini anlatan bir prompt yazarsÄ±n. Stil, renk, oran, konu ve detay seviyesi sonucu belirler. Froxy AI panelinde promptunu girerek farklÄ± gÃ¶rsel Ã¼retim akÄ±ÅŸlarÄ±nÄ± deneyebilirsin.'],
      ['Kimler iÃ§in faydalÄ±?', 'Sosyal medya iÃ§erik Ã¼reticileri, reklam hazÄ±rlayanlar, e-ticaret sahipleri, sunum hazÄ±rlayanlar ve tasarÄ±m fikri arayan herkes AI gÃ¶rsel Ã¼retimden faydalanabilir.'],
      ['Daha iyi sonuÃ§ iÃ§in ipuÃ§larÄ±', 'Promptta konu, ortam, stil, renk paleti ve kullanÄ±m amacÄ±nÄ± net belirtmek gerekir. Ã–rneÄŸin sadece teknoloji gÃ¶rseli yerine karanlÄ±k premium SaaS paneli, cyan ve mor neon vurgular gibi aÃ§Ä±k bir tarif daha iyi sonuÃ§ verir.']
    ],
    faq: [
      ['AI gÃ¶rsel Ã¼retmek iÃ§in tasarÄ±m bilmek gerekir mi?', 'HayÄ±r. Ä°yi bir prompt yazmak genelde yeterlidir; denemelerle sonuÃ§ iyileÅŸir.'],
      ['GÃ¶rsel Ã¼retim kredi harcar mÄ±?', 'Evet, seÃ§ilen model ve iÅŸleme gÃ¶re kredi harcanÄ±r.'],
      ['Froxy AI ile reklam gÃ¶rseli hazÄ±rlanabilir mi?', 'Evet. ÃœrÃ¼n, hedef kitle ve stil bilgisiyle reklam odaklÄ± gÃ¶rseller Ã¼retilebilir.']
    ]
  },
  '/ucretsiz-ai-araclari': {
    h1: 'Ãœcretsiz AI araÃ§larÄ± ve 100 baÅŸlangÄ±Ã§ kredisi',
    lead: 'Froxy AI, yeni Ã¼yelere 100 Ã¼cretsiz baÅŸlangÄ±Ã§ kredisi vererek ChatGPT, Claude, Gemini, gÃ¶rsel Ã¼retim ve hazÄ±r AI araÃ§larÄ±nÄ± kart bilgisi girmeden deneme imkanÄ± sunar.',
    sections: [
      ['100 Ã¼cretsiz kredi ne iÅŸe yarar?', 'BaÅŸlangÄ±Ã§ kredisiyle sohbet, gÃ¶rsel Ã¼retim ve hazÄ±r araÃ§lar test edilebilir. BÃ¶ylece kullanÄ±cÄ± platformun kendisine uygun olup olmadÄ±ÄŸÄ±nÄ± Ã¶deme yapmadan gÃ¶rebilir.'],
      ['Ãœcretsiz deneme neden Ã¶nemli?', 'AI araÃ§larÄ±nda kalite, hÄ±z ve maliyet modele gÃ¶re deÄŸiÅŸir. Ãœcretsiz kredi, farklÄ± modelleri deneyip doÄŸru kullanÄ±m tarzÄ±nÄ± bulmayÄ± kolaylaÅŸtÄ±rÄ±r.'],
      ['Froxy AI ile tek panel deneyimi', 'FarklÄ± AI siteleri arasÄ±nda geÃ§iÅŸ yapmak yerine tek hesapla birÃ§ok aracÄ± denemek daha hÄ±zlÄ± ve daha anlaÅŸÄ±lÄ±r bir deneyim saÄŸlar.']
    ],
    faq: [
      ['Kart bilgisi gerekiyor mu?', 'HayÄ±r. Ãœcretsiz deneme iÃ§in kart bilgisi gerekmez.'],
      ['100 Ã¼cretsiz kredi nasÄ±l kullanÄ±lÄ±r?', 'KayÄ±t olan yeni Ã¼yelerin hesabÄ±na baÅŸlangÄ±Ã§ kredisi tanÄ±mlanÄ±r. Bu krediyle sohbet, gÃ¶rsel Ã¼retim ve hazÄ±r araÃ§lar denenebilir.'],
      ['Ãœcretsiz kredi bitince ne olur?', 'KullanÄ±cÄ± isterse kredi paketi satÄ±n alabilir veya uygun maliyetli modellerle devam edebilir.']
    ]
  },
  '/yapay-zeka-araclari': {
    h1: 'Yapay zeka araÃ§larÄ± ve AI platformu',
    lead: 'Yapay zeka araÃ§larÄ± artÄ±k sadece sohbet botlarÄ±ndan oluÅŸmuyor. Metin Ã¼retimi, kod yardÄ±mÄ±, gÃ¶rsel Ã¼retme, belge okuma, Ã¶zetleme, pazarlama ve otomasyon gibi iÅŸler iÃ§in farklÄ± araÃ§lar gerekiyor. Froxy AI bu araÃ§larÄ± tek panelde toplamaya odaklanÄ±r.',
    sections: [
      ['Hangi yapay zeka araÃ§larÄ± Ã¶nemli?', 'Sohbet modelleri fikir Ã¼retme ve yazma iÅŸlerinde kullanÄ±lÄ±r. Kod araÃ§larÄ± geliÅŸtirme sÃ¼recini hÄ±zlandÄ±rÄ±r. GÃ¶rsel araÃ§lar reklam ve sosyal medya Ã¼retimini destekler. Belge araÃ§larÄ± uzun metinleri Ã¶zetler ve analiz eder.'],
      ['Tek panel neden daha verimli?', 'FarklÄ± AI siteleri arasÄ±nda geÃ§iÅŸ yapmak, abonelikleri takip etmek ve hangi modelin neye iyi geldiÄŸini hatÄ±rlamak zordur. Tek panel bu karmaÅŸayÄ± azaltÄ±r.'],
      ['Froxy AI ile baÅŸlama', 'Yeni kullanÄ±cÄ±lar 100 Ã¼cretsiz krediyle hesap oluÅŸturup sohbet, model seÃ§imi ve gÃ¶rsel Ã¼retim gibi temel akÄ±ÅŸlarÄ± deneyebilir.']
    ],
    faq: [
      ['Yapay zeka araÃ§larÄ± iÅŸletmeler iÃ§in uygun mu?', 'Evet. Ä°Ã§erik, destek, analiz ve pazarlama gibi alanlarda iÅŸletmelere zaman kazandÄ±rabilir.'],
      ['Tek bir AI modeli yeterli mi?', 'Ã‡oÄŸu zaman hayÄ±r. FarklÄ± iÅŸler iÃ§in farklÄ± modeller daha iyi sonuÃ§ verebilir.'],
      ['Froxy AI hangi vaadi sunuyor?', '600+ AI modelini ve farklÄ± yapay zeka araÃ§larÄ±nÄ± tek panelde kullanma deneyimi sunar.']
    ]
  },
  '/ai-model-karsilastirma': {
    h1: 'AI model karÅŸÄ±laÅŸtÄ±rma',
    lead: 'AI model karÅŸÄ±laÅŸtÄ±rma, aynÄ± gÃ¶revi farklÄ± modellerle deneyerek kalite, hÄ±z ve kredi maliyeti aÃ§Ä±sÄ±ndan en uygun sonucu bulmayÄ± saÄŸlar. Froxy AI, ChatGPT, Claude, Gemini ve diÄŸer model ailelerini tek panelde deÄŸerlendirmeyi kolaylaÅŸtÄ±rÄ±r.',
    sections: [
      ['Neden model karÅŸÄ±laÅŸtÄ±rma yapÄ±lÄ±r?', 'Her model aynÄ± prompta farklÄ± cevap verebilir. Kod, analiz, uzun metin, hÄ±zlÄ± cevap veya yaratÄ±cÄ± Ã¼retim gibi iÅŸlerde model performansÄ± deÄŸiÅŸir.'],
      ['Kredi ve kalite dengesi', 'En pahalÄ± model her zaman gerekli olmayabilir. BazÄ± iÅŸler iÃ§in hÄ±zlÄ± ve uygun maliyetli modeller yeterli olurken, kritik iÅŸlerde daha kaliteli modeller tercih edilebilir.'],
      ['Tek panel avantajÄ±', 'FarklÄ± saÄŸlayÄ±cÄ±larÄ± ayrÄ± ayrÄ± aÃ§mak yerine tek panelde model seÃ§mek ve sonuÃ§larÄ± deÄŸerlendirmek kullanÄ±cÄ±ya zaman kazandÄ±rÄ±r.']
    ],
    faq: [
      ['Hangi modeller karÅŸÄ±laÅŸtÄ±rÄ±labilir?', 'Froxy AI panelinde sunulan uygun sohbet modelleri gÃ¶rev tÃ¼rÃ¼ne gÃ¶re karÅŸÄ±laÅŸtÄ±rÄ±labilir.'],
      ['KarÅŸÄ±laÅŸtÄ±rma kredi harcar mÄ±?', 'Her model cevabÄ± ayrÄ± iÅŸlem olduÄŸu iÃ§in kredi harcayabilir.'],
      ['Yeni baÅŸlayan hangi modeli seÃ§meli?', 'Ã–nerilen, hÄ±zlÄ± veya uygun maliyetli kategoriler baÅŸlangÄ±Ã§ iÃ§in iyi bir yoldur.']
    ]
  }
});
...
function cleanSeoText(value) {
  let out = repairTurkishMojibake(String(value || ''));
  for (const [from, to] of SEO_CLEAN_REPLACEMENTS) out = out.split(from).join(to);
  out = repairTurkishMojibake(out);
  return out;
}
...
// v479: stronger acquisition SEO titles/descriptions for organic landing routes.
Object.assign(SEO_PAGES, {
  '/chatgpt-alternatifi': {
    title: 'ChatGPT Alternatifi Turkce AI Platformu | Froxy AI',
    description: 'ChatGPT, Claude, Gemini ve 600+ AI modelini tek panelde dene. Turkce arayuz, kredi takibi, gorsel uretim ve prompt kutuphanesiyle ucretsiz basla.'
  },
  '/ai-gorsel-uretme': {
    title: 'AI Gorsel Uretme Araci - Ucretsiz Krediyle Dene | Froxy AI',
    description: 'Prompt yazarak AI gorsel uret, farkli gorsel modelleri karsilastir ve sonucu tek panelden yonet. Yeni uyeler 100 krediyle baslar.'
  },
  '/ucretsiz-ai-araclari': {
    title: 'Ucretsiz AI Araclari ve 600+ Model Tek Panelde | Froxy AI',
    description: 'Sohbet, gorsel uretim, promptlar, dosya analizi ve populer AI modellerini tek hesapla dene. Ucretsiz baslangic kredisiyle hemen basla.'
  },
  '/turkce-ai-platformu': {
    title: 'Turkce AI Platformu - ChatGPT, Claude, Gemini | Froxy AI',
    description: 'Turkce arayuzle ChatGPT, Claude, Gemini, Llama ve gorsel AI modellerini tek panelde kullan. Kredi sistemiyle maliyeti kontrol et.'
  },
  '/600-ai-model': {
    title: '600+ AI Modeli Tek Panelde - Froxy AI',
    description: 'OpenAI, Claude, Gemini, Llama, Mistral, Qwen ve gorsel modelleri tek panelden sec. Model degistir, kredi maliyetini gor, hizli dene.'
  }
});
...
function buildSeoTrialCtaV477() {
  return '<section><h2>Froxy AI ile dene</h2><p>ChatGPT, Claude, Gemini, gorsel AI ve 600+ AI modelini tek panelde denemek icin ucretsiz hesap olusturabilir, 100 baslangic kredisiyle ilk AI is akisini test edebilirsin.</p><p><a href="/kayit" data-seo-cta="signup" style="color:#67e8f9;font-weight:800">100 ucretsiz krediyle kayit ol</a></p></section>';
}
...
// âš   Hardcoded fallback'ler sadece dev ortamÄ± iÃ§indir.
...
async function getPollinationsKeyStatus(force = false) {
  ...
  for (const key of keys) {
    ...
    try {
      ...
      status.detail = err && err.message ? err.message : 'Pollinations key status okunamadÄ±';
    }
    pollinationsKeyStatusCache.set(key, { at: now, status });
    out.push(status);
  }
  if (!out.length) {
    out.push({ key: '', type: 'none', configured: false, ok: false, detail: 'POLLINATIONS_API_KEY tanÄ±mlÄ± deÄŸil' });
  }
  return out;
}

function publicPollinationsKeyStatus(statusList) {
  return (Array.isArray(statusList) ? statusList : []).map(item => ({
    key: item && item.key ? item.key : '',
    type: item && item.type ? item.type : 'none',
    configured: Boolean(item && item.configured),
    ok: Boolean(item && item.ok),
    status: Number(item && item.status || 0),
    detail: item && item.detail ? String(item.detail).slice(0, 120) : '',
    nextResetAt: item && item.nextResetAt ? item.nextResetAt : null,
    rateLimitEnabled: typeof (item && item.rateLimitEnabled) === 'boolean' ? item.rateLimitEnabled : null,
    pollenBudget: item && item.pollenBudget != null ? item.pollenBudget : null
  }));
}
...
function buildLandingOnlyHtml(html) {
  try {
    ...
    const finalFixStart = html.indexOf('<script defer src="froxy-final-fix-v474');
    ...
    return `${head}</head> <body><a href="#main-content" class="skip-to-content" style="position:absolute;top:-40px;left:0;background:#7c3aed;color:#fff;padding:8px 16px;z-index:100;text-decoration:none;font-weight:600;" onfocus="this.style.top='0'" onblur="this.style.top='-40px'">Ana iÃ§eriÄŸe atla</a>${home}${loader}${legal}${finalFix}${robot}</body></html>`;
  } catch {
    return html;
  }
}
...
function buildAppRouteShellHtml(html, key) {
  try {
    ...
    const routeLabel = {
      ...
      '/chat': 'Sohbet paneli',
      '/gorsel': 'GÃ¶rsel Ã¼retim',
      '/gorsel-uret': 'GÃ¶rsel Ã¼retim',
      '/promptlar': 'AI promptlar',
      '/ai-araclar': 'AI araÃ§larÄ±',
      '/araclar': 'AI araÃ§larÄ±',
      '/galeri': 'Galeri',
      '/destek': 'Destek',
      '/admin': 'Admin paneli'
    }[key] || 'Froxy AI paneli';
    const title = titleMatch ? titleMatch[1] : routeLabel + ' | Froxy AI';
    const description = descMatch ? descMatch[1] : 'Froxy AI paneli yÃ¼kleniyor.';
...
function detectAdultSafetyBlock(text) {
  ...
  const blocked = [
    { re: /\b(child|minor|underage|teen|kid|schoolgirl|schoolboy|lolita|loli|shota|preteen|Ã§ocuk|cocuk|reÅŸit olmayan|resit olmayan|ergen|liseli|ortaokul)\b/i, reason: 'ReÅŸit olmayan kiÅŸilerle ilgili cinsel iÃ§erik desteklenmez.' },
    { re: /\b(rape|raped|non[-\s]?consensual|forced|coercion|blackmail|drugged|unconscious|sleeping|zorla|tecavÃ¼z|tecavuz|baygÄ±n|baygin|bilinci kapalÄ±|bilinci kapali)\b/i, reason: 'RÄ±za dÄ±ÅŸÄ± veya zorlayÄ±cÄ± cinsel iÃ§erik desteklenmez.' },
    { re: /\b(deepfake|celebrity nude|famous nude|real person nude|ex nude|revenge porn|leak nudes|ifÅŸa|ifsa|Ã¼nlÃ¼ Ã§Ä±plak|unlu ciplak|gerÃ§ek kiÅŸi Ã§Ä±plak|gercek kisi ciplak)\b/i, reason: 'GerÃ§ek kiÅŸiler, Ã¼nlÃ¼ler veya ifÅŸa/deepfake cinsel iÃ§erikleri desteklenmez.' },
    { re: /\b(hidden cam|voyeur|upskirt|downblouse|spy cam|gizli kamera|habersiz|rÄ±zasÄ±z Ã§ekim|rizasiz cekim)\b/i, reason: 'Gizli Ã§ekim veya rÄ±za dÄ±ÅŸÄ± mahremiyet ihlali desteklenmez.' }
  ];
...
app.get('/api/local-providers/status', async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, error: err.message || 'Local provider status okunamadÄ±' });
  }
});
...
function mergeNegativePrompts(...parts) {
  return compactPromptParts(parts.flatMap(part => String(part || '').split(',').map(x => x.trim())));
}

function buildImageIntentGuidance(originalPrompt) {
  const raw = String(originalPrompt || '').trim();
  const lower = normalizeIntentText(raw);
  const requestedPeople = /\b(kisi|insan|adam|erkek|kadin|kiz|portre|person|people|man|woman|girl|boy|model|portrait)\b/i.test(lower);
  const requestedAnimal = /\b(kedi|kopek|at|kus|hayvan|animal|cat|dog|horse|bird)\b/i.test(lower);
  const requestedVehicle = /\b(araba|otomobil|motor|kamyon|bisiklet|vehicle|car|motorcycle|truck|bike)\b/i.test(lower);
  const requestedTextOrLogo = /\b(logo|amblem|marka|yazi|text|typography|wordmark|poster|afis)\b/i.test(lower);
  const requestedScene = /\b(manzara|sokak|oda|ev|ofis|orman|park|deniz|dag|sehir|landscape|street|room|office|forest|beach|mountain|city)\b/i.test(lower);
  const requestedObject = /\b(urun|nesne|obje|parca|yedek|makine|cihaz|aksesuar|taki|ayakkabi|kiyafet|product|object|item|part|spare|component|device|machine|tool|accessory)\b/i.test(lower);
  const requestedStyle = /\b(realistic|photoreal|fotogercek|fotograf|anime|3d|cinematic|sinematik|illustration|illustrasyon|vector|vektor|minimal|render|katalog|catalog|ecommerce)\b/i.test(lower);
  const wantsExactCount = /\b(1|bir|2|iki|3|uc|4|dort|5|bes|one|two|three|four|five)\b/i.test(lower);
  const positive = [
    'strict prompt adherence',
    'depict the exact subject requested by the user prompt',
    'keep the requested action, relationship, count, setting, style, colors, materials, camera angle, and composition',
    'the main requested subject must be clearly visible and centered as the focus',
    'coherent realistic composition, accurate details, no random theme drift'
  ];
  const negative = [
    'wrong subject',
    'unrelated scene',
    'random objects',
    'extra unrequested elements',
    'missing requested subject',
    'changed action',
    'generic pose',
    'abstract filler',
    'fantasy scene unless requested',
    'watermark',
    'signature',
    'low quality',
    'blurry'
  ...
  if (requestedObject && !requestedPeople && !requestedAnimal) {
    positive.push('catalog product photography, clean studio lighting, isolated main object, accurate shape and material, commercial product photo');
    negative.push('person holding object unless requested', 'hands unless requested', 'random background scene', 'duplicate objects unless requested');
  }
  if ((requestedScene || requestedVehicle) && !requestedPeople) {
    positive.push('preserve the requested environment and subject scale, no people unless explicitly requested');
    negative.push('unrequested person', 'unrequested portrait', 'cropped unrelated face');
  }
  if (requestedTextOrLogo) {
    positive.push('clean graphic design, readable intended text only when text is requested, no extra random letters');
    negative.push('misspelled random text', 'extra words', 'fake brand text unless requested');
  }
  if (wantsExactCount) {
    positive.push('respect the exact requested number of subjects or objects');
    negative.push('wrong number of subjects', 'extra duplicate subject', 'missing subject');
  }
  if (!requestedStyle) {
    positive.push('natural believable visual style, not surreal unless requested');
    negative.push('surreal distortion', 'overly stylized unless requested');
  }

  return {
    prompt: positive.join('. ') + '.',
    negativePrompt: mergeNegativePrompts(negative),
    tags: { requestedPeople, requestedAnimal, requestedVehicle, requestedObject, requestedScene, requestedTextOrLogo }
  };
}

function buildIntentAwarePrompt(originalPrompt) {
  const raw = String(originalPrompt || '').trim();
  const intent = buildImageIntentGuidance(raw);
  return [raw, 'Image intent lock: ' + intent.prompt].filter(Boolean).join('\n');
}

async function callFooocusLocalImage({ prompt, imageSize, negativePrompt = '' }) {
  ...
  const response = await fetch(`${base}/v1/generation/text-to-image`, {
    ...
    body: JSON.stringify({
      prompt,
      negative_prompt: mergeNegativePrompts(negativePrompt, fromEnv('LOCAL_SD_NEGATIVE_PROMPT', 'low quality, blurry, watermark, text, logo, bad anatomy, deformed')),
...
async function callStableDiffusionWebUiImage({ prompt, imageSize, envName, fallbackBase, provider, model, negativePrompt = '' }) {
  ...
  const response = await fetch(`${base}/sdapi/v1/txt2img`, {
    ...
    body: JSON.stringify({
      prompt,
      negative_prompt: mergeNegativePrompts(negativePrompt, fromEnv('LOCAL_SD_NEGATIVE_PROMPT', 'low quality, blurry, watermark, text, logo, bad anatomy, deformed, extra limbs, missing limbs, bad hands, bad feet')),
      width: size.width,
      height: size.height,
...
async function callSwarmUiLocalImage({ prompt, imageSize, negativePrompt = '' }) {
  ...
  const response = await fetch(`${base}/API/GenerateText2Image`, {
    ...
    body: JSON.stringify({
      prompt,
      negativeprompt: mergeNegativePrompts(negativePrompt, fromEnv('LOCAL_SD_NEGATIVE_PROMPT', 'low quality, blurry, watermark, text, logo, bad anatomy, deformed')),
      width: size.width,
      height: size.height,
...
async function callModalImage({ prompt, imageSize, qualityMode = 'quality', model = 'modal-sdxl', negativePrompt = '' }) {
  ...
  const response = await fetch(MODAL_IMAGE_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      negative_prompt: mergeNegativePrompts(negativePrompt, fromEnv('MODAL_IMAGE_NEGATIVE_PROMPT', 'low quality, blurry, watermark, text, logo, deformed anatomy, extra limbs, missing limbs'))
    }),
    signal: AbortSignal.timeout(360000)
  });
  const data = await response.json().catch(() => ({}));
...
function buildComfyPromptHints(prompt) {
  ...
  const positive = [];
  const negative = [];
  const wantsTwoWomen = /\b(2|iki|two)\b/.test(lower) && /\b(kad[Ä±i]n|woman|women|female)\b/i.test(lower);
  const wantsFeet = /\b(ayak|ayaklar|foot|feet|barefoot)\b/i.test(lower);
  const wantsInteraction = /\b(yal\w*|lick\w*|kiss\w*|kissing|Ã¶p\w*|op\w*|birbirinin|each other|mutual)\b/i.test(lower);
  ...
  if (/orman|park|bahÃ§e|bahce|dÄ±ÅŸ mekan|dis mekan|outdoor|garden|forest/i.test(lower)) {
...
function buildComfyWorkflow(prompt, imageSize, qualityMode = 'quality', extraNegativePrompt = '') {
...
async function getLocalProviderStatus(options = {}) {
  ...
  const skippedPing = { ready: false, checkedPath: null, error: 'CanlÄ± ortamda PC localhost saÄŸlayÄ±cÄ±sÄ± pinglenmedi' };
  ...
  const errors = {};
  if (!comfyPing.ready) errors.comfyui = comfyPing.error || 'ComfyUI yanÄ±t vermedi';
  if (!fooocusPing.ready) errors.fooocus = fooocusPing.error || 'Fooocus yanÄ±t vermedi';
...
async function callComfyLocalImage({ prompt, imageSize, qualityMode = 'quality', negativePrompt = '' }) {
  ...
  const startRes = await fetch(`${base}/prompt`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ client_id: clientId, prompt: buildComfyWorkflow(prompt, imageSize, qualityMode, negativePrompt) }),
...
async function callCloudflareImageModel({ modelId, prompt, imageSize }) {
  ...
  if (!config) throw new Error('Bilinmeyen Cloudflare gÃ¶rsel modeli: ' + modelId);
  if (!CLOUDFLARE_ACCOUNT_ID || !CLOUDFLARE_API_TOKEN) throw new Error('Cloudflare image key yok');
  ...
  if (contentType.includes('application/json')) {
    ...
    if (typeof imageValue === 'string' && /^https?:\/\//i.test(imageValue)) {
      ...
      if (!dl.ok) throw new Error(config.name + ' gÃ¶rsel linki indirilemedi (' + dl.status + ')');
      buffer = Buffer.from(await dl.arrayBuffer());
    } else {
...
function shouldResearchImagePrompt(prompt) {
  ...
  const words = p.match(/\b[A-ZÃ‡?Ä°Ã–?Ãœ][\wÃ‡?Ä°Ã–?ÃœÃ§ÄŸÄ±Ã¶ÅŸÃ¼-]{2,}\b/g) || [];
...
async function buildImagePromptForQuality(originalPrompt, model, options = {}) {
  ...
  const intentGuidance = buildImageIntentGuidance(raw);
  if (options.raw === true || rawModels.has(modelKey) || MODAL_IMAGE_MODEL_ALIASES.has(modelKey)) {
    return {
      prompt: buildIntentAwarePrompt(raw),
      originalPrompt: raw,
      referenceUsed: false,
      referenceSnippets: [],
      rawPrompt: true,
      intentLock: true,
      intentTags: intentGuidance.tags,
      negativePrompt: intentGuidance.negativePrompt,
      model
    };
  }
  ...
  const finalPrompt = [
    ...
    comfyEnglishBridge,
    styleHint,
    'Image intent lock: ' + intentGuidance.prompt,
    quality.join(', ') + '.'
  ].filter(Boolean).join('\n');
  return {
    prompt: finalPrompt,
...
function stripProviderNotice(text) {
  return String(text || '')
    .replace(/âš \s*IMPORTANT NOTICE\s*âš [\s\S]*?(?:continue to work normally\.|normally\.)/gi, '')
    .replace(/âš \s*IMPORTANT NOTICE\s*âš /gi, '')
...
async function groqFallbackChat(messages, maxTokens, fallbackModel = 'llama-3.1-8b-instant') {
  if (!GROQ_KEY) throw new Error('Groq API key tanÄ±mlÄ± deÄŸil');
  ...
  if (fbJson.error && fallbackModel !== 'llama-3.1-8b-instant') {
    // Groq key havuzu varsa kÄ±sÄ±tlÄ±/limitli keyden diÄŸerine geÃ§.
...
app.get('/api/health', async (req, res) => {
  ...
  res.json({
    ...
    monitoring: {
      sentry: Boolean(Sentry && SENTRY_DSN),
      release: SENTRY_RELEASE,
      environment: process.env.SENTRY_ENVIRONMENT || process.env.NODE_ENV || 'production'
    },
    database: dbStats,
    databaseStorage: {
      persistent: DATABASE_IS_PERSISTENT,
      source: DATABASE_IS_PERSISTENT ? 'railway-volume' : 'local-app-directory'
    },
    models: {
    ...
    imageProviderStatus: {
      ...
      pollinations: {
        ...
        keys: publicPollinationsKeyStatus(pollinationsStatus),
...
app.get('/api/model-check', async (req, res) => {
  ...
  const now = Date.now();
  if (!deep && modelCheckCacheV477.data && now - modelCheckCacheV477.at < 120000) {
    return res.json({ ...modelCheckCacheV477.data, cached: true, cacheAgeSeconds: Math.round((now - modelCheckCacheV477.at) / 1000), deep: false });
  }
  ...
  if (CLOUDFLARE_ACCOUNT_ID && CLOUDFLARE_API_TOKEN) {
    imgPing.push(tryCheck('image:cloudflare', async (signal) => {
      const status = deep ? await getCloudflareImageGenerationStatus(30000) : await getCloudflareImageReadyStatus(2500);
      if (!status.ready) throw new Error(status.detail || 'Cloudflare check failed');
      return status.detail || (deep ? 'generation ok' : 'reachable');
    }, deep ? 35000 : 3500));
  }
  if (RUNWARE_API_KEY) {
  ...
  if (!deep) modelCheckCacheV477 = { at: Date.now(), data: payload };
  res.json(payload);
});
      ...
      if (payload && typeof payload === 'object' && !payload.error) {
        ...
        if (localChatDidFallback && localChatFallbackFrom) {
          ...
          payload.fallbackReason = typeof upstreamFallback === 'string'
            ? `Pollinations yanÄ±t vermedi, yedek model kullanÄ±ldÄ±: ${upstreamFallback}`
            : 'Pollinations yanÄ±t vermedi, yedek model kullanÄ±ldÄ±.';
        }
      }
    } catch (_) {}
...
app.post('/api/tts', chatLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!edgeVoice) {
      const hasTurkish = /[Ã§ÄŸÄ±Ã¶ÅŸÃ¼Ã‡?Ä°Ã–?Ãœ]/.test(cleanText) || /\b(bir|bu|ve|iÃ§in|ile|ya|da|mi|ne|ben|sen|biz|siz)\b/i.test(cleanText);
...
app.post('/api/image', chatLimiter, optionalAuthMiddleware, async (req, res) => {
  ...
  if (legacyImageAliasesNoFallback.has(originalImageModel)) {
    return res.status(400).json({
      error: 'Bu eski gÃ¶rsel model adÄ± artÄ±k baÅŸka modele yÃ¶nlendirilmiyor. LÃ¼tfen listeden Ã§alÄ±ÅŸan bir gÃ¶rsel modeli seÃ§.',
      model: imgModel,
      ok: false
    });
  }
  ...
  const imageMeta = {
    ...
    intentLock: promptMeta.intentLock,
    intentTags: promptMeta.intentTags,
    negativePrompt: promptMeta.negativePrompt
  };
  ...
  if (imgModel === 'comfyui-local') {
    try {
      const out = await callComfyLocalImage({ prompt, imageSize, qualityMode, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] ComfyUI local failed:', err.message);
      return res.status(503).json({ error: 'ComfyUI Ã§alÄ±ÅŸmÄ±yor. COMFYUI_API_URL veya http://127.0.0.1:8188 adresini kontrol et: ' + err.message, model: imgModel, provider: 'comfyui-local', ok: false });
    }
  }

  if (imgModel === 'fooocus-local') {
    try {
      const out = await callFooocusLocalImage({ prompt, imageSize, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] Fooocus local failed:', err.message);
      return res.status(503).json({ error: 'Fooocus Ã§alÄ±ÅŸmÄ±yor. FOOOCUS_API_URL veya http://127.0.0.1:8888 adresini kontrol et: ' + err.message, model: imgModel, provider: 'fooocus-local', ok: false });
    }
  }

  if (imgModel === 'a1111-local') {
    try {
      const out = await callStableDiffusionWebUiImage({ prompt, imageSize, envName: 'A1111_API_URL', fallbackBase: 'http://127.0.0.1:7860', provider: 'a1111-local', model: imgModel, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] A1111 local failed:', err.message);
      return res.status(503).json({ error: 'A1111 / Stable Diffusion WebUI Ã§alÄ±ÅŸmÄ±yor. --api ile http://127.0.0.1:7860 adresini kontrol et: ' + err.message, model: imgModel, provider: 'a1111-local', ok: false });
    }
  }

  if (imgModel === 'forge-local') {
    try {
      const out = await callStableDiffusionWebUiImage({ prompt, imageSize, envName: 'FORGE_API_URL', fallbackBase: 'http://127.0.0.1:7861', provider: 'forge-local', model: imgModel, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] Forge local failed:', err.message);
      return res.status(503).json({ error: 'Forge local Ã§alÄ±ÅŸmÄ±yor. --api ile FORGE_API_URL veya http://127.0.0.1:7861 adresini kontrol et: ' + err.message, model: imgModel, provider: 'forge-local', ok: false });
    }
  }

  if (imgModel === 'swarmui-local') {
    try {
      const out = await callSwarmUiLocalImage({ prompt, imageSize, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] SwarmUI local failed:', err.message);
      return res.status(503).json({ error: 'SwarmUI local Ã§alÄ±ÅŸmÄ±yor. SWARMUI_API_URL veya http://127.0.0.1:7801 adresini kontrol et: ' + err.message, model: imgModel, provider: 'swarmui-local', ok: false });
    }
  }

  if (MODAL_IMAGE_MODEL_ALIASES.has(String(imgModel || '').toLowerCase())) {
    try {
      const out = await callModalImage({ prompt, imageSize, qualityMode, model: imgModel, negativePrompt: promptMeta.negativePrompt });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
      console.warn('[IMAGE] Modal image failed:', err.message);
      return res.status(503).json({ error: 'Modal GPU gÃ¶rsel endpoint hazÄ±r deÄŸil veya hata verdi: ' + err.message, model: imgModel, provider: 'modal', ok: false });
    }
  }
  ...
  if (explicitOpenAIImage) {
    try {
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({
          error: 'SeÃ§ili OpenAI gÃ¶rsel modeli ÅŸu an yanÄ±t vermedi: ' + err.message,
          model: imgModel,
          provider: 'openai',
          ok: false
        });
  ...
  // â?€â?€ RUNWARE (Flux/SDXL â€? free tier, quality for adult mood / editorial) â?€â?€â?€
  if (imgModel === 'runware-flux' || imgModel === 'runware-sdxl') {
    const rwKey = overrideKey || RUNWARE_API_KEY;
    if (!rwKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili Runware gÃ¶rsel modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'runware', ok: false });
      }
      imgModel = 'flux';
    } else try {
      ...
      if (!imgUrl) throw new Error('Runware URL boÅŸ');
      const dlRes = await fetch(imgUrl, { signal: AbortSignal.timeout(60000) });
      if (!dlRes.ok) throw new Error('Runware gÃ¶rsel indirilemedi');
      const buf = Buffer.from(await dlRes.arrayBuffer());
      const genDir = GENERATED_DIR;
      if (!fs.existsSync(genDir)) fs.mkdirSync(genDir, { recursive: true });
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili Runware gÃ¶rsel modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'runware', ok: false });
      }
      imgModel = IMAGEGPT_API_KEY ? 'imagegpt-free' : 'flux';
    }
  }
  ...
  if (imgModel === 'stability-core' || imgModel === 'stability-ultra') {
    ...
    if (!stabilityKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili Stability modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'stability', ok: false });
      }
      imgModel = 'flux';
    } else try {
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili Stability modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'stability', ok: false });
      }
      imgModel = 'flux';
    }
  }
  ...
  if (imgModel === 'together-flux' || TOGETHER_IMAGE_MODELS[imgModel]) {
    ...
    if (!togetherKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili Together gÃ¶rsel modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'together', ok: false });
      }
      imgModel = IMAGEGPT_API_KEY ? 'imagegpt-free' : 'flux';
    } else try {
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili Together gÃ¶rsel modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'together', ok: false });
      }
      imgModel = IMAGEGPT_API_KEY ? 'imagegpt-free' : 'flux';
    }
  }

  // â?€â?€ AIML API (Nano Banana / Flux â€? unified, good for quality adult mood) â?€â?€â?€
  if (imgModel === 'aiml-flux' || imgModel === 'aiml-nano') {
    const aimlKey = overrideKey || AIMLAPI_KEY;
    if (!aimlKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili AIML gÃ¶rsel modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'aimlapi', ok: false });
      }
      imgModel = 'flux';
    } else try {
      ...
      if (!imgUrl) throw new Error('AIML URL boÅŸ');
      let buf;
      if (typeof imgUrl === 'string' && imgUrl.startsWith('http')) {
        const dl = await fetch(imgUrl);
        if (!dl.ok) throw new Error('AIML gÃ¶rsel indirilemedi');
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili AIML gÃ¶rsel modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'aimlapi', ok: false });
      }
      imgModel = IMAGEGPT_API_KEY ? 'imagegpt-free' : 'flux';
    }
  }
  ...
  if (imgModel === 'imagegpt-free') {
    ...
    if (!imageGptKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili ImageGPT modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'imagegpt', ok: false });
      }
      imgModel = 'flux';
    } else try {
      const igRes = await fetch('https://api.imagegpt.online/generate/text-image', {
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili ImageGPT modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'imagegpt', ok: false });
      }
      imgModel = 'flux';
    }
  }
  ...
  if (CLOUDFLARE_IMAGE_MODELS[imgModel]) {
    if (!CLOUDFLARE_ACCOUNT_ID || !CLOUDFLARE_API_TOKEN) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'SeÃ§ili Cloudflare gÃ¶rsel modeli iÃ§in API key tanÄ±mlÄ± deÄŸil.', model: imgModel, provider: 'cloudflare', ok: false });
      }
      imgModel = 'flux';
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'SeÃ§ili Cloudflare gÃ¶rsel modeli ÅŸu an yanÄ±t vermedi: ' + err.message, model: imgModel, provider: 'cloudflare', ok: false });
      }
      imgModel = 'flux';
  ...
  if (POLLINATIONS_IMAGE_MODEL_ALIASES[imgModel] || ['turbo', 'sana', 'klein', 'wan-image', 'qwen-image', 'flux-realism', 'flux-anime', 'flux-3d'].includes(imgModel) || imgModel.startsWith('style-')) {
    ...
    if (!POLLINATIONS_API_KEYS.length) {
      return res.status(503).json({
        error: 'Pollinations gÃ¶rsel modeli iÃ§in POLLINATIONS_API_KEY gerekli. Yeni Pollinations API artÄ±k token istiyor.',
        model: finalModel,
        provider: 'pollinations',
        ok: false
      });
    ...
    if (cooldown.coolingDown) {
      lastErr = new Error(`Pollinations ${finalModel} kÄ±sa sÃ¼reli beklemede (${Math.ceil(cooldown.remainingMs / 1000)} sn). Model DeÄŸiÅŸtir veya Ã‡alÄ±ÅŸan Modele GeÃ§ kullan.`);
    } else {
      try {
            ...
            try {
              ...
              if (!response.ok && ct.includes('application/json')) {
                ...
                if (response.status === 402 && /insufficient balance/i.test(errMessage)) {
                  ...
                  errMessage = `Pollinations kuyruÄŸu dolu. ${errMessage}`;
                } else if (response.status === 401) {
                  errMessage = `Pollinations API key geÃ§ersiz veya eksik. ${errMessage}`;
    ...
    // Pollinations retry baÅŸarÄ±sÄ±zsa yalnÄ±zca auto-quality iÃ§in baÅŸka gerÃ§ek saÄŸlayÄ±cÄ± denenir.
    ...
    return res.status(502).json({
      error: 'SeÃ§ili gÃ¶rsel saÄŸlayÄ±cÄ±sÄ± ÅŸu an yanÄ±t vermedi: ' + (lastErr?.message || 'bilinmeyen hata'),
      prompt: finalPrompt,
      model: finalModel,
  ...
  try {
    ...
    return res.status(502).json({
      error: 'SeÃ§ili gÃ¶rsel saÄŸlayÄ±cÄ±sÄ± ÅŸu an yanÄ±t vermedi: ' + err.message,
      prompt,
      model: imgModel,
...
app.get('/api/img-proxy', async (req, res) => {
  ...
  try {
    ...
    return res.status(503).json({ error: err.message || 'GÃ¶rsel Ã¼retilemedi, lÃ¼tfen tekrar deneyin.' });
  }
});
    ...
    try {
      ...
      if (fromEnv('HF_TOKEN') && model !== 'hf-ltx' && model !== 'ltx-video') {
        try {
          ...
          if (buf.length < 1000) throw new Error(`HF video Ã§ok kÃ¼Ã§Ã¼k yanÄ±t (${buf.length} bytes)`);
          const genDir = GENERATED_DIR;
          if (!fs.existsSync(genDir)) fs.mkdirSync(genDir, { recursive: true });
...
app.post('/api/imagen', chatLimiter, async (req, res) => {
  ...
  try {
    ...
    res.status(503).json({
      error: 'GÃ¶rsel Ã¼retimi ÅŸu an kullanÄ±lamÄ±yor: ' + err.message,
...
function buildSupportBotFallback(message) {
  ...
  if (/fiyat|paket|kredi|Ã¼cret|ucret|para|satÄ±n|satin|maÄŸaza|magaza/.test(s)) {
    return 'Paketleri MaÄŸaza bÃ¶lÃ¼mÃ¼nden inceleyebilirsin. Froxy AI kredi sistemiyle Ã§alÄ±ÅŸÄ±r; sohbet, gÃ¶rsel Ã¼retim ve araÃ§ kullanÄ±mÄ± seÃ§ilen modele gÃ¶re kredi harcar.';
  }
  if (/giriÅŸ|giris|kayÄ±t|kayit|kod|otp|mail|ÅŸifre|sifre|doÄŸrulama|dogrulama/.test(s)) {
    return 'Mail/ÅŸifre ile giriÅŸ ve kayÄ±t akÄ±ÅŸÄ±nda doÄŸrulama kodu gerekir. Kod gelmezse spam klasÃ¶rÃ¼nÃ¼ kontrol et, tekrar kod iste ve e-posta adresini doÄŸru yazdÄ±ÄŸÄ±ndan emin ol.';
  }
  if (/gÃ¶rsel|gorsel|foto|resim|image|galeri|dÃ¼zenle|duzenle|model/.test(s)) {
    return 'GÃ¶rsel Ã¼retim veya fotoÄŸraf dÃ¼zenleme iÃ§in model, kalite modu ve oran seÃ§ebilirsin. DÃ¼zenleme modunda yalnÄ±zca edit destekli modeller Ã§alÄ±ÅŸÄ±r; hata alÄ±rsan seÃ§ili modeli ve promptu destek talebine ekle.';
  }
  if (/Ã¶deme|odeme|fatura|iade|kart|shopier|dodo|paytr|iyzico/.test(s)) {
    return 'Ã–deme, fatura ve iade gibi Ã¶zel konularda gÃ¼venli iÅŸlem iÃ§in destek talebi aÃ§manÄ± Ã¶neririm. Kart veya Ã¶zel Ã¶deme bilgilerini hÄ±zlÄ± sohbet alanÄ±na yazma.';
  }
  if (/api|entegrasyon|key|anahtar|limit|saÄŸlayÄ±cÄ±|saglayici/.test(s)) {
    return 'API ve saÄŸlayÄ±cÄ± limitleri iÃ§in seÃ§ili model, hata mesajÄ± ve iÅŸlem zamanÄ±nÄ± destek talebine ekle. BÃ¶ylece hangi saÄŸlayÄ±cÄ± hattÄ±nda sorun olduÄŸunu daha hÄ±zlÄ± kontrol edebiliriz.';
  }
  return 'MesajÄ±nÄ± aldÄ±m. KÄ±sa kullanÄ±m sorularÄ±nda yardÄ±mcÄ± olabilirim; hesap, Ã¶deme veya Ã¶zel kullanÄ±cÄ± verisi gerekiyorsa gÃ¼venli olmasÄ± iÃ§in destek talebi aÃ§manÄ± Ã¶neririm.';
}
...
app.post('/api/support-bot', chatLimiter, optionalAuthMiddleware, async (req, res) => {
  try {
    ...
    if (!message) return res.status(400).json({ error: 'Mesaj boÅŸ olamaz.' });
    if (message.length > 700) return res.status(400).json({ error: 'Mesaj Ã§ok uzun. LÃ¼tfen daha kÄ±sa yaz.' });
          ...
          messages: [
            {
              role: 'system',
              content: [
                'Sen Froxy AI sitesinin TÃ¼rkÃ§e hÄ±zlÄ± destek asistanÄ±sÄ±n.',
                'KÄ±sa, net ve sakin cevap ver.',
                'Froxy AI: tek panelde sohbet modelleri, gÃ¶rsel Ã¼retim, fotoÄŸraf dÃ¼zenleme, AI araÃ§larÄ±, promptlar, kredi sistemi ve destek sunar.',
                'Ã–deme, fatura, iade, hesap gÃ¼venliÄŸi veya Ã¶zel kullanÄ±cÄ± verisi gereken konularda kesin iÅŸlem sÃ¶zÃ¼ verme; kullanÄ±cÄ±yÄ± destek talebi aÃ§maya veya destek@froxyai.com adresine yÃ¶nlendir.',
                'API anahtarÄ±, provider metadata, fallback detayÄ± veya gizli sistem bilgisini asla yazma.',
                'YanÄ±t en fazla 4 kÄ±sa cÃ¼mle olsun.'
              ].join(' ')
            },
            {
              role: 'user',
              content: `Sayfa: ${route}\nKullanÄ±cÄ± planÄ±: ${String(user.plan || 'bilinmiyor').slice(0, 32)}\nMesaj: ${message}`
            }
          ]
        }),
    ...
    return res.status(500).json({ error: 'Destek asistanÄ± ÅŸu an yanÄ±t veremedi. LÃ¼tfen tekrar dene.' });
  }
});
  ...
  app.use((err, req, res, next) => {
    Sentry.withScope(scope => {
      ...
      scope.setContext('request_safe', {
        ...
        path: req.path,
        query: scrubSentryObject(req.query || {}),
        body: scrubSentryObject(req.body || {})
      });
      if (req.user && req.user.id) scope.setUser({ id: String(req.user.id) });
    ...
    return res.status(status).json({ error: status >= 500 ? 'Sunucu hatası kaydedildi.' : (err.message || 'İstek işlenemedi.') });
  });
}
...
<body>
  ...
  <div class="container">
    ...
    <p>AradÄ±ÄŸÄ±nÄ±z sayfa taÅŸÄ±ndÄ±, silindi veya hiÃ§ var olmadÄ±. Ana sayfaya dÃ¶nerek devam edebilirsiniz.</p>
