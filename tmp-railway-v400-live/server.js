...
function repairTurkishMojibake(value) {
  ...
  if (!/[ÃÄÅâ�]|\?[a-zA-ZğüşıöçĞÜŞİÖÇ]/.test(value)) return value;
  const pairs = [
    ['Ã‡', 'Ç'], ['Ã§', 'ç'], ['Ã–', 'Ö'], ['Ã¶', 'ö'], ['Ãœ', 'Ü'], ['Ã¼', 'ü'],
    ['Ä°', 'İ'], ['Ä±', 'ı'], ['Ä', 'Ğ'], ['Äž', 'Ğ'], ['ÄŸ', 'ğ'],
    ['Å', 'Ş'], ['Åž', 'Ş'], ['ÅŸ', 'ş'], ['â€™', "'"], ['â€˜', "'"],
    ['â€œ', '"'], ['â€�', '"'], ['â€“', '-'], ['â€”', '-'], ['â€¦', '...'],
    ['â†’', '->'], ['â†', '<-'], ['â‚º', 'TL']
  ];
...
Object.assign(SEO_CONTENT, {
  ...
  '/fiyatlandirma': {
    ...
    sections: [
      ...
      ['Åeffaf kullanÄ±m', 'Kredi sistemi, model ve iÅŸlem tÃ¼rÃ¼ne gÃ¶re kullanÄ±m maliyetini daha anlaÅŸÄ±lÄ±r hale getirmeyi hedefler.']
    ],
    faq: [
...
const SEO_CLEAN_REPLACEMENTS = [
  ['gÃ¶rsel', 'gÃ¶rsel'], ['GÃ¶rsel', 'GÃ¶rsel'], ['Ã¼cretsiz', 'Ã¼cretsiz'], ['Ãœcretsiz', 'Ãœcretsiz'],
  ['baÅŸ', 'baÅŸ'], ['BaÅŸ', 'BaÅŸ'], ['araÃ§', 'araÃ§'], ['AraÃ§', 'AraÃ§'], ['TÃ¼rkÃ§e', 'TÃ¼rkÃ§e'],
  ['Ã§', 'Ã§'], ['Ã‡', 'Ã‡'], ['Ä±', 'Ä±'], ['Ä°', 'Ä°'], ['ÄŸ', 'ÄŸ'], ['Ä', 'Ä'], ['ÅŸ', 'ÅŸ'], ['Å', 'Å'],
  ['Ã¶', 'Ã¶'], ['Ã–', 'Ã–'], ['Ã¼', 'Ã¼'], ['Ãœ', 'Ãœ'], ['Ã§', 'Ã§'], ['Ã‡', 'Ã‡'],
  ['â€™', 'â€™'], ['â€œ', 'â€œ'], ['â€', 'â€']
];
...
const POLLINATIONS_API_KEY   = fromEnv('POLLINATIONS_API_KEY') || fromEnv('POLLINATIONS_KEY');
...
const POLLINATIONS_IMAGE_MODEL_ALIASES = {
  ...
  'pollinations-gpt-image-2': 'gpt-image-2',
...
async function callModalImage({ prompt, imageSize, qualityMode = 'quality' }) {
...
async function pingLocalProvider(base, paths, timeoutMs = 2500) {
...
async function getLocalProviderStatus() {
  ...
  const [comfyPing, fooocusPing, a1111Ping, forgePing, swarmPing] = await Promise.all([
    pingLocalProvider(comfyBase, ['/system_stats', '/object_info']),
    pingLocalProvider(fooocusBase, ['/docs', '/', '/ping']),
    pingLocalProvider(a1111Base, ['/sdapi/v1/sd-models', '/docs', '/']),
    pingLocalProvider(forgeBase, ['/sdapi/v1/sd-models', '/docs', '/']),
    pingLocalProvider(swarmBase, ['/API/GetNewSession', '/', '/API/GetServerResourceInfo'])
  ]);
  const errors = {};
  ...
  return {
    ...
    modal: { ready: modalReady, baseUrl: MODAL_IMAGE_ENDPOINT || '', checkedPath: modalReady ? 'configured' : null },
    errors
  };
}
...
async function callCloudflareImageModel({ modelId, prompt, imageSize }) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`${config.name} HTTP ${response.status}: ${text.slice(0, 220)}`);
  }
  ...
  if (!buffer || buffer.length < 1000) throw new Error(config.name + ' boÅŸ/kÃ¼Ã§Ã¼k gÃ¶rsel dÃ¶ndÃ¼rdÃ¼');
  const savedUrl = saveGeneratedImageBuffer(buffer, 'cf', ext);
...
function cleanServerAssistantReply(text) {
  ...
  s = s.replace(/ÄŸÃ…Â¸[^ .,!?]{0,3}/g, '').replace(/ÃƒÂ¢[^ .,!?]{0,2}/g, '');
...
app.get('/api/health', async (req, res) => {
  ...
  res.json({
    ...
    videoProviders: {
      ...
      note: 'Pollinations video genellikle POLLINATIONS_KEY ister; anonim kullanÄ±m IP kuyruÄŸuna takÄ±labilir.'
    },
    imageProviders: {
...
app.post('/api/chat', chatLimiter, optionalAuthMiddleware, async (req, res) => {
  ...
  if (provider === 'pollinations') {
    ...
    p = fallback.p;
    key = fallback.key;
    model = fallback.provider === 'groq' ? 'openai/gpt-oss-20b' : fallback.model;
    url = provider === 'pollinations' ? 'https://text.pollinations.ai/openai' : p.base + '/chat/completions';
  }
...
app.post('/api/image', chatLimiter, optionalAuthMiddleware, async (req, res) => {
  ...
  const legacyImageAliasesNoFallback = new Set(['pollinations', 'wan-image', 'qwen-image', 'klein', 'sana', 'turbo', 'flux-realism', 'flux-anime', 'flux-3d']);
  ...
  if (imgModel === 'modal-sdxl') {
    try {
      const out = await callModalImage({ prompt, imageSize, qualityMode });
      return res.json({ ...out, prompt, provider: out.provider, ...imageMeta });
    } catch (err) {
  ...
  if (imgModel === 'perchance-experimental') {
    try {
      ...
      try {
        const out = await callPollinationsImageFallback({ prompt, imageSize, fallbackFrom: imgModel, fallbackReason: err.message });
        return res.json({ ...out, ...imageMeta });
      } catch (fbErr) {
        return res.status(503).json({ error: 'Perchance experimental baglantisi hazir degil ve fallback de basarisiz: ' + err.message + ' | fallback: ' + fbErr.message, model: imgModel, provider: 'perchance-experimental', ok: false });
      }
    }
  }
  ...
  if (explicitOpenAIImage) {
    try {
      ...
      if (model !== 'auto-quality') {
        try {
          const fallbackOut = IMAGEGPT_API_KEY
            ? await callImageGPTFreeImage({ prompt })
            : await callTogetherFallbackImage({ prompt, imageSize });
          return res.json({ ...fallbackOut, fallbackFrom: imgModel, fallbackReason: err.message, ...imageMeta });
        } catch (fallbackErr) {
          console.warn('[IMAGE] OpenAI fallback failed:', fallbackErr.message);
        }
        return res.status(502).json({
  ...
  if (POLLINATIONS_IMAGE_MODEL_ALIASES[imgModel] || ['turbo', 'sana', 'klein', 'wan-image', 'qwen-image', 'flux-realism', 'flux-anime', 'flux-3d'].includes(imgModel) || imgModel.startsWith('style-')) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        const imgUrl = `https://gen.pollinations.ai/image/${encodeURIComponent(finalPrompt)}?model=${encodeURIComponent(finalModel)}&width=${imageSize.width}&height=${imageSize.height}&nologo=true&seed=${seed}&key=${encodeURIComponent(POLLINATIONS_API_KEY)}`;
        const controller = new AbortController();
...
app.post('/api/deduct-image-credit', authMiddleware, (req, res) => {
  ...
  } else if (m === 'comfyui-local' || m === 'fooocus-local') {
    cost = 1;
