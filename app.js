...
function renderModelPickerStatus(currentModel){
  ...
  host.innerHTML=`<div class="mp-status-main">
    <span class="mp-status-orb">${esc(providerLabel(provider)).slice(0,2).toUpperCase()}</span>
...
function renderModelPicker(filter){
  ...
  if(pickerOpen&&!modelCatalogLoaded && ALL_MODELS.length<REMOTE_MODEL_TARGET_COUNT){
    ...
    listEl.innerHTML='<div class="mp-item mp-loading-row" style="min-height:76px;pointer-events:none"><div class="mp-item-icon"><span>AI</span></div><div class="mp-item-info"><div class="mp-item-name">Model kataloğu yükleniyor</div><div class="mp-item-meta"><span class="mp-badge mp-badge-free">'+REMOTE_MODEL_TARGET_COUNT.toLocaleString('tr-TR')+' model</span><span>Sağlayıcılar hazırlanıyor</span></div></div></div>';
  ...
  const providerCats=Object.entries(providerCounts).sort((a,b)=>b[1]-a[1]).slice(0,7).map(([p,count])=>({id:'provider:'+p,label:providerLabel(p),icon:'globe',count}));
  const allCats=[...quickCats,{id:'fav',label:'Favoriler',icon:'sparkles',count:favorites.length},...providerCats];
  catsEl.innerHTML=allCats.map(cat=>'<button type="button" class="mp-cat '+(mpActiveCat===cat.id?'active':'')+'" onclick="mpActiveCat=\''+jsStr(cat.id)+'\';renderModelPicker(document.getElementById(\'mp-search\')?.value)">'+figIcon(cat.icon,'inline')+' <span>'+esc(cat.label)+'</span><em class="mp-cat-count">'+Number(cat.count||0).toLocaleString('tr-TR')+'</em></button>').join('');
  let filtered=models;
  ...
  if(filter){const q=filter.toLowerCase();filtered=filtered.filter(m=>m.name.toLowerCase().includes(q)||m.id.toLowerCase().includes(q)||providerLabel(modelProviderKey(m)).toLowerCase().includes(q)||modelUseCase(m).toLowerCase().includes(q))}
  if(countEl)countEl.textContent=modelCountLabel();
  const sel=document.getElementById('model-sel');
  const currentModel=sel?.value||'';
  renderModelPickerStatus(currentModel);
  if(!filtered.length){listEl.innerHTML='<div class="mp-empty-state">Model bulunamadı</div>';return}
  listEl.innerHTML=filtered.map((m,i)=>{
    ...
    return '<div class="mp-item '+(sel2?'selected':'')+'" data-model-id="'+esc(m.id)+'" role="button" tabindex="0" onclick="selectModel(\''+jsStr(m.id)+'\')" style="animation:fadeSlideUp .2s '+(Math.min(i,24)*0.012)+'s both"><button type="button" class="mp-star '+(fav?'on':'')+'" onclick="toggleModelFavorite(\''+jsStr(m.id)+'\',event)" aria-label="'+(fav?'Favoriden çıkar':'Favoriye ekle')+'" title="Favori"><span class="mp-star-dot" aria-hidden="true"></span></button><div class="mp-item-icon"><span>'+esc(providerLabel(provider)).slice(0,2).toUpperCase()+'</span></div><div class="mp-item-info"><div class="mp-item-name">'+esc(m.name)+'</div><div class="mp-item-meta">'+badge+'<span>'+esc(providerLabel(provider))+'</span><span>'+esc(modelUseCase(m))+'</span></div></div><div class="mp-item-status"><span>'+esc(status)+'</span>'+(sel2?'<b>Aktif</b>':'')+'</div></div>';
  }).join('');
}
