...
async function doAuth(type) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if(localMatch){await fallbackLocalAuth(type);return}
      if(errDiv){errDiv.textContent = data.error || 'Giriş/kayıt başarısız.';errDiv.style.display = 'block'}
    }
  } catch (e) {
    if(e.apiUnavailable || e instanceof TypeError){
...
function updDash(){
  ...
  if(ml){
    ...
    const featured=models.slice(0,48);
...
function renderModelPicker(filter){
  ...
  catsEl.innerHTML='<div class="mp-cat '+(mpActiveCat==='all'?'active':'')+'" onclick="mpActiveCat=\'all\';renderModelPicker(document.getElementById(\'mp-search\')?.value)">'+figIcon('globe','inline')+' T\u00fcm\u00fc<span class="mp-cat-count">'+models.length+'</span></div>';
  catsEl.innerHTML+='<div class="mp-cat mp-cat-fav '+(mpActiveCat==='fav'?'active':'')+'" onclick="mpActiveCat=\'fav\';renderModelPicker(document.getElementById(\'mp-search\')?.value)">'+figIcon('sparkles','inline')+' Favoriler<span class="mp-cat-count">'+favorites.length+'</span></div>';
  Object.keys(CAT_INFO).forEach(c=>{
    if(!catCounts[c])return;
    const ci=CAT_INFO[c];
    catsEl.innerHTML+='<div class="mp-cat mp-cat-'+c+' '+(mpActiveCat===c?'active':'')+'" onclick="mpActiveCat=\''+c+'\';renderModelPicker(document.getElementById(\'mp-search\')?.value)">'+figIcon(ci.icon,'inline')+' '+ci.label+'<span class="mp-cat-count">'+catCounts[c]+'</span></div>';
  });
  let filtered=models;
  ...
  if(countEl)countEl.textContent=filtered.length+'/'+models.length+' model';
  const sel=document.getElementById('model-sel');
...
function renderMsgs(opts={}){
  ...
  c.messages.forEach((m,idx)=>{
    ...
    row.innerHTML=`<div class="msg-ava">${ava}</div><div class="msg-body"><div class="msg-name">${m.role==='user'?esc(user.name):aiName}</div><div class="msg-text">${formatMsg(m.content)}</div><div class="msg-actions">${editBtn}${feedbackBtns}${regenBtn}${copyMsgBtn}${ttsBtn}${refineBtns}</div></div>`;
...
function adminSetApiState(mode){
  ...
  if(authMode)authMode.textContent=mode==='api'?'JWT backend':'Local fallback';
}
...
function getUnifiedImageGallery(){
  ...
  const saved=getImageGallery();
  const seen=new Set();
  return [...history,...saved].filter(x=>{
...
function renderImageHistory(){
  ...
  box.innerHTML=items.map((item,i)=>{
    ...
    return `<div class="img-history-card pro-history-card">
      <img src="${esc(imageUrlForDisplay(item.url))}" alt="Üretilen görsel" loading="lazy" onerror="this.parentElement.style.display='none'">
      <div class="img-history-info">
...
function renderImageGalleryPro(){
  ...
  const items=typeof getImageHistory==='function'?getImageHistory():[];
  host.innerHTML=`<div class="pro-section-head">
  ...
  <div class="pro-image-strip">${items.slice(0,6).map((x,i)=>`<button type="button" onclick="reuseImageHistory(${i})">
    <img src="${esc(x.url)}" alt=""><span>${esc((x.prompt||'Görsel').slice(0,44))}</span>
  </button>`).join('')||'<div class="pro-empty">İlk görsel üretiminden sonra galeri burada canlanacak.</div>'}</div>`;
}
