...
/* v323: final UI override for tools, prompts and support. Strings are escaped so Turkish stays stable. */
(function(){
  if(window.__froxyFinalSurfacesV323)return;
  window.__froxyFinalSurfacesV323=true;

  const T={
    toolsKicker:'AI ara\u00e7 merkezi',
    toolsTitle:'Satış sayfası, reklam ve içerik akışlarını AI ile üret',
    toolsDesc:'Brief yaz, aracı seç, çıktıyı sohbette hazır al. Landing page, kısa reklam videosu, Instagram mini site ve UI section promptları satışa dönük şekilde hazırlanır.',
    topicPh:'Örn: AI model aboneliği satan SaaS, hedef kitle freelancer ve ajanslar...',
    promptImprove:'Prompt iyile\u015ftir',
    supportBot:'Destek botu',
    liveFlow:'Canl\u0131 i\u015f ak\u0131\u015f\u0131',
    step1:'1. Brief al\u0131nd\u0131',
    step2:'2. Ara\u00e7 promptu kuruldu',
    step3:'3. Sohbete aktar\u0131ld\u0131',
    runnable:'\u00e7al\u0131\u015ft\u0131r\u0131labilir ara\u00e7',
    category:'kategori',
    freeModel:'free model',
    provider:'sa\u011flay\u0131c\u0131',
    workflows:'Haz\u0131r i\u015f ak\u0131\u015flar\u0131',
    workflowsDesc:'\u00c7al\u0131\u015ft\u0131r butonu konu alan\u0131n\u0131 kullan\u0131r; konu bo\u015fsa promptu sohbete haz\u0131r yazar.',
    run:'\u00c7al\u0131\u015ft\u0131r',
    copy:'Kopyala',
    ready:'haz\u0131r. Konuyu tamamlay\u0131p g\u00f6nderebilirsin.',
    copied:'Prompt kopyaland\u0131',
    copyFail:'Kopyalama desteklenmedi',
    imageReady:'G\u00f6rsel promptu haz\u0131rland\u0131. \u00dcret butonuna basabilirsin.',
    imageHint:'\u00dcr\u00fcn veya konu yazarsan prompt daha net olur.',
    ragHint:'Bilgi bankas\u0131na dosyan\u0131 y\u00fckle, sonra chat i\u00e7inde bu ak\u0131\u015f\u0131 kullanabilirsin.',
    market:'Prompt Market',
    marketTitle:'Haz\u0131r profesyonel ak\u0131\u015flar',
    marketDesc:'Sat\u0131\u015f, kod, destek, analiz ve g\u00f6rsel \u00fcretim i\u00e7in h\u0131zl\u0131 ba\u015flang\u0131\u00e7 \u015fablonlar\u0131.',
    goChat:'Sohbete ge\u00e7',
    templateMoved:'\u015eablon sohbete aktar\u0131ld\u0131',
    favEmpty:'Hen\u00fcz favori prompt yok. Kartlardaki y\u0131ld\u0131zdan favori ekleyebilirsin.',
    favAdd:'Prompt favorilere eklendi',
    favRemove:'Prompt favorilerden \u00e7\u0131kar\u0131ld\u0131',
    favAddTitle:'Favoriye ekle',
    favRemoveTitle:'Favoriden \u00e7\u0131kar',
    addChat:'T\u0131kla, sohbete ekle',
    directSupport:'Do\u011frudan destek',
    directSupportDesc:'Ticket d\u0131\u015f\u0131nda e-posta ile de ula\u015fabilirsin.'
  };
  ...
  const packs=[
    ...
    {id:'landing',cat:'Kod',icon:'code',title:'Landing Page Builder',desc:'\u00dcr\u00fcn fikrinden modern, responsive ve h\u0131zl\u0131 tek sayfa tasla\u011f\u0131 \u00fcretir.',target:'chat',prompt:'T\u00fcrk\u00e7e cevap ver. A\u015fa\u011f\u0131daki \u00fcr\u00fcn i\u00e7in modern, responsive, h\u0131zl\u0131 ve profesyonel tek dosyal\u0131k HTML/CSS/JS landing page kodu \u00fcret. Kod \u00e7al\u0131\u015ft\u0131r\u0131labilir olsun. \u00dcr\u00fcn: '},
  ...
  function selectedTopic(){return (document.getElementById('ai-tool-topic')?.value||'').trim();}
  function getPack(id){return packs.find(x=>x.id===id);}

  function runPack(tool){
    const topic=selectedTopic();
    if(tool.target==='img'){
      panelTab('img');
      setTimeout(()=>{const p=document.getElementById('img-prompt');if(p){p.value=tool.prompt+(topic||'');p.focus();}},80);
      if(typeof msg==='function')msg(topic?T.imageReady:T.imageHint,'ok');
      return;
    }
    if(tool.target==='rag'){
      panelTab('rag');
      if(typeof msg==='function')msg(T.ragHint,'ok');
      return;
    }
    panelTab('chat');
    ...
    setTimeout(()=>{
      ...
      if(input){input.value=tool.prompt+(topic||'');input.focus();}
      if(topic&&typeof sendMsg==='function')sendMsg();
      else if(typeof msg==='function')msg(tool.title+' '+T.ready,'ok');
    },120);
  }

  window.useAITool=function(id){const tool=getPack(id);if(tool)runPack(tool);};
  try{useAITool=window.useAITool}catch(e){}
  window.copyAIToolPrompt=function(id){
    const tool=getPack(id);if(!tool)return;
    navigator.clipboard?.writeText(tool.prompt+(selectedTopic()||'')).then(()=>msg(T.copied,'ok')).catch(()=>msg(T.copyFail,'err'));
  };
  try{copyAIToolPrompt=window.copyAIToolPrompt}catch(e){}

  window.renderAIToolsHub=function(){
    const root=document.getElementById('ai-tools-page');if(!root)return;
    if(typeof AI_TOOL_PACKS!=='undefined'&&Array.isArray(AI_TOOL_PACKS))AI_TOOL_PACKS.splice(0,AI_TOOL_PACKS.length,...packs);
    const cats=[...new Set(packs.map(t=>t.cat))];
    const freeCount=typeof ALL_MODELS!=='undefined'?ALL_MODELS.filter(m=>m.tier==='free').length:0;
    const providerCount=typeof ALL_MODELS!=='undefined'?new Set(ALL_MODELS.map(m=>m.provider||'other')).size:0;
    ...
    root.innerHTML=`<section class="ai-tools-hero ai-tools-runner">
      <div class="ai-tools-copy">
        <span class="tools-kicker"><i></i> ${T.toolsKicker}</span>
        <h2>${T.toolsTitle}</h2>
        <p>${T.toolsDesc}</p>
        <div class="ai-tool-runbar">
          <input id="ai-tool-topic" type="text" placeholder="${T.topicPh}" autocomplete="off">
          <button type="button" onclick="useAITool('prompt-score')">${iconSvg('sparkles',16)} ${T.promptImprove}</button>
          <button type="button" onclick="useAITool('support-widget')">${iconSvg('message',16)} ${T.supportBot}</button>
        </div>
      </div>
      <div class="ai-tools-lab" aria-hidden="true">
        <div class="tools-lab-head"><span></span><span></span><span></span><b>${T.liveFlow}</b></div>
        <div class="tools-lab-flow"><i>${T.step1}</i><i>${T.step2}</i><i>${T.step3}</i></div>
        <div class="tools-lab-bars"><span></span><span></span><span></span><span></span></div>
      </div>
    </section>
    <section class="ai-tools-stats">
      <div><strong>${packs.length}</strong><span>${T.runnable}</span></div>
      <div><strong>${cats.length}</strong><span>${T.category}</span></div>
      <div><strong>${freeCount}</strong><span>${T.freeModel}</span></div>
      <div><strong>${providerCount}</strong><span>${T.provider}</span></div>
    </section>
    <section class="ai-tools-section">
      <div class="section-title"><h3>${T.workflows}</h3><p>${T.workflowsDesc}</p></div>
      <div class="ai-tools-grid">${packs.map((t,i)=>`<article class="ai-tool-card" style="--tool-accent:${colors[i%colors.length]}">
        <div class="tool-card-top"><span>${iconSvg(t.icon,22)}</span><em>${esc(t.cat)}</em></div>
        <h4>${esc(t.title)}</h4><p>${esc(t.desc)}</p>
        <div class="tool-card-actions"><button type="button" onclick="useAITool('${jsStr(t.id)}')">${T.run}</button><button type="button" onclick="copyAIToolPrompt('${jsStr(t.id)}')">${T.copy}</button></div>
      </article>`).join('')}</div>
    </section>`;
    window.fixTurkishUiText&&window.fixTurkishUiText(root);
  };
  try{renderAIToolsHub=window.renderAIToolsHub}catch(e){}

  window.useProPrompt=function(index){
    const item=pro[index];if(!item)return;
    panelTab('chat');
    setTimeout(()=>{const ta=document.getElementById('chat-in')||document.getElementById('chat-input');if(ta){ta.value=item.prompt;ta.focus();}if(typeof msg==='function')msg(T.templateMoved,'ok');},80);
  };
  try{useProPrompt=window.useProPrompt}catch(e){}
  ...
  window.renderPromptMarketPro=function(){
    ...
    if(!host){host=document.createElement('section');host.id='pro-prompt-market';host.className='pro-market-section';page.prepend(host);}
    host.innerHTML=`<div class="pro-section-head">
      <div><span class="pro-kicker"><i></i> ${T.market}</span><h3>${T.marketTitle}</h3><p>${T.marketDesc}</p></div>
      <button type="button" onclick="panelTab('chat')">${iconSvg('message',15)} ${T.goChat}</button>
    </div>
    <div class="pro-prompt-grid">${pro.map((p,i)=>`<button type="button" class="pro-prompt-card" onclick="useProPrompt(${i})"><span>${esc(p.cat)}</span><strong>${esc(p.title)}</strong><em>${esc(p.desc)}</em></button>`).join('')}</div>`;
    window.fixTurkishUiText&&window.fixTurkishUiText(host);
  };
  try{renderPromptMarketPro=window.renderPromptMarketPro}catch(e){}

  window.renderPrompts=function(cat='all'){
    window.__activePromptCat=cat;
    const catsEl=document.getElementById('prompt-cats');
    const grid=document.getElementById('prompt-grid');
    if(!catsEl||!grid||typeof PROMPT_CATS==='undefined'||typeof PROMPT_LIB==='undefined')return;
    const favs=typeof getPromptFavorites==='function'?getPromptFavorites():[];
    catsEl.innerHTML=PROMPT_CATS.map(c=>`<button class="tool-chip ${cat===c.id?'active':''}" onclick="renderPrompts('${c.id}')">${figIcon(iconForEmoji(c.icon),'inline')} ${esc(c.name)}${c.id==='fav'?` <span>${favs.length}</span>`:''}</button>`).join('');
    const filtered=cat==='all'?PROMPT_LIB:(cat==='fav'?PROMPT_LIB.filter(p=>favs.includes(promptKey(p))):PROMPT_LIB.filter(p=>p.cat===cat));
    if(cat==='fav'&&!filtered.length){grid.innerHTML=`<div class="prompt-empty-v188">${T.favEmpty}</div>`;return;}
    ...
    grid.innerHTML=filtered.map((p,i)=>{
      const key=promptKey(p), isFav=favs.includes(key), grad=colors[i%colors.length];
      return `<div class="card prompt-card" onclick="usePrompt('${esc(p.prompt).replace(/'/g,"\\\\'")}')">
        <button class="prompt-fav-btn ${isFav?'on':''}" onclick="return togglePromptFavorite('${jsStr(key)}',event)" title="${isFav?T.favRemoveTitle:T.favAddTitle}">\u2605</button>
        ...
        <h3>${esc(p.title)}</h3><p>${esc(p.desc)}</p><div class="prompt-card-cta">${T.addChat}</div>
      </div>`;
    }).join('');
    if(typeof upgradeEmojiFigures==='function')upgradeEmojiFigures(grid);
    window.fixTurkishUiText&&window.fixTurkishUiText(document.getElementById('ptab-prompts'));
  };
  try{renderPrompts=window.renderPrompts}catch(e){}

  const priorFav=window.togglePromptFavorite || (typeof togglePromptFavorite==='function'?togglePromptFavorite:null);
  window.togglePromptFavorite=function(key,event){
    if(event){event.preventDefault();event.stopPropagation();}
    const favs=typeof getPromptFavorites==='function'?getPromptFavorites():[];
    const next=favs.includes(key)?favs.filter(x=>x!==key):[key,...favs].slice(0,60);
    if(typeof LS!=='undefined')LS.set('ap_prompt_favorites',next);
    window.renderPrompts(window.__activePromptCat||'all');
    if(typeof msg==='function')msg(next.includes(key)?T.favAdd:T.favRemove,'ok');
    if(priorFav&&false)return priorFav(key,event);
    return false;
  };
  try{togglePromptFavorite=window.togglePromptFavorite}catch(e){}

  function cleanSupport(){
    const support=document.getElementById('ptab-support');if(!support)return;
    const old=document.getElementById('support-contact-card-v306');if(old)old.remove();
    let card=document.getElementById('support-contact-card-v322');
    if(!card){card=document.createElement('div');card.id='support-contact-card-v322';card.className='support-contact-card-v306 support-contact-card-v322';}
    card.innerHTML=`<div><strong>${T.directSupport}</strong><span>${T.directSupportDesc}</span></div><a href="mailto:destek@froxyai.com">destek@froxyai.com</a>`;
    if(!card.parentNode){
      const anchor=support.querySelector('.sup-stats')||support.querySelector('.sup-cta-row')||support.querySelector('.sp4-wrap');
      if(anchor)anchor.insertAdjacentElement('afterend',card);else support.prepend(card);
    }
    window.fixTurkishUiText&&window.fixTurkishUiText(support);
  }

  function runFinalSurfaces(){
    if(document.querySelector('#ptab-tools.on'))window.renderAIToolsHub();
    if(document.querySelector('#ptab-prompts.on')){window.renderPromptMarketPro();window.renderPrompts(window.__activePromptCat||'all');}
    if(document.querySelector('#ptab-support.on'))cleanSupport();
  }

  const prevTab=window.panelTab || (typeof panelTab==='function'?panelTab:null);
  if(prevTab&&!window.__surfacesPanelTabWrappedV323){
    window.__surfacesPanelTabWrappedV323=true;
    window.panelTab=function(tab){
      const out=prevTab.apply(this,arguments);
      if(['tools','prompts','support'].includes(tab))setTimeout(runFinalSurfaces,180);
      return out;
    };
    try{panelTab=window.panelTab}catch(e){}
  }

  const schedule=()=>[120,500,1100,2200,4200,7200].forEach(d=>setTimeout(runFinalSurfaces,d));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule);else schedule();
})();

/* v322: clean Turkish copy and calmer premium surfaces for AI tools, prompts and support. */
(function(){
  if(window.__froxyCleanAppSurfacesV322)return;
  window.__froxyCleanAppSurfacesV322=true;

  const cleanToolPacks=[
    {id:'slides',cat:'İçerik',icon:'file',title:'AI Sunum Oluşturucu',desc:'Konuya göre slayt başlıkları, konuşmacı notu ve görsel önerisi çıkarır.',target:'chat',prompt:'Türkçe cevap ver. Aşağıdaki konu için profesyonel bir sunum hazırla. Çıktı formatı: 10 slayt başlığı, her slayt için kısa konuşmacı notu, görsel önerisi ve kapanış CTA. Konu: '},
    {id:'landing',cat:'Kod',icon:'code',title:'Landing Page Builder',desc:'Ürün fikrinden modern, responsive ve hızlı tek sayfa taslağı üretir.',target:'chat',prompt:'Türkçe cevap ver. Aşağıdaki ürün için modern, responsive, hızlı ve profesyonel tek dosyalık HTML/CSS/JS landing page kodu üret. Kod çalıştırılabilir olsun. Ürün: '},
    {id:'brand',cat:'Tasarım',icon:'palette',title:'Marka Kiti',desc:'Slogan, renk paleti, font, logo yönü ve sosyal medya kimliği önerir.',target:'chat',prompt:'Türkçe cevap ver. Aşağıdaki marka için premium marka kiti oluştur: konumlandırma, slogan, renk paleti, font önerisi, logo yönü, sosyal medya bio ve örnek marka sesi. Marka: '},
    {id:'product-photo',cat:'Görsel',icon:'image',title:'Ürün Fotoğraf Stüdyosu',desc:'Ürün görseli için reklam odaklı prompt ve varyasyon hazırlar.',target:'img',prompt:'premium product photography, clean studio lighting, e-commerce hero image, sharp focus, high conversion ad visual, product: '},
  ...
  function toolByIdV322(id){
    return cleanToolPacks.find(t=>t.id===id) || (typeof aiToolById==='function'?aiToolById(id):null);
  }
  ...
  function runToolPromptV322(tool,topic){
    const finalPrompt=tool.prompt+(topic||'');
    if(typeof panelTab==='function')panelTab('chat');
    if(typeof newChat==='function')newChat();
    setTimeout(()=>{
      const sel=document.getElementById('model-sel');
      if(sel && (!sel.value || (typeof ALL_MODELS!=='undefined' && ALL_MODELS.find(m=>m.id===sel.value)?.cat==='image'))){
        ...
        if(pick){
          sel.value=pick;
          if(typeof LS!=='undefined')LS.set('ap_selected_model',pick);
      ...
      if(topic && typeof sendMsg==='function')sendMsg();
      else if(typeof msg==='function')msg(tool.title+' hazır. Konuyu tamamlayıp gönderebilirsin.','ok');
    },120);
  }

  window.useAITool=function(id){
    const tool=toolByIdV322(id);
    if(!tool)return;
    const topic=topicV322();
    if(tool.target==='img'){
      if(typeof panelTab==='function')panelTab('img');
      setTimeout(()=>{const p=document.getElementById('img-prompt');if(p){p.value=tool.prompt+(topic||'');p.focus();}},80);
      if(typeof msg==='function')msg(topic?'Görsel promptu hazırlandı. Üret butonuna basabilirsin.':'Ürün veya konu yazarsan prompt daha net olur.','ok');
      return;
    }
    if(tool.target==='rag'){
      if(typeof panelTab==='function')panelTab('rag');
      if(typeof msg==='function')msg('Bilgi bankasına dosyanı yükle, sonra chat içinde bu akışı kullanabilirsin.','ok');
      return;
    }
    runToolPromptV322(tool,topic);
  };
  try{useAITool=window.useAITool}catch(e){}
  ...
  window.copyAIToolPrompt=function(id){
    const tool=toolByIdV322(id);
    if(!tool)return;
    const text=tool.prompt+(topicV322()||'');
    navigator.clipboard?.writeText(text).then(()=>msg('Prompt kopyalandı','ok')).catch(()=>msg('Kopyalama desteklenmedi','err'));
  };
  try{copyAIToolPrompt=window.copyAIToolPrompt}catch(e){}

  window.renderAIToolsHub=function(){
    const root=document.getElementById('ai-tools-page');
    if(!root)return;
    if(typeof AI_TOOL_PACKS!=='undefined' && Array.isArray(AI_TOOL_PACKS))AI_TOOL_PACKS.splice(0,AI_TOOL_PACKS.length,...cleanToolPacks);
    const cats=[...new Set(cleanToolPacks.map(t=>t.cat))];
    const freeCount=typeof ALL_MODELS!=='undefined'?ALL_MODELS.filter(m=>m.tier==='free').length:0;
    const providerCount=typeof ALL_MODELS!=='undefined'?new Set(ALL_MODELS.map(m=>m.provider||'other')).size:0;
    const colors=['#7c3aed','#2563eb','#ec4899','#f59e0b','#22c55e','#06b6d4','#ef4444','#8b5cf6','#14b8a6','#64748b'];
    root.innerHTML=`<section class="ai-tools-hero ai-tools-runner">
      <div class="ai-tools-copy">
        ...
        <h2>Hazır araç seç, işi sohbette çalıştır</h2>
        <p>Sunum, SEO, sosyal medya, marka kiti, ürün görseli, destek botu ve dosya sohbeti gibi işleri tek tıkla doğru prompt ve doğru panel akışına taşı.</p>
        <div class="ai-tool-runbar">
          <input id="ai-tool-topic" type="text" placeholder="Konu, ürün, sektör veya kısa brief yaz..." autocomplete="off">
      ...
      <div class="ai-tools-lab" aria-hidden="true">
        ...
        <div class="tools-lab-flow"><i>1. Brief alındı</i><i>2. Araç promptu kuruldu</i><i>3. Sohbete aktarıldı</i></div>
        <div class="tools-lab-bars"><span></span><span></span><span></span><span></span></div>
      </div>
    </section>
    <section class="ai-tools-stats">
      <div><strong>${cleanToolPacks.length}</strong><span>çalıştırılabilir araç</span></div>
    ...
    <section class="ai-tools-section">
      ...
      <div class="ai-tools-grid">${cleanToolPacks.map((t,i)=>`<article class="ai-tool-card" style="--tool-accent:${colors[i%colors.length]}">
        <div class="tool-card-top"><span>${iconSvg(t.icon,22)}</span><em>${esc(t.cat)}</em></div>
        <h4>${esc(t.title)}</h4><p>${esc(t.desc)}</p>
  ...
  window.renderPromptMarketPro=function(){
    ...
    <div class="pro-prompt-grid">${cleanProPrompts.map((p,i)=>`<button type="button" class="pro-prompt-card" onclick="useProPrompt(${i})">
  ...
  window.renderPrompts=function(cat='all'){
    ...
    grid.innerHTML=filtered.map((p,i)=>{
      ...
      return `<div class="card prompt-card" onclick="usePrompt('${esc(p.prompt).replace(/'/g,"\\\\'")}')">
        ...
        <div class="prompt-card-cta">Tıkla, sohbete ekle</div>
      </div>`;
    }).join('');
    if(typeof upgradeEmojiFigures==='function')upgradeEmojiFigures(grid);
  };
  ...
  const oldToggleFavorite=window.togglePromptFavorite || (typeof togglePromptFavorite==='function'?togglePromptFavorite:null);
  window.togglePromptFavorite=function(key,event){
    const result=oldToggleFavorite?oldToggleFavorite(key,event):false;
    try{
      const favs=typeof getPromptFavorites==='function'?getPromptFavorites():[];
      if(typeof msg==='function')msg(favs.includes(key)?'Prompt favorilere eklendi':'Prompt favorilerden çıkarıldı','ok');
    }catch(e){}
    return result;
  };
  try{togglePromptFavorite=window.togglePromptFavorite}catch(e){}

  function cleanSupportContactV322(){
    const support=document.getElementById('ptab-support');
    if(!support)return;
    const old=document.getElementById('support-contact-card-v306');
    if(old)old.remove();
    if(document.getElementById('support-contact-card-v322'))return;
    const card=document.createElement('div');
    card.id='support-contact-card-v322';
    card.className='support-contact-card-v306 support-contact-card-v322';
  ...
  function runCleanV322(){
    if(document.querySelector('#ptab-tools.on'))window.renderAIToolsHub();
    if(document.querySelector('#ptab-prompts.on')){
      window.renderPromptMarketPro();
      window.renderPrompts(window.__activePromptCat||'all');
    }
    if(document.querySelector('#ptab-support.on'))cleanSupportContactV322();
  }

  const prevPanelTab=window.panelTab || (typeof panelTab==='function'?panelTab:null);
  if(prevPanelTab&&!window.__cleanSurfacesPanelTabWrappedV322){
    window.__cleanSurfacesPanelTabWrappedV322=true;
    window.panelTab=function(tab){
      const result=prevPanelTab.apply(this,arguments);
      if(['tools','prompts','support'].includes(tab))setTimeout(runCleanV322,150);
      return result;
    };
    try{panelTab=window.panelTab}catch(e){}
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{[250,900,1800,3200].forEach(d=>setTimeout(runCleanV322,d));});
  else [120,700,1600,3000].forEach(d=>setTimeout(runCleanV322,d));
})();
...
/* v324: last-mile route polish runner. This intentionally lives at EOF so older UI layers cannot override it. */
(function(){
  if(window.__froxyFinalRoutePolishV324)return;
  window.__froxyFinalRoutePolishV324=true;

  const cleanText={
    toolsTitle:'Satış sayfası, reklam ve içerik akışlarını AI ile üret',
    toolsDesc:'Brief yaz, aracı seç, çıktıyı sohbette hazır al. Landing page, kısa reklam videosu, Instagram mini site ve UI section promptları satışa dönük şekilde hazırlanır.',
    topicPh:'Örn: AI model aboneliği satan SaaS, hedef kitle freelancer ve ajanslar...',
    promptMarket:'Haz\u0131r profesyonel ak\u0131\u015flar',
    promptDesc:'Sat\u0131\u015f, kod, destek, analiz ve g\u00f6rsel \u00fcretim i\u00e7in h\u0131zl\u0131 ba\u015flang\u0131\u00e7 \u015fablonlar\u0131.',
    supportTitle:'Nas\u0131l yard\u0131mc\u0131 olabiliriz?',
    supportDesc:'Sorunlar\u0131n\u0131z\u0131 an\u0131nda \u00e7\u00f6z\u00fcme yakla\u015ft\u0131r\u0131n. API, fatura, g\u00f6rsel \u00fcretim ve model ak\u0131\u015flar\u0131 i\u00e7in destek taleplerinizi tek merkezden y\u00f6netin.',
    directSupport:'Do\u011frudan destek',
    directSupportDesc:'Ticket d\u0131\u015f\u0131nda e-posta ile de ula\u015fabilirsin.'
  };
  ...
  function polishTools(){
    const page=document.getElementById('ai-tools-page');
    if(!page)return;
    setText('.ai-tools-copy h2',cleanText.toolsTitle,page);
    setText('.ai-tools-copy p',cleanText.toolsDesc,page);
    const input=page.querySelector('#ai-tool-topic');
    if(input)input.placeholder=cleanText.topicPh;
    const ticker=page.querySelector('.tools-kicker');
    if(ticker)ticker.innerHTML='<i></i> AI ara\u00e7 merkezi';
    page.querySelectorAll('.ai-tool-card').forEach(card=>{
      card.dataset.tiltInit='1';
      card.classList.remove('tilt-card','tool-card-pressed','tool-card-running');
      card.style.transform='';
      card.style.transition='';
      const actions=card.querySelectorAll('.tool-card-actions button');
      if(actions[0])actions[0].textContent='\u00c7al\u0131\u015ft\u0131r';
      if(actions[1])actions[1].textContent='Kopyala';
    });
    window.fixTurkishUiText&&window.fixTurkishUiText(page);
  }

  function polishPrompts(){
    const page=document.querySelector('#ptab-prompts .panel-page');
    if(!page)return;
    const market=page.querySelector('#pro-prompt-market');
    if(market){
      setText('.pro-section-head h3',cleanText.promptMarket,market);
      setText('.pro-section-head p',cleanText.promptDesc,market);
      const btn=market.querySelector('.pro-section-head button');
      if(btn)btn.innerHTML=(typeof iconSvg==='function'?iconSvg('message',15):'')+' Sohbete ge\u00e7';
    }
    page.querySelectorAll('.prompt-card-cta').forEach(el=>el.textContent='T\u0131kla, sohbete ekle');
    page.querySelectorAll('.prompt-fav-btn').forEach(btn=>{
      const on=btn.classList.contains('on');
      btn.textContent='\u2605';
      btn.title=on?'Favoriden \u00e7\u0131kar':'Favoriye ekle';
    });
    page.querySelectorAll('.prompt-card,.pro-prompt-card').forEach(card=>{
      card.dataset.tiltInit='1';
      card.classList.remove('tilt-card','card-pressed-v189');
      card.style.transform='';
      card.style.transition='';
    });
    window.fixTurkishUiText&&window.fixTurkishUiText(page);
  ...
  function polishSupport(){
    const support=document.getElementById('ptab-support');
    if(!support)return;
    setText('.sup-pro-copy h1',cleanText.supportTitle,support);
    setText('.sup-pro-copy p',cleanText.supportDesc,support);
    const old=document.getElementById('support-contact-card-v306');
    if(old)old.remove();
    let card=document.getElementById('support-contact-card-v322');
    if(!card){
    ...
    card.innerHTML='<div><strong>'+cleanText.directSupport+'</strong><span>'+cleanText.directSupportDesc+'</span></div><a href="mailto:destek@froxyai.com">destek@froxyai.com</a>';
    if(!card.parentNode){
      const anchor=support.querySelector('.sup-stats')||support.querySelector('.sup-cta-row')||support.querySelector('.sp4-wrap');
      if(anchor)anchor.insertAdjacentElement('afterend',card);
      else support.prepend(card);
  ...
  function polishActive(){
    if(document.querySelector('#ptab-tools.on'))polishTools();
    if(document.querySelector('#ptab-prompts.on'))polishPrompts();
    if(document.querySelector('#ptab-support.on'))polishSupport();
  }

  const prevPanelTab=window.panelTab || (typeof panelTab==='function'?panelTab:null);
  if(prevPanelTab&&!window.__routePolishPanelTabWrappedV324){
    window.__routePolishPanelTabWrappedV324=true;
    window.panelTab=function(tab){
      const out=prevPanelTab.apply(this,arguments);
      if(['tools','prompts','support'].includes(tab))setTimeout(polishActive,260);
      return out;
    };
    try{panelTab=window.panelTab}catch(e){}
  }

  const prevToolsRender=window.renderAIToolsHub || (typeof renderAIToolsHub==='function'?renderAIToolsHub:null);
  if(prevToolsRender&&!window.__toolsRenderPolishWrappedV324){
    window.__toolsRenderPolishWrappedV324=true;
    window.renderAIToolsHub=function(){
      const out=prevToolsRender.apply(this,arguments);
      setTimeout(polishTools,0);
      setTimeout(polishTools,80);
      return out;
    };
    try{renderAIToolsHub=window.renderAIToolsHub}catch(e){}
  }

  [160,520,1200,2400,3000,3800,5200,8200].forEach(delay=>setTimeout(polishActive,delay));
})();

/* v341: conversion-focused growth tools from creator/AI-site inspiration.
   Kept at EOF so older AI tools layers cannot reintroduce broken copy. */
(function(){
  if(window.__froxyGrowthToolsV341)return;
  window.__froxyGrowthToolsV341=true;
  ...
  const growthTools=[
    {
      ...
      cat:'Satış',
      icon:'target',
      title:'AI Landing Page Üretici',
      desc:'Sektör, hedef kitle ve teklif bilgisiyle dönüşüm odaklı hero, CTA, fiyat, güven ve SEO metni çıkarır.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki brief için yüksek dönüşüm odaklı bir landing page paketi hazırla. Çıktı formatı: 1) Tek cümle değer önerisi, 2) Hero başlığı ve alt metin, 3) Birincil/ikincil CTA, 4) 5 saniyede anlaşılacak açıklama, 5) 4 özellik kartı, 6) Güven/kanıt alanı, 7) Fiyatlandırma önerisi, 8) FAQ, 9) SEO title ve meta description, 10) sayfayı tasarlayacak kısa UI promptu. Brief: '
    },
    {
      id:'ad-video-10s',
      cat:'Reklam',
      icon:'video',
      title:'10 Saniye Reklam Promptu',
      desc:'Instagram, TikTok ve Shorts için hook, sahne akışı, seslendirme, altyazı ve üretim promptu verir.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki ürün veya teklif için 10 saniyelik kısa video reklam paketi hazırla. Çıktı formatı: 1) hedef kitle, 2) ilk 2 saniye hook, 3) saniye saniye sahne planı, 4) ekrandaki yazılar, 5) seslendirme metni, 6) CapCut/Veo/Gemini video promptu, 7) başlık, açıklama ve hashtagler, 8) A/B test için 3 alternatif hook. Ürün/teklif: '
    },
    {
      id:'instagram-mini-site',
      cat:'Satış',
      icon:'globe',
      title:'Instagram Mini Site Üretici',
      desc:'Profil bio, hizmet ve linklerden Link-in-bio tarzı mini satış sayfası yapısını hazırlar.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki Instagram profili veya iş fikri için link-in-bio mini site paketi hazırla. Çıktı formatı: bio metni, hero, 5 hızlı link, güven alanı, teklif kartı, WhatsApp/DM CTA, renk/stil önerisi ve SEO açıklaması. Brief: '
    },
    {
      id:'ui-section-lab',
      cat:'Tasarım',
      icon:'layers',
      title:'UI Section Prompt Üretici',
      desc:'Hero, pricing, dashboard, trust veya support bölümü için profesyonel tasarım promptu üretir.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki bölüm ihtiyacı için premium SaaS UI tasarım promptu hazırla. Çıktı formatı: amaç, bilgi mimarisi, layout, renk/ışık dili, mikro animasyonlar, mobil davranış, component listesi ve uygulanabilir HTML/CSS notları. Bölüm ihtiyacı: '
    },
    {
      id:'slides',
      cat:'İçerik',
      ...
      desc:'Konuya göre slayt başlıkları, konuşmacı notu ve görsel önerisi çıkarır.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki konu için profesyonel bir sunum hazırla. Çıktı formatı: 10 slayt başlığı, her slayt için kısa konuşmacı notu, görsel önerisi ve kapanış CTA. Konu: '
    },
    {
      id:'brand',
      cat:'Tasarım',
      icon:'palette',
      title:'Marka Kiti',
      desc:'Slogan, renk paleti, font, logo yönü ve sosyal medya kimliği önerir.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki marka için premium marka kiti oluştur: konumlandırma, slogan, renk paleti, font önerisi, logo yönü, sosyal medya bio ve örnek marka sesi. Marka: '
    },
    {
      id:'product-photo',
      cat:'Görsel',
      icon:'image',
      title:'Ürün Fotoğraf Stüdyosu',
      desc:'Ürün görseli için reklam odaklı prompt ve varyasyon hazırlar.',
      target:'img',
      prompt:'premium product photography, clean studio lighting, e-commerce hero image, sharp focus, high conversion ad visual, product: '
    },
    {
      id:'prompt-score',
      cat:'Prompt',
      icon:'sparkles',
      title:'Prompt İyileştirici',
      desc:'Promptu puanlar, eksiklerini bulur ve daha güçlü sürüm yazar.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki promptu kalite, netlik, bağlam, çıktı formatı ve risk açısından 100 üzerinden puanla. Sonra daha iyi bir versiyonunu yaz. Prompt: '
    },
    {
      id:'seo',
      cat:'Pazarlama',
      icon:'search',
      title:'SEO İçerik Merkezi',
      desc:'Anahtar kelime, meta açıklama, blog iskeleti ve CTA üretir.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki konu için SEO paketi hazırla: anahtar kelimeler, search intent, meta title, meta description, H2 planı, blog giriş paragrafı ve CTA. Konu: '
    },
    {
      id:'social',
      cat:'Pazarlama',
      ...
      desc:'30 günlük içerik planı, hook, caption ve hashtag üretir.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki marka/niş için 30 günlük sosyal medya takvimi hazırla. Her gün: format, hook, caption, CTA ve hashtag ver. Niş: '
    },
    {
      id:'doc-chat',
      cat:'RAG',
      icon:'book',
      title:'PDF / Dosya ile Sohbet',
      desc:'Bilgi bankasına geçip doküman soru-cevap akışını başlatır.',
      target:'rag',
      prompt:'Bu dokümanı özetle, riskleri çıkar ve aksiyon listesi oluştur.'
    },
    {
      id:'support-widget',
      cat:'İş',
      icon:'message',
      title:'Destek Botu Widget',
      desc:'Siteye gömülebilecek satış ve destek botu senaryosu yazar.',
      target:'chat',
      prompt:'Türkçe cevap ver. Aşağıdaki sektör için web sitesine gömülecek müşteri destek botu tasarla. Bilgi tabanı, karşılama mesajı, lead toplama soruları, fiyat itirazları ve cevap akışı ver. Sektör: '
    }
  ];

  function growthTopic(){
    return (document.getElementById('ai-tool-topic')?.value||'').trim();
  }

  function growthToolById(id){
    return growthTools.find(t=>t.id===id) || null;
  }

  function putPromptIntoChat(tool,topic){
    const finalPrompt=tool.prompt+(topic||'');
    if(typeof panelTab==='function')panelTab('chat');
    if(typeof newChat==='function')newChat();
    setTimeout(()=>{
      ...
      if(input){input.value=finalPrompt;input.focus();}
      if(topic && typeof sendMsg==='function')sendMsg();
      else if(typeof msg==='function')msg(tool.title+' hazır. Briefi tamamlayıp gönderebilirsin.','ok');
    },140);
  }

  window.useAITool=function(id){
    const tool=growthToolById(id);
    if(!tool)return;
    const topic=growthTopic();
    if(tool.target==='img'){
      if(typeof panelTab==='function')panelTab('img');
      setTimeout(()=>{
        const p=document.getElementById('img-prompt');
        if(p){p.value=tool.prompt+(topic||'');p.focus();}
      },100);
      if(typeof msg==='function')msg('Görsel promptu hazırlandı. İstersen modeli seçip üretime geç.','ok');
      return;
    }
    if(tool.target==='rag'){
      if(typeof panelTab==='function')panelTab('rag');
      if(typeof msg==='function')msg('Bilgi bankasına dosyanı yükle; ardından doküman sohbetini başlatabilirsin.','ok');
      return;
    }
    putPromptIntoChat(tool,topic);
  };

  window.copyAIToolPrompt=function(id){
    const tool=growthToolById(id);
    if(!tool)return;
    const text=tool.prompt+(growthTopic()||'');
    navigator.clipboard?.writeText(text).then(()=>msg('Prompt kopyalandı','ok')).catch(()=>msg('Kopyalama desteklenmedi','err'));
  };
  ...
  window.renderAIToolsHub=function(){
    const root=document.getElementById('ai-tools-page');
    if(!root)return;
    if(typeof AI_TOOL_PACKS!=='undefined'&&Array.isArray(AI_TOOL_PACKS))AI_TOOL_PACKS.splice(0,AI_TOOL_PACKS.length,...growthTools);
    const cats=[...new Set(growthTools.map(t=>t.cat))];
    const freeCount=typeof ALL_MODELS!=='undefined'?ALL_MODELS.filter(m=>m.tier==='free').length:0;
    const providerCount=typeof ALL_MODELS!=='undefined'?new Set(ALL_MODELS.map(m=>m.provider||'other')).size:0;
    const colors=['#22d3ee','#8b5cf6','#3b82f6','#14b8a6','#ec4899','#f59e0b','#22c55e','#64748b'];
    root.innerHTML=`<section class="ai-tools-hero ai-tools-runner growth-tools-hero-v341">
      <div class="ai-tools-copy">
        <span class="tools-kicker"><i></i> Growth araç merkezi</span>
        <h2>Satış sayfası, reklam ve içerik akışlarını AI ile üret</h2>
        <p>Instagram’da gördüğümüz “tek fikirden çalışan araç” mantığını Froxy’ye taşıdık: brief yaz, aracı seç, çıktı sohbette hazır olsun.</p>
        <div class="ai-tool-runbar">
          ...
          <button type="button" onclick="useAITool('landing-funnel')">${iconSvg('target',16)} Landing üret</button>
          <button type="button" onclick="useAITool('ad-video-10s')">${iconSvg('video',16)} Reklam promptu</button>
        </div>
      </div>
      <div class="growth-lab-preview-v341" aria-hidden="true">
        <div class="growth-lab-top"><span></span><span></span><span></span><b>Froxy Growth Lab</b></div>
        <div class="growth-lab-card is-main"><small>Brief</small><strong>Ürünü 5 saniyede anlat</strong></div>
        <div class="growth-lab-grid">
          <i>Hero</i><i>CTA</i><i>Video</i><i>SEO</i>
        </div>
        <div class="growth-lab-line"><span></span></div>
      </div>
    </section>
    <section class="ai-tools-stats">
      <div><strong>${growthTools.length}</strong><span>çalıştırılabilir araç</span></div>
      <div><strong>${cats.length}</strong><span>kategori</span></div>
      <div><strong>${freeCount}</strong><span>free model</span></div>
      <div><strong>${providerCount}</strong><span>sağlayıcı</span></div>
    </section>
    <section class="ai-tools-section">
      <div class="section-title"><h3>Para kazandıracak iş akışları</h3><p>İlk dört kart satış, reklam ve site üretimi için öncelikli. Diğerleri günlük üretim akışlarını hızlandırır.</p></div>
      <div class="ai-tools-grid growth-tools-grid-v341">${growthTools.map((t,i)=>`<article class="ai-tool-card ${i<4?'growth-featured-tool':''}" style="--tool-accent:${colors[i%colors.length]}">
        <div class="tool-card-top"><span>${iconSvg(t.icon,22)}</span><em>${esc(t.cat)}</em></div>
        <h4>${esc(t.title)}</h4><p>${esc(t.desc)}</p>
        <div class="tool-card-actions"><button type="button" data-growth-tool-run="${esc(t.id)}" onclick="useAITool('${jsStr(t.id)}')">Çalıştır</button><button type="button" data-growth-tool-copy="${esc(t.id)}" onclick="copyAIToolPrompt('${jsStr(t.id)}')">Kopyala</button></div>
      </article>`).join('')}</div>
    </section>`;
    window.fixTurkishUiText&&window.fixTurkishUiText(root);
  };
  ...
  try{
    useAITool=window.useAITool;
    copyAIToolPrompt=window.copyAIToolPrompt;
    renderAIToolsHub=window.renderAIToolsHub;
    globalThis.useAITool=window.useAITool;
    globalThis.copyAIToolPrompt=window.copyAIToolPrompt;
    globalThis.renderAIToolsHub=window.renderAIToolsHub;
  }catch(e){}

  if(!window.__growthToolsClickBoundV341){
    window.__growthToolsClickBoundV341=true;
    document.addEventListener('click',function(ev){
      const runBtn=ev.target&&ev.target.closest&&ev.target.closest('[data-growth-tool-run]');
      const copyBtn=ev.target&&ev.target.closest&&ev.target.closest('[data-growth-tool-copy]');
      if(runBtn){
        ev.preventDefault();
        ev.stopPropagation();
        window.useAITool(runBtn.getAttribute('data-growth-tool-run'));
        return;
      }
      if(copyBtn){
        ev.preventDefault();
        ev.stopPropagation();
        window.copyAIToolPrompt(copyBtn.getAttribute('data-growth-tool-copy'));
      }
    },true);
  }

  function runGrowthTools(){
    if(document.querySelector('#ptab-tools.on'))window.renderAIToolsHub();
  }

  const prevPanelTab=window.panelTab || (typeof panelTab==='function'?panelTab:null);
  if(prevPanelTab&&!window.__growthToolsPanelTabWrappedV341){
    window.__growthToolsPanelTabWrappedV341=true;
    window.panelTab=function(tab){
      const out=prevPanelTab.apply(this,arguments);
      if(tab==='tools')setTimeout(runGrowthTools,180);
      return out;
    };
    try{panelTab=window.panelTab}catch(e){}
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>[180,700,1500,3000].forEach(d=>setTimeout(runGrowthTools,d)));
  else [120,620,1400,2800].forEach(d=>setTimeout(runGrowthTools,d));
})();
