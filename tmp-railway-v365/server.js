...
app.post('/api/forgot-password', authLimiter, (req, res) => {
  ...
  try {
    ...
    if(user) {
      ...
      const payload = { success: true, message: '?ifre s?f?rlama ba?lant?s? e-postan?za g?nderildi.' };
      if (process.env.NODE_ENV !== 'production') payload.dev_token = token;
      res.json(payload);
    } else {
      res.json({ success: true, message: '?ifre s?f?rlama ba?lant?s? e-postan?za g?nderildi.' });
    }
  } catch(e) {
    res.status(500).json({error: e.message});
  }
...
app.post('/api/reset-password', authLimiter, (req, res) => {
  ...
  if(!token || !password) return res.status(400).json({error: 'Token ve ?ifre gerekli'});
  ...
  try {
    ...
    res.json({ success: true, message: '?ifreniz ba?ar?yla s?f?rland?.' });
  } catch(e) {
    res.status(500).json({error: e.message});
  }
});
...
app.post('/api/change-password', authMiddleware, authLimiter, (req, res) => {
  ...
  if(newPassword.length < 6) return res.status(400).json({error: '?ifre en az 6 karakter olmal?'});
  try {
...
function localSafeChatAnswer(messages) {
  ...
  return [
    'İsteğini aldım ancak şu an cevap üretirken servis yoğunluğu oluştu.',
    '',
    short ? 'Mesajını aldım: "' + short + '"' : 'Mesajını aldım.',
    'Lütfen biraz sonra tekrar dene veya mesajı daha kısa gönder.'
  ].join('\n');
}
...
app.post('/api/image', chatLimiter, optionalAuthMiddleware, async (req, res) => {
  ...
  if (imgModel === 'stability-core' || imgModel === 'stability-ultra') {
    ...
    if (!stabilityKey) {
      if (model !== 'auto-quality') {
        return res.status(503).json({ error: 'Se?ili Stability modeli i?in API key tan?ml? de?il.', model: imgModel, provider: 'stability', ok: false });
      }
      imgModel = 'flux';
    } else try {
      ...
      if (model !== 'auto-quality') {
        return res.status(502).json({ error: 'Se?ili Stability modeli ?u an yan?t vermedi: ' + err.message, model: imgModel, provider: 'stability', ok: false });
      }
      imgModel = 'flux';
    }
  }
  ...
  if (['flux', 'turbo', 'sana', 'zimage', 'klein', 'gptimage', 'wan-image', 'qwen-image', 'flux-realism', 'flux-anime', 'flux-3d'].includes(imgModel) || imgModel.startsWith('style-')) {
    ...
    if (model === 'auto-quality' && CLOUDFLARE_ACCOUNT_ID && CLOUDFLARE_API_TOKEN) {
      try {
